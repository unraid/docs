...
| ---------- | ------------------------------------ | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 80 / 443   | %%WebGUI\|web-gui%% (HTTP/HTTPS)     | Exposes management interface; risk of interception or brute force if password is weak | Use Unraid Connect or VPN for remote access, noting Unraid Connect requires WAN port forwarding or UPnP (not an always-on cloud relay) |
...
### Avoid exposing the boot device share - or make it private

...
6. In the %%WebGUI|web-gui%%, open a terminal and run `lsblk` to list all drives (excluding the boot device).
...
### My boot device is reporting an invalid GUID. What do I do?
...
- Traditional hardware %%RAID|raid%% cards are _not_ necessary; you can use simple HBAs to allow Unraid to manage drives directly.
...
1. Ensure you have a valid %%parity|parity%% and a current backup of your boot device.
...
### I can't get the USB Flash Creator to install Unraid on my boot media. What do I do?

If the USB Flash Creator isn't working for your system or USB drive, you can use the [manual installation method](../getting-started/set-up-unraid/create-your-bootable-media.mdx#manual-install-method) to prepare your Unraid boot device. This method is compatible with Windows, macOS, and Debian/Ubuntu Linux.
...
#### Option 1: During boot media creation

When using the USB Flash Creator, select the option to enable %%UEFI|uefi%% boot mode before writing Unraid to the USB drive.

#### Option 2: After booting in Legacy Mode

In the %%WebGUI|web-gui%%, open ***Settings → Boot device*** (on Unraid **7.3.0**+). On earlier releases, the same page is under ***Settings → Flash device***.

Enable %%UEFI|uefi%% boot mode and reboot your server.

#### Option 3: Manual folder rename

On the boot volume (USB drive), rename the `EFI-` folder to `EFI` (remove the dash `-`).

Insert the boot device into your server, then enter your motherboard BIOS/%%UEFI|uefi%% settings.

Set the Unraid boot device (USB, when applicable) as the primary boot target and enable %%UEFI|uefi%% boot mode (be sure to disable CSM/Legacy/Compatibility mode, if available).

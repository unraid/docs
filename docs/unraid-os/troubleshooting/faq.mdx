...
1. Stop the %%array|array%% and shut down your server.
1. Remove the USB flash device.
1. Use the Unraid USB Flash Creator tool to reformat and reinstall Unraid onto the flash drive.
1. Reinsert the flash device and boot your server.
1. In the %%WebGUI|web-gui%%, open a terminal and run `lsblk` to list all drives (excluding the flash).
1. Wipe existing filesystems from each data drive:
...
To change your Unraid server's hostname, navigate the %%WebGUI|web-gui%% to **_Settings → System Settings → Identification_**.
...
2. Enter **Maintenance Mode** from the **_Main_** tab.
...
In the %%WebGUI|web-gui%%, head to the Flash Device **_Settings → Flash Device_** page.

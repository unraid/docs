...
| ----------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------- | ------------------------------------------- |
| **Mirror syslog to boot drive** | Captures boot process events                                                             | Can wear out the boot device quickly      | Short-term diagnostics (a few days)         |
...
| **Local %%syslog\|syslog%%**    | Keeps logs on the %%array\|array%% or %%cache\|cache%%, reducing wear on the boot device | Less accessible if there's a system crash | Continuous logging without external devices |
...
<Tabs>
  <TabItem value="mirror" label="Mirror syslog to boot drive" default>
    <SyslogMirror />
  </TabItem>

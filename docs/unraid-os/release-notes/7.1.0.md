...
* If you want to use a USB WiFi adapter, see this list of [USB WiFi adapters that are supported with Linux in-kernel drivers](https://github.com/morrownr/USB-WiFi/blob/main/home/USB_WiFi_Adapters_that_are_supported_with_Linux_in-kernel_drivers.md) [-rc.4]
...
* Fix: Resolve an issue with iptables config that was preventing some WireGuard tunnels from starting. [-rc.4]
* Fix: Improved carrier detection and interface selection for Docker on WiFi. [-rc.4]
* Fix: Various WiFi improvements. [-rc.4]
...
* Fix: Could not change VM icon when using XML view [-rc.4]
...
* Reload the Dashboard and Main pages when they become visible [-rc.2], if ***Settings → Display Settings → Allow realtime updates on inactive browsers*** is disabled [-rc.4]
...
* Fix: update `rc.messagebus` to correct handling of `/etc/machine-id` [-rc.4]
* Diagnostics
  ...
  * Fix: suppress some of those diagnostics errors [-rc.4]

### Linux kernel

* version 6.12.24-Unraid [-rc.3]
  ...
  * CONFIG_PSI: Pressure stall information tracking [-rc.4]
  * CONFIG_PSI_DEFAULT_DISABLED: Require boot parameter to enable pressure stall information tracking, i.e., `psi=1` [-rc.4]
  * CONFIG_ENCLOSURE_SERVICES: Enclosure Services [-rc.4]
  * CONFIG_SCSI_ENCLOSURE:  SCSI Enclosure Support [-rc.4]
  * CONFIG_DRM_ACCEL: Compute Acceleration Framework [-rc.4]
  * CONFIG_DRM_ACCEL_HABANALABS: HabanaLabs AI accelerators [-rc.4]
  * CONFIG_DRM_ACCEL_IVPU: Intel NPU (Neural Processing Unit) [-rc.4]
  * CONFIG_DRM_ACCEL_QAIC:  Qualcomm Cloud AI accelerators [-rc.4]
  * zfs: version 2.3.1
...
* ca-certificates: version 20250425 [-rc.4]
...
* elfutils: version 0.193 [-rc.4]
* ethtool: version 6.14
* firefox: version 128.10 (AppImage) [-rc.4]
* floppy: version 5.6
...
* kernel-firmware: version 20250425_cf6ea3d [-rc.4]
* kmod: version 34.2
* less: version 674
* libSM: version 1.2.6

...
in /mnt/user/<share name> pointing directly to the pool share directory.  (An additional check is made to ensure the share also does
...
Appearing on the Shares page, a button called CLEAN UP, when enabled indicates there are config/share/<share name>.cfg files for

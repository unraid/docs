# Version 7.2.0-beta.2 2025-08-xx
...
Unraid now supports the Ext2/3/4, NTFS and ExFAT [-beta.2] drive formats in addition to XFS, BTRFS, and ZFS.
...
Critical note: you can continue adding filled data drives to the array up until you start the array with a parity drive installed. Once a parity drive has been added, any new data drives will be zerod out when they are added to the array.
...
* Improvement: Enhance multimonitor support, automatically enabling spicevmc when needed
...
* Fix: Resolved various Filemanager issues introduced with the responsive webGUI changes [-beta.2]
* Fix: Text input boxes were hidden on various popups [-beta.2]
* Fix: Some tooltips were missing from the Main page [-beta.2]
* Fix: Footer alignment [-beta.2]

#### Login to the WebGUI via SSO

Login to the Unraid webGUI using Single Sign On with your Unraid.net account or any other OIDC-compliant provider [new to -beta.2]. For details on this *optional* feature, see [OIDC Provider Setup](/API/oidc-provider-setup.md) in the Docs.
...
* Improvement: Require auth on `http://localhost`. This improves security and allows Tailscale Funnel to work with the webGUI. Note that when booting in gui mode, you will now need to login again to access the webGUI.
...
* kbd: version 2.8.0
* kernel-firmware: version 20250808_8f1ce11
* less: version 679

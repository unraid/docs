...
For other known issues, see the [7.0.0 release notes](7.0.0.mdx#known-issues).
...
* mover:
  ...
  * Fix: [Using mover to empty an array disk](7.0.0.mdx#using-mover-to-empty-an-array-disk) now only moves top level folders that have a corresponding share.cfg file, also fixed a bug that prevented the list of files *not moved* from displaying.
...
* If you want to use a USB WiFi adapter, see this list of [USB WiFi adapters that are supported with Linux in-kernel drivers](https://github.com/morrownr/USB-WiFi/blob/main/home/USB_WiFi_Adapters_that_are_supported_with_Linux_in-kernel_drivers.mdx)
...
As a step toward making the webGUI responsive, we have reworked the CSS. For the most part, this should not be noticeable aside from some minor color adjustments. We expect that most plugins will be fine as well, although plugin authors may want to review [this documentation](https://github.com/unraid/webgui/blob/master/emhttp/plugins/dynamix/styles/themes/README.mdx). Responsiveness will continue to be improved in future releases.

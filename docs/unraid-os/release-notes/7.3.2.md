...
Unraid OS 7.3.2-rc.1 refreshes the Linux kernel, Docker, the Unraid API, and a broad set of base packages, with security-related updates across core networking, web, storage, and encryption components. This release also includes ZFS and QEMU updates, along with storage, Docker and other improvements.
...
- Review the rollback notes for Unraid OS 7.3.1 before downgrading from this release.
- Be aware that this release updates the kernel, ZFS, Docker, and the Unraid API; verify compatibility before rolling back.
...
- Fix: carried forward storage-related fixes from the 7.3.2 prerelease series.
...
- Update Unraid API to dynamix.unraid.net 4.35.1 - see changes.

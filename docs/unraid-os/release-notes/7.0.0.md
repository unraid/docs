...
* @SimonF for signicant new features in the Unraid OS VM Manager
...
but will throw error and disable update if invalid characters are found.
...
* For btrfs/zfs single-slot pools, the file system type can be changed any time.
...
In this case to change the file syste type first remove the subpool(s).
  * The pool root slot count can only be changed if the file system type is auto.  Therefore to change
the root slot count first set file system type to auto, change the slot count, and set file system type
back to zfs.
...
* Fix parity-swap case where partition 1 of target device is missing.
...
* rc.docker:
  * Fix handle interfaces with higher index than 0
...
* Show Docker RAM usage on Dashboard [-beta.3]
  * Add Docker RAM usage to Dashboard
...
* Diagnostics: Improved anonymization of IPv6 addresses [-beta.3]
...
* ShareList page: Display warning icon on Share List if storage misconfiguration.
...
* Fix: Mover status next to 'Move' button on 'Array Operation' page
...
* Share List
  * Display warning icon on Share List if storage misconfiguration.
...
* kbd: version 2.6.4
* kernel-firmware: version 20240913_6c88d9b

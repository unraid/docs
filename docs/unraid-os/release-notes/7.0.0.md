...
In -beta.3, if single device XFS pools show as "not installed", see the fix in [this forum post](https://forums.unraid.net/bug-reports/prereleases/unraid-os-version-700-beta3-available-r3269/?do=findComment&comment=30214). This will be resolved in the next release.

In -beta.3, you may see an issue related to `--fd` when starting certain VMs. The temporary fix is in
[this forum post](https://forums.unraid.net/bug-reports/prereleases/700-beta3-cant-start-windows-vm-with-virtiofs-mounts-r3271/?do=findComment&comment=30182).

...
Standalone XFS pools (outside of the array) that were initially created prior to 6.12.0 will become unassigned at first array start in 7.0.0-rc.1. The one-time fix is to manually re-add the pool (this will not cause issues if you choose to roll back). This will be resolved in 7.0.0-rc.2 so the manual step will not not required.

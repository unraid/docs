...
(@TODO:Needs update) - Significant improvements were made to NFS share handling [-rc.2]. If your NFS shares still disappear, edit the `config/share.cfg` file on the flash drive and add `shareNOFILE=65536` to increase the number of open file descriptors available to the shfs process. The default value is 40960, 813384 is the max. Please let us know if this helps.
...
* (@TODO:remove) Bug fix: Single XFS pools created before 6.12.x fail to import with 7.0.0-rc1 [-rc.2]
* (@TODO:remove) Bug fix: XFS disks without an XFS signature don't mount [-rc.2]
* (@TODO:remove) Bug fix: Allocation Method not showing when editing a share [-rc.2]
* (@TODO:remove) Bug fix: When editing a share, don't show secondary storage or mover action if primary storage is array [-rc.2]
* (@TODO:remove) Bug fix: Array-only share doesn't display correctly on the share listing page [-rc.2]
...
* **VM Tab**
  ...
  * noVNC version: 1.5
  * (@TODO:remove) Bug fix: fix SPICE viewer when using non-standard webGUI port [-rc.3]
* **VM Manager Settings**
...
* **Add/edit VM template**
  ...
  * (@TODO:remove) Bug fix: Don't allow quotes in a name of a VM [-rc.2]
  * (@TODO:remove) Bug fix: QCOW2 fix for manual disk additions [-rc.2]
  * (@TODO:remove) Bug fix: Can't create or edit VMs when using translation [-rc.3]
* **Dashboard**
  * Add VM usage statistics to the dashboard, enable on ***Settings → VM Manager → Show VM Usage***
  * (@TODO:remove) Bug fix: Fix vm_dashusage processing when array not started [-rc.2]
...
* (@TODO:remove) Warn if cannot find persistent storage for Tailscale state [-rc.2]
* (@TODO:remove) Let Docker XML authors specify the TailscaleStateDir (see docs) [-rc.2]
* (@TODO:remove) Bug fix: fixes for Mullvad integration [-rc.2]
...
(@TODO:remove) Bug fix: Change password fields to type of password and allow special characters in passwords [-rc.2]
...
* (@TODO:remove) Bug fix: restrict open streams display to disk and user shares [-rc.2]
...
* (@TODO:remove) Bug fix: Tabbed page mode acts erratic at times [-rc.2]
* Bug fix: wrong urls in /etc/os-release [-rc.2]
* (@TODO:remove) Bug fix: mc no longer remains on its current directory after exiting [-rc.2]
* Misc translation fixes [-rc.2]
* Bug fix: JavaScript console logging functionality restored [-rc.3]
* Clicking Unraid version number loads release notes from Unraid Docs website [-rc.3]

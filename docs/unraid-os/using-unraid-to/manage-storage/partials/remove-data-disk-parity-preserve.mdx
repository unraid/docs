...
<details>
  ...
  Since zeroing a disk can take many hours, use a persistent terminal session, such as `tmux` over %%SSH|ssh%% or a terminal plugin installed from [Community Applications](/community-applications/). Review any plugin's maintainer, template, and support details before installing it.

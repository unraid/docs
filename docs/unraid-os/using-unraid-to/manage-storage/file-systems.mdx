...
1. **Copy all data off the drive**: Move your files to another location. This could be another drive in your %%array|array%%, a %%cache pool|cache-pool%%, or an external backup.  
   ...
   - Starting with Unraid 7.2, you can empty an array drive to other array drives via the webGUI:
     ...
     - Navigate to ***Settings → Global Share Settings***. In the **Emptying disk(s)** area, select the disk(s) to empty and **Apply**.
     - Navigate to ***Main → Array Operation*** and start the array
     - Navigate to ***Settings → Scheduler → Mover Settings*** and click **Move Now**
     ...
     - Navigate to ***Settings → Global Share Settings***. In the Emptying disk(s) area, remove any selected disk(s) and Apply.
...
%%ZFS|zfs%% uses **scrubbing** instead of traditional filesystem checks like `fsck`. A %%scrub|scrub%% examines all data in the pool to verify checksums and automatically repairs any corruption found in redundant configurations.  

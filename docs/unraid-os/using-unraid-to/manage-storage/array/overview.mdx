...
| **Expansion**           | Mix drive sizes; add/remove devices dynamically                                                                  | Limited add/remove device support; cannot remove from RAIDZ; single-device add to expand single-vdev RAIDZ in Unraid 7.2 (see [RAIDZ expansion](../cache-pools.mdx#raidz-expansion))   | Add drives, but no striping or performance scaling |

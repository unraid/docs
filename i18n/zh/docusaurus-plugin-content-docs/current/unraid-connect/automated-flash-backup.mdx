...
<details>
  ...
  备份只包括配置文件，确保敏感信息，例如密码和%%WireGuard|wireguard%%密钥，被排除在外。Docker模板XML文件可能是备份的一部分并可能包含应用特定的凭证。

  <h4>数据保留</h4>

  仅保留最新的备份。如果某个备份超出特定限制——具体来说，如果单个文件超过10MB或者总存储库大小超过100MB——它将被删除并重新创建。在您停用闪存备份功能或者经过一段时间的不活动后，备份也将被移除。

  <h4>数据共享</h4>

  闪存备份数据绝不会与第三方共享。它只被存储用于为您提供备份和恢复服务。
</details>

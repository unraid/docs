...
本页面介绍 **Unraid Connect 远程访问**，它通过使用 %%UPnP|upnp%% 或在路由器上手动设置端口转发规则，将您的 %%WebGUI|web-gui%% 发布到互联网。如果您希望在不向 WAN 流量暴露 %%WebGUI|web-gui%% 的情况下进行完整远程管理，建议使用官方的 [Tailscale 集成](../unraid-os/system-administration/secure-your-server/tailscale.mdx)。对于更高级的需求，例如连接 Docker 容器或访问网络驱动器，%%VPN Tunnel|vpn-tunnel%% 仍然是推荐的解决方案。
...
对于大多数用户来说，官方 [Tailscale 集成](../unraid-os/system-administration/secure-your-server/tailscale.mdx) 是远程管理的更佳选择，因为它避免了将 %%WebGUI|web-gui%% 暴露给公共互联网。仅当您明确需要通过浏览器访问面向公网的 %%WebGUI|web-gui%% URL 时，才使用 Unraid Connect 远程访问。
...
动态按需远程访问目前未在 Unraid API 设置页面中提供。如果您需要远程管理，但又不想通过端口转发让 %%WebGUI|web-gui%% 保持暴露，请优先使用 [Tailscale](../unraid-os/system-administration/secure-your-server/tailscale.mdx) 或其他 %%VPN Tunnel|vpn-tunnel%%。

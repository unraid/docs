...
OpenID Connect (OIDC) 是一种身份验证协议，允许用户使用来自 Google、Microsoft 或您企业身份提供商等现有账户登录。它支持单点登录（%%SSO|sso%%），实现无缝且安全的身份验证。

:::

本指南将引导您在 Unraid API 中使用 %%WebGUI|web-gui%% 配置用于 %%SSO|sso%% 身份验证的 OIDC（OpenID Connect）提供商。
...
1. 前往您的 Unraid 服务器的 %%WebGUI|web-gui%%。
...
*显示传统登录表单以及 %%SSO|sso%% 选项的登录页面 - “Login With Unraid.net”和“Sign in with Google”按钮*
...
1. **授权规则模式**: 选择`OR`（任何规则通过）或`AND`（所有规则必须通过）
2. 为每个组添加规则：
   - **声明**: `groups`
   - **操作符**: `contains`
   - **值**: `admins`
...
- **启用开发者沙盒**：切换以启用位于 `/graphql` 的 %%GraphQL|graphql%% 沙盒。
...
**⚠️ 安全注意**：始终尽量使用基本URL格式。系统会自动附加`/.well-known/openid-configuration`以便进行OIDC发现。直接使用完整发现URL会禁用重要的发行者验证检查，OpenID Connect规范不推荐这样做。
...
*Unraid 登录页面同时显示传统的用户名/密码身份验证以及带有自定义提供商按钮的 %%SSO|sso%% 选项*
...
如果遇到问题，请查阅您的提供商的 OIDC 文档，查看 Unraid API 日志以获取详细错误信息，确保您的提供商支持标准 OIDC 发现，并验证 Unraid 与提供商之间的网络连接。如需更多帮助，请访问 [Unraid 论坛](https://forums.unraid.net/)。
...
将协议与您的服务器设置相匹配。如果在未使用 %%SSL|ssl%%/%%TLS|tls%% 的情况下访问您的 Unraid 服务器，请使用 `http://`（本地网络访问通常如此）。如果您已在服务器上配置 %%SSL|ssl%%/%%TLS|tls%%，请使用 `https://`。某些 OIDC 提供商（如 Google）要求使用 HTTPS，不接受 HTTP 重定向 URI。
...
- **选项 2：%%Tailscale|tailscale%%** - 使用 %%Tailscale|tailscale%% 获取一个 Google 会接受的有效 `*.ts.net` 域。有关 %%Tailscale|tailscale%% 的更多信息，请参阅 [远程访问](../unraid-connect/remote-access.mdx)。

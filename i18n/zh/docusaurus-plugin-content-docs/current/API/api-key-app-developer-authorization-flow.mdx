...
范围遵循模式：`资源:操作`。示例包括`docker:read`（读取 Docker 访问权限）、`vm:*`（对虚拟机的完全访问）、`system:update`（系统更新权限）、`role:viewer`（查看者角色权限）和`role:admin`（管理员角色权限）。

可用资源包括`docker`、`vm`、`system`、`share`、`user`、`network`、`disk`等。可用操作为`create`、`read`、`update`、`delete`，或`*`表示所有操作。

重定向 URI 必须使用 HTTPS（用于开发的 localhost 除外）。用户需明确批准每项权限，并且该流程利用现有的 Unraid 认证会话。API 密钥仅显示一次，必须妥善保存。

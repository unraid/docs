...
%%ZFS|zfs%% 是一种现代文件系统和卷管理器，旨在保护您的数据、防止损坏并简化存储管理。
...
Unraid 支持在任何存储池中使用 %%ZFS|zfs%%。您可以创建新的 %%ZFS|zfs%% 池、从其他系统导入现有池，或使用 Unraid 独特的混合 %%ZFS|zfs%% 配置：将 %%ZFS|zfs%% 格式化的磁盘直接添加到 Unraid %%array|array%%（不是池）中，并将 %%ZFS|zfs%% 功能与 Unraid 的 %%parity|parity%% 保护结合使用。
...
在完成之前，请查看有关分配配置文件和拓扑的章节，以便作出合适选择。

...
| 日志 vdev（SLOG）  | 提高同步写入性能              | Optional. Only beneficial for certain workloads.          |
| 缓存 vdev（L2ARC） | 提供基于 SSD 的读取缓存        | Optional. Can improve read speeds for large working sets. |
...
| 缓存（L2ARC）vdev    | 池导入，但读取缓存丢失    | The pool works normally, but you lose the performance boost from the L2ARC cache. No data is lost. |

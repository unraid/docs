...
4. 如果发现并修复了错误，您可以继续使用该设备。如果问题仍然存在或再次出现，建议更换。

单次断电或不完整的写入有时可能导致轻微的损坏，但通常可修复。然而，反复出现的问题表明需要更换新设备。

:::

有两种方法可以替换Unraid USB闪存设备：推荐用于Windows或macOS的[USB闪存创建工具](https://unraid.net/download)，或针对高级用户的手动方法。**始终在开始前备份您的闪存设备。**
...
如果您将试用配置转移到新闪存设备上，您将无法在购买有效注册码前启动%%array|array%%。试用密钥仅在原设备上有效。
...
- 如果您需要在12个月内再次更换密钥，请[联系Unraid支持](https://unraid.net/contact)。为了更快的服务，包含您旧的和新的USB %%GUIDs|guid%%、您的注册码，以及购买时使用的电子邮件地址。
...
- \*\*%%Parity drives|parity-drives%%\*\*没有可挂载的文件系统。如果驱动器无法挂载，则它很可能是%%parity|parity%%驱动器。
...
2. 转到***Main***选项卡
   ...
   - 请勿点击任何驱动器上的**格式化**。如果出现比预期更多的无法挂载的驱动器%%parity|parity%%驱动器，请停止并在论坛中寻求帮助。
...
5. 如果您有单个%%parity|parity%%驱动器，数据盘的顺序并不重要。只有在您确信分配了相同物理校验盘且在原始配置之后没有写入时，才勾选**校验有效**。对于双%%parity|parity%%，数据盘的顺序和校验1与校验2的分配都很重要——校验1和校验2不可互换，如果更改了其中任何一个，则必须重建%%parity|parity%%校验。

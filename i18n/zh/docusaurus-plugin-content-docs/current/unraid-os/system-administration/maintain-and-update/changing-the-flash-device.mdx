...
3. 在任何硬件更改后保留磁盘分配的截图。
   :::
4. 如果发现错误并修复，您可以继续使用该设备。如果问题持续或重复，建议进行更换。

单次断电或不完整的写入有时可能导致轻微的损坏，但通常可修复。然而，重复出现的问题表明需要更换新设备。

:::

有两种方法可以替换 Unraid USB 闪存设备：推荐用于 Windows 或 macOS 的 [USB 闪存创建工具](https://unraid.net/download)，或针对高级用户的手动方法。**在开始之前请务必备份您的闪存设备。**
...
- 如果您需要在12个月内再次更换您的密钥，请[联系Unraid支持](https://unraid.net/contact)。为了更快的服务，请提供旧的和新的USB %%GUIDs|guid%%，您的注册码以及用于购买的电子邮件地址。
...
绝不要将数据盘指定为%%parity drive|parity-drives%%。错误的分配可能会用校验信息覆盖数据，导致数据永久丢失。

:::

:::tip[Identifying 所有驱动器]

- **%%Parity drives|parity-drives%%** 没有可挂载的文件系统。如果驱动无法挂载，很可能是%%parity|parity%%驱动。
...
2. 转到***Main***选项卡
   ...
   - 不要点击任何驱动上的**格式化**。如果出现比预期更多的无法挂载的驱动器%%parity|parity%%驱动器，请停止并在论坛中寻求帮助。
...
5. 如果您有一个%%parity|parity%%驱动，数据盘的顺序并不重要。仅在您确定分配了相同的物理校验盘且自原始配置以来没有发生写入时，才勾选**校验有效**。对于双%%parity|parity%%，数据盘的顺序和校验1与校验2的分配都很重要——校验1和校验2不可互换，如果更改，必须重建%%parity|parity%%。
6. 启动%%array|array%%以提交分配。数据盘应该挂载，其内容应该保持完整。

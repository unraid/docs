...
4. 如果发现并修复了错误，您可以继续使用该设备。如果问题仍然存在或重新出现，建议更换设备。

单次停电或写入不完整有时会导致轻微的损坏，通常可以修复。然而，重复出现的问题则表明需要更换新设备。

:::

更换 Unraid USB 闪存设备有两种方法：推荐使用 [USB 闪存创建工具](https://unraid.net/download) ，适用于 Windows 或 macOS，或为高级用户提供的手动方法。**开始之前请备份您的闪存设备。**
...
如果将试用配置转移到新的闪存设备上，您将无法启动 %%array|array%% ，直到您购买有效的注册密钥为止。试用密钥仅在原始设备上有效。
...
您现在已更换您的 USB 闪存设备并转移了 Unraid 注册密钥。系统将显示注册日期及下次可更换日期。

---

## 注册码更换频率

更换 Unraid 注册密钥很简单，但有一些重要的时间和资格细节需要注意。
...
永远不要将数据磁盘分配为 %%parity drive|parity_drives%%。错误的分配可能会用奇偶信息覆盖数据，导致了不可恢复的数据丢失。

:::

:::tip[Identifying 驱动器]

- **%%Parity drives|parity_drives%%** 没有可装载的文件系统。如果驱动器不能被挂载，那么它很可能是一个 %%parity|parity%% 驱动器。
- **数据驱动器** 以前曾被 Unraid 使用，即使在重置 %%array|array%% 配置后也能保留其数据。
...
2. 转到***Main***选项卡
   ...
   - 不要对任何驱动器点击 **格式化**。如果出现的不可挂载驱动器比预期的 %%parity|parity%% 驱动器多，请停止并在论坛中寻求帮助。
...
5. 如果您只有一个 %%parity|parity%% 驱动器，则数据驱动器的顺序并不重要。只有在您确定分配了相同的物理奇偶驱动器且自原始配置以来没有发生写操作时才勾选“奇偶性有效”。对于双 %%parity|parity%%，数据驱动器的顺序以及 parity1 和 parity2 的分配都很重要。parity1 和 parity2 不能互换，如果更改则%%parity|parity%%必须重建。
6. 启动 %%array|array%% 以提交分配。数据驱动器应挂载，其内容应保持完整。

...
%%SSL|ssl%% （安全套接字层）加密您浏览器与 Unraid 服务器之间的所有流量，以防止窃听和中间人攻击。如果没有 %%SSL|ssl%%，任何有网络访问权限的人都可能截取敏感数据或劫持您的会话。强烈建议在本地和远程管理您的 Unraid 服务器时使用 %%SSL|ssl%%。
:::
...
Unraid 中的多个参数会影响 %%SSL|ssl%% 的配置和使用。了解这些有助于您根据需求选择合适的证书和连接方法。
...
自定义证书是指您自己提供和管理的任何 %%SSL|ssl%% 证书，而不是由 Unraid 或 Let\u0027s Encrypt 生成的。这对于您想使用自己的域名、通配符证书或集成到您的组织 PKI 基础设施中非常有用。
...
DNS 重绑定保护是许多路由器上的一项安全功能，可防止公共 DNS 条目解析为本地 IP 地址。这有助于保护您的网络免受某些攻击，但在尝试使用 %%SSL|ssl%% 证书进行本地访问 Unraid %%WebGUI|web-gui%% 时可能会导致问题。
...
:::note
%%SSL|ssl%% 管理是 Unraid 的核心功能，并不依赖于 Unraid Connect 插件。您可以禁用 %%SSL|ssl%% 而不影响其他 Unraid 功能。
:::

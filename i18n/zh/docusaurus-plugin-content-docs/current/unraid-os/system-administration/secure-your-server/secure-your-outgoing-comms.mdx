...
2. [**Tailscale 出口节点**](#tailscale-exit-nodes) - 使用%%Tailscale|tailscale%% 出口节点，您的所有流量都可通过%%Tailscale|tailscale%% 的网状网络路由，确保设备间的安全连接。
3. [**WireGuard VPN**](#outgoing-vpn-connections) - 您还可以使用%%WireGuard|wireguard%% %%VPN|vpn-tunnel%% 通过商业%%VPN|vpn-tunnel%% 提供商来路由流量，增加隐私和安全层。
...
1. **通过使用[官方Tailscale Docker镜像](https://hub.docker.com/r/tailscale/tailscale)部署%%Tailscale|tailscale%%容器**。

...
| 服务器枢纽和辐射访问                          | Allow multiple %%VPN\|vpn-tunnel%% clients to connect to each other through the server.                      |
...
| ------- | ---------- | ------------------ | ---------------------- | --------------- | ----------------- |
| Yes     | 禁用（静态路由可选） | 可访问                | 可访问                    | 不可访问            | 简单的网络设置；推荐给大多数用户  |
| Yes     | 启用（静态路由可选） | 可访问                | 不可访问                   | 不可访问            | 避免这种配置            |

...
| %%VPN隧道的\|vpn-tunnel%%访问 | 通过商业 %%WireGuard\|wireguard%% VPN 提供商路由特定的 Docker 容器和 %%VM\|vm%%s。                              |
...
对等是一个连接到您的 Unraid %%WireGuard|wireguard%% VPN 的客户端设备，比如手机、笔记本电脑或另一台服务器。定义对等意味着为该设备创建唯一身份和安全密钥。
...
对于大多数用户，默认的 **使用 NAT** 设置开箱即用，允许访问 Unraid 和大多数局域网设备。然而，如果您使用带有自定义 IP 的 Docker 容器或具有严格网络要求的 %%VM|vm%%s:
...
- ✅ 隧道在 Unraid 和客户端设备上均已激活。*（“已激活”意味着隧道已启动，但不一定已连接。）*

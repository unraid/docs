...
| 阵列\ 在正常模式下启动  | 这是捕获诊断的首选方法，因为它提供了最完整的信息，尤其是关于驱动器状态的信息。                                         | 如果这不可能，请参阅 [Persistent logs section](#persistent-logs-syslog-server) 以找到替代捕获方法。 |
...
<Tabs>
  <TabItem value="镜像" label="Mirror syslog to boot drive" default>
    <SyslogMirror />
  </TabItem>

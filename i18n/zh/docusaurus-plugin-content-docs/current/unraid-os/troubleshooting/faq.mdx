...
### 我需要构建或系统配置方面的帮助。我应该去哪里？ {/* #build-help */}
...
### 我能在 Unraid 的虚拟机中使用 HASP 密钥吗？如果使用多个虚拟机，它是如何工作的？ {/* #hasp-key-vm */}
...
### 我的引导设备已损坏，而且我没有备份。我该如何恢复我的配置？ {/* #usb-failed-restore */}
...
### 如何完全从头开始安装 Unraid OS？（恢复出厂设置步骤） {/* #factory-reset */}

1. 备份任何你希望保留的数据。
2. 停止%%array|array%%并关闭服务器。
3. 移除启动设备。
4. 使用 Unraid USB Flash Creator 工具重新格式化该驱动器并在其上重新安装 Unraid。
5. 重新插入启动设备并启动您的服务器。
6. 在 %%WebGUI|web-gui%% 中打开终端并运行 `lsblk` 以列出所有驱动器（不包括启动设备）。
7. 清除每个数据驱动器上的现有文件系统：
...
### 我的引导设备报告无效的 GUID。我该怎么办？ {/* #invalid-guid */}
...
### 每当我关闭分配有 AMD GPU 的 Windows 虚拟机时，它就无法重新启动。我该怎么办？ {/* #amd-gpu-vm-restart */}

许多 AMD GPU 在功能级别复位时会出现问题，从而导致此问题。解决方法包括在 Windows 中关闭前弹出 GPU 或使用通常可以避免该问题的 NVIDIA GPU。

<br />

### 如果我的 CPU 没有集成显卡，如何将主 GPU 直通给虚拟机？ {/* #primary-gpu-passthrough */}

这是可行的，但需要额外的步骤。请参阅 [GPU passthrough for virtual machines](../using-unraid-to/create-virtual-machines/vm-setup.mdx) 获取设置指导。
...
- 传统硬件 %%RAID|raid%% 卡_不是_必需的；您可以使用简单的 HBA 让 Unraid 直接管理驱动器。

<br />

### 我目前有一组设备使用 MBR 风格的分区表格式化，想要转换为 GPT。我该怎么做？ {/* #mbr-to-gpt-conversion */}
...
1. 确保您拥有有效的 %%parity|parity%% 和当前的启动设备备份。
2. 从主标签页进入**维护模式**。
...
- 从 Unraid OS 6.9 开始，无论分区样式如何，分区 1 在旋转设备上从 32KiB 开始，在非旋转设备上从 1MiB 开始。
  \-- 在开始此转换过程之前，请始终[备份您的启动设备](/unraid-os/system-administration/secure-your-server/secure-your-boot-drive/#backups)。
...
### 有没有办法禁用 br0 桥？ {/* #disable-br0-bridge */}
...
### 我似乎无法使用 `http://tower` 或 `http://tower.local` 连接到 WebGUI。我该怎么办？ {/* #webgui-connection */}
...
- 如果您在使用 USB Flash Creator 创建启动介质时分配了静态 IP，请使用该地址。
...
### 我无法让 USB Flash Creator 在我的启动介质上安装 Unraid。我该怎么办？ {/* #usb-creator-issue */}

如果 USB Flash Creator 不适用于您的系统或 USB 驱动器，您可以使用[手动安装方法](../getting-started/set-up-unraid/create-your-bootable-media.mdx#manual-install-method)来准备您的 Unraid 启动设备。此方法兼容 Windows、macOS 和 Debian/Ubuntu Linux。

<br />

### 我需要将系统配置为使用 UEFI 启动。我该如何做？ {/* #uefi-boot-config */}

您可以通过几种不同的方法设置%%UEFI|uefi%%启动模式：

#### 安装

使用 USB Flash Creator 时，在将 Unraid 写入 USB 驱动器之前，选择启用 %%UEFI|uefi%% 启动模式的选项。

#### 选项 2： 在遗留模式启动后

在 %%WebGUI|web-gui%% 中，打开***设置 → 启动设备***（适用于 Unraid **7.3.0** 及以上）。在较早版本中，同一页面位于***设置 → 闪存设备***。

启用 %%UEFI|uefi%% 启动模式并重新启动服务器。

#### 选项 3：手动重命名文件夹

在启动卷（USB 驱动器）上，将 `EFI~` 文件夹重命名为 `EFI`（移除波浪号 `~`）。

您可以通过几种不同的方法设置%%UEFI|uefi%%启动模式：

将 Unraid 启动设备（如适用则为 USB）设为主要启动目标，并启用 %%UEFI|uefi%% 启动模式（如果可用，请务必禁用 CSM/Legacy/Compatibility 模式）。

<br />

### 我在使用网页浏览器访问 WebGUI 时遇到问题。我该怎么办？ {/* #webgui-browser-issues */}
...
:::important
您必须使用同一个启动设备才能继续试用。更换启动介质将需要从头开始新的试用。
:::

...
:::note
如果您使用加密的驱动器并忘记了加密密码，则无法恢复数据 - 没有后门。
:::
...
### 我如何从头开始彻底启动 Unraid 操作系统？（工厂重置流程）
...
要更改 Unraid 服务器的主机名，请浏览至 %%WebGUI|web-gui%%，依次选择 ***Settings → System Settings → Identification***。
...
- 传统硬件 %%RAID|raid%% 卡并不是必须的；您可以使用简单的 HBA 让 Unraid 直接管理驱动器。
...
- 从 Unraid OS 6.9 开始，分区 1 在旋转设备上开始于 32KiB，而在非旋转设备上开始于 1MiB，无论分区风格如何。
  \-- 在开始此转换过程之前，请始终[备份您的闪存设备](../system-administration/secure-your-server/secure-your-flash-drive.mdx#backups)。
...
在 %%WebGUI|web-gui%%，转到闪存设备页面的 ***Settings → Flash Device***。

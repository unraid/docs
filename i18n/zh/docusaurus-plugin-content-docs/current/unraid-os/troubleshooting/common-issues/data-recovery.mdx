...
当标准 Unraid 恢复方法（如 [更换磁盘](/unraid-os/using-unraid-to/manage-storage/array/replacing-disks-in-array) 过程）由于多个磁盘故障或 %%parity|parity%% 无效不可行时，像 **ddrescue** 这样的专业工具可以帮助您从失败的驱动器中拯救尽可能多的数据。

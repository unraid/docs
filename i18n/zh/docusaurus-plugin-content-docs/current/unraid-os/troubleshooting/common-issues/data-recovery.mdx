...
- **定期健康检查：** 监控您的驱动器的 [SMART 数据](../../system-administration/monitor-performance/smart-reports-and-disk-health.mdx)，并定期执行文件系统检查。
  :::
...
- 奇偶校验驱动器**没有**文件系统。在奇偶校验驱动器上运行任何修复工具可能会导致损坏，并可能导致不可逆的数据丢失。
  :::
...
| %%Array\|阵列%% 分区 | /dev/md7p1 | Unraid 管理的设备 | Yes                      |
...
- 使用原始分区（例如，`/dev/sdj1`）不会更新%%parity|parity%%，会使其失效。
  :::
...
| 文件系统             | 启动模式 | 需要维护模式？ | 笔记                                       |
| ---------------- | ---- | ------- | ---------------------------------------- |
| %%XFS\|xfs%%     | 维护模式 | Yes     | %%阵列\|array%%必须在维护模式下启动（驱动器未挂载）以进行检查/修复。 |
...
- 这类似于在 Windows 上运行 chkdsk 或 scandisk 并处理重命名为 `File0000.chk` 的文件。在查看 `lost+found` 内容时，请慢慢来。
  :::
...
- **坏区域/读取错误**：这些数字可能会随着 ddrescue 发现更多损坏而增加，但目标是尽量减少无法恢复的数据。
  :::

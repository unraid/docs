...
在***设置 → 虚拟机管理器 → 虚拟机关闭***中配置虚拟机关闭超时（启用高级视图）。
...
:::warning[No 无休眠的安全超时]
在没有休眠和%%QEMU|qemu%%%%Guest Agent|guest-agent%%的情况下，Windows %%VMs|vm%%没有真正安全的超时。对话框或正在进行的更新安装可能导致任何超时不充分，从而导致强制关机和数据损坏的风险。
:::

#### Docker超时

在***设置 → Docker***（启用高级视图）中配置Docker容器停止超时。
...
- 在关闭期间监控您的容器，识别任何始终需要更多时间的容器。

:::

#### 通用超时

在***设置 → 磁盘设置 → 关闭超时***中配置通用关闭超时。

**不间断电源（UPS）注意事项（最关键因素）：**

- 您的UPS必须提供足够的运行时间以完成完整的关闭序列，在电池用完之前。
- 对于\*\*手动关闭，\*\*您可以设置更长的超时，因为您可以控制何时开始关闭。
- 在\*\*断电关闭时，\*\*您的超时受限于UPS电池寿命。
- **测试您的UPS**，模拟断电以确保服务器能在有足够余地的情况下自动关闭。

:::tip[General 超时建议]

- **如果遇到非正常关闭：** 对于无%%VMs|vm%%的系统，将超时增加到**180秒（3分钟）**。
- **对于有%%VMs|vm%%的系统：** 如果不使用休眠，使用**300+秒（5+分钟）**。
- **如果使用休眠：** **180-300秒** 通常足够。
- 确保超时时间不超过UPS在断电时可以支持的时间。
...
**LXC容器：**
LXC插件有自己的容器停止超时设置。与Docker容器类似，LXC容器通常在几秒钟内停止，但有些可能需要更多时间。检查LXC插件设置中的容器停止超时，并将其包括在一般关闭超时计算中。

**其他服务：**
某些插件或自定义服务可能有自己的关闭程序。请参阅插件文档获取特定的超时设置，并将其纳入您的计算中。

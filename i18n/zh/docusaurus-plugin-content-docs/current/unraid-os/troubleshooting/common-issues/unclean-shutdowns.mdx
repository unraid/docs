...
- **在设置中启用 UPS 支持。**
- **配置关闭超时：** 设置不间断电源（UPS）在电池电量不足前触发自动关闭。调整"剩余电池运行时间"或"电池电量水平"阈值，确保Unraid有足够时间[停止array](../../using-unraid-to/manage-storage/array/overview.mdx#startstop-the-array)并安全关闭电源。
...
<Tabs>
  ...
  <TabItem value="设备" label="设备虚拟机">
    ...
    设备 %%VMs|vm%% 旨在运行特定软件，通常不允许安装额外的软件包，例如 %%QEMU|qemu%% %%Guest Agent|guest-agent%%。这意味着无法使用休眠，因此您需要依赖正确的超时配置。
...
:::warning[No 无休眠的安全超时]
没有休眠和%%QEMU|qemu%% %%Guest Agent|guest-agent%%，对于Windows %%VMs|vm%%，没有真正安全的超时。对话框或正在进行的更新安装可能会使任何超时不足，导致强制关机和数据损坏风险。
:::

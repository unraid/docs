...
and the Tailnet URLs will be displayed on the _**Settings → Management Access**_ page.
...
NGINX\_CUSTOMFA in `/etc/defaults/nginx`
...
如果您的服务器使用我们以前的NTP默认值time1.google.com，time2.google.com等，您可能会注意到您的syslog中出现一些困惑的NTP相关信息。要避免这种情况，请考虑切换到我们的新默认值：导航到_**设置 → 日期和时间**_并将**NTP服务器1**配置为time.google.com，其他保持为空。

当然，您可以使用任何您喜欢的时间服务器，只是通知您我们调整了默认设置。

### NFS 共享

我们添加了一些新设置以帮助解决NFS共享的问题。在_**设置 → 全局共享设置**_中，您可以调整保险丝文件描述符的数量，并在_**设置 → NFS**_中调整NFS协议版本和它使用的线程数。请参阅在线帮助以获取详细信息。

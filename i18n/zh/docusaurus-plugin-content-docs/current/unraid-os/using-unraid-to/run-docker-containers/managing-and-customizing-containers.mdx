...
<Tabs>
  <TabItem value="network-type" label="网络类型" default>
    <DockerNetworkType />
  </TabItem>

  <TabItem value="volume-mappings" label="卷映射">
    <DockerVolumeMappings />
  </TabItem>

  <TabItem value="port-mappings" label="端口映射">
    <DockerPortMappings />
  </TabItem>

  <TabItem value="environment-variables" label="环境变量">
...
| **日志**        | Shows the container's log output for troubleshooting.                                        |
| **编辑**        | 更改容器设置，例如端口或卷映射。保存后更改立即生效。                                                                   |
| **移除**        | Deletes the container. You can choose to remove just the container or also delete its image. |
| **项目页面**      | Opens the container's project page for documentation and support.                            |

...
\*\*工作原理：\*\*Unraid 使用一个称为奇校验位的特殊位，存储在所有数据磁盘中的每个位位置，并由一个专用的%%parity disk|parity-drives%%管理。%%parity|parity%% 计算的结果是所有磁盘（包括%%parity disk|parity-drives%%）中每个位位置的总比特数始终是偶数（奇偶）。Unraid 使用异或（异或）运算进行此计算。

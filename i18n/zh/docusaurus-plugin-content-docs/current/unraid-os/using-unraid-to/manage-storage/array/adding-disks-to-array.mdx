...
**工作原理：** Unraid 使用一位特殊的位称为 *奇偶位*，存储在所有数据磁盘的每个位位置，并由专用的 %%parity disk|parity-drives%% 管理。 %%parity|parity%% 的计算方式是确保所有磁盘（包括 %%parity disk|parity-drives%%）中每个位置的位数总是偶数（偶校验）。Unraid 使用 XOR（异或）运算进行此计算。

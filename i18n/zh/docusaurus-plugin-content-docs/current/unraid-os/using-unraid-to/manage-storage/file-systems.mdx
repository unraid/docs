...
| 文件系统             | Data Protection                                                                       | Space Efficiency               | 高级功能                                                             | 最佳使用案例                                                  |
| ---------------- | ------------------------------------------------------------------------------------- | ------------------------------ | ---------------------------------------------------------------- | ------------------------------------------------------- |
| %%XFS\|xfs%%     | 没有内置冗余                                                                                | 100%                           | 对损坏的强大抗性                                                         | 默认的数组驱动器；高可靠性                                           |
| %%ZFS\|zfs%%     | %%RAIDZ1\|raidz1%%/%%RAIDZ2\|raidz2%%/%%RAIDZ3\|raidz3%%，镜像等。                         | Varies by %%RAID\|raid%% level | %%Checksums\|checksum%%, %%Snapshots\|snapshot%%, %%RAID\|raid%% | Multi-device pools, %%VMs\|vm%%, Docker, advanced users |
| %%BTRFS\|btrfs%% | %%RAID 0\|raid0%%/%%RAID 1\|raid1%%/%%10\|raid10%%/%%5\|raid5%%/%%6\|raid6%%, mirrors | Varies by %%RAID\|raid%% level | %%Checksums\|checksum%%, %%Snapshots\|snapshot%%, %%RAID\|raid%% | 多设备池，混合SSD/HDD，灵活的池                                     |
...
| **未找到有效的 %%BTRFS\|btrfs%%**                         | Disk may not belong to a valid pool; check pool assignments. |
| **挂载：错误的文件系统类型、错误的选项等**                             | 文件系统选择错误或磁盘未格式化。                                             |
| **Cannot mount /dev/mdX: Structure needs cleaning** | 文件系统已损坏；执行修复。                                                |

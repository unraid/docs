...
您可以通过进入 **设置 → 调度器 → Mover 设置** 来配置 [Mover 计划](../../getting-started/set-up-unraid/customize-unraid-settings.mdx#scheduler)。在非高峰时段自动运行它，有助于在 %%cache|cache%% 和 %%array|array%% 之间移动文件，而无需手动操作。
...
- **打开网页终端：** 您可以在 Unraid WebGUI 的右上角菜单中找到此选项。

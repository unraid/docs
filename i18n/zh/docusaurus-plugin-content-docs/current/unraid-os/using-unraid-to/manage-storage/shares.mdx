...
:::note
要管理共享，只需导航到 WebGUI 中的 **Shares** 选项卡。在此处，您可以通过 ***Settings → Global Share Settings*** 轻松启用或禁用共享。默认情况下，%%user shares|user-share%% 是启用的，为您的文件组织提供了简单的起点。
:::
...
%%User shares|user-share%% 提供了在 **%%cache|cache%%** 和 **array 驱动器** 跨同名顶级文件夹的聚合视图。共享名称对应于文件夹名称，创建了一个跨多个驱动器的统一网络访问视图。需要注意的是，虽然目录看起来是合并的，但单个文件仍然存储在单个驱动器上。

### 管理用户共享

您可以从 **WebGUI** 中的 **Shares** 选项卡中访问和管理 %%User shares|user-share%%。此界面允许您：
...
在 Linux 上，%%User shares|user-share%% 可通过 `/mnt/user` 访问，结合了来自 %%array|array%% 和 %%pools|cache-pool%% 的文件。此逻辑视图覆盖了物理文件系统，这意味着文件也可以通过 **Disk Shares** 在驱动器级别查看。
...
存储文件的物理驱动器由共享设置决定，例如\*\*%%Allocation Method|allocation-method%%**、**%%Included/Excluded Disks|included-or-excluded-disks%%\*\* 和 **%%Split Level|split-level%%**。对这些设置的更改只会影响新文件；现有文件将保持不变，除非手动移动。
...
有关安全的文件管理方法和更多详细信息，请参阅下面的[磁盘共享指南](#disk-share-guidelines)。
:::
...
- 如果文件似乎被“卡住”，请考虑暂时禁用Docker和虚拟机管理器，然后通过 **Main → 阵列操作 → 移动** 手动运行 %%Mover|mover%%。
  :::
...
为了安全起见，最好将您的共享保持在*私有*模式中，并仅授予可信用户访问权限。
:::
...
此外，如果你通过终端以 `root` 用户复制文件，它们可能具有限制性权限。如果出现这种情况，通过**WebGUI**中**工具**菜单的**新权限**工具或使用与Docker相关的共享处理时的**Docker安全新权限**工具重置权限，确保所有用户都有网络访问权限。
:::

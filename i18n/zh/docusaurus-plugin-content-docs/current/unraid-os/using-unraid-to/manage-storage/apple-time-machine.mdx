...
2. 输入您的 Unraid 服务器的 %%SMB|samba%% 地址。例如，使用格式 `smb://[server-ip]/[Time Machine share name]` 并点击 **连接**。
   ...
   | **时间机器卷大小**                 | 设置为您期望的最大大小，或保留未设置。如果没有设置最小空余空间为 1，请先应用共享，然后重新编辑并将卷设置为小于所显示的计算最小空余空间。                              |
...
2. 输入您 Unraid 服务器的 %%SMB|samba%% 地址。例如，使用格式 `smb://[server-ip]/[Time Machine 共享名称]`，然后点击 **连接**。
3. 当提示输入时，输入有共享访问权限的 Unraid 用户名和密码，然后点击 **连接**。
4. 如果您使用 macOS Ventura 13.0 或更高版本，请打开 **系统设置**，如果是 macOS Monterey 12.x 或更早版本，请打开 **系统偏好设置**。然后导航到 **Time Machine**。
...
1. 为每个用户或Mac **创建独特的 Unraid %%root user|root-user%% 账户**。例如，用户命名为 `larry`、`curly` 和 `moe`。这确保每个用户都有自己的身份和权限。

2. 为每个用户 **创建独立的时间机器 %%user shares|user-share%%**，并清晰地命名与用户匹配，如 `tm-larry`、`tm-curly` 和 `tm-moe`。保持共享独立有助于管理每个用户的存储和访问。

...
本指南适用于 macOS Ventura (13.x)、Sonoma (14.x) 和 Sequoia (15.x) 版本。某些功能，例如 %%SMB|samba%% 增强和通过 %%SMB|samba%% 的 Time Machine 支持，需 macOS 支持这些协议的版本。
...
2. 输入您的 Unraid 服务器的 %%SMB|samba%% 地址。例如，使用格式 `smb://[server-ip]/[Time Machine share name]` 并点击 **连接**。

   | 设置                          | 描述                                                                        |
   | --------------------------- | ------------------------------------------------------------------------- |
   | **%%用户共享\|user-share%% 名称** | 为您的 %%user share\|user-share%% 起一个名字（例如，Time Machine）。                    |
   | **最小可用空间**                  | 设置为 **1**（1 KB）以获得最佳效果。这可以防止当磁盘接近容量时 Time Machine 失败。                     |
   | **包含的磁盘**                   | 选择 %%array\|array%% 中用于 Time Machine %%user share\|user-share%% 的磁盘。      |
   | **启用写入时复制**                 | 保持 **自动** 以获得最佳性能。                                                        |
   | **%%SMB\|samba%% 导出**       | 设为 **是（时间机器）** 以启用时间机器支持。                                                 |
   | **时间机器卷大小**                 | 设置为您所需的最大大小，或保持未设置。如果没有将最小空闲空间设置为1，请先应用共享，然后重新编辑并将卷大小设置为小于显示的计算出的最小空闲空间值。 |
   | **安全设置**                    | 根据需要配置 %%SMB\|samba%% 的安全参数。                                              |
...
2. 输入您 Unraid 服务器的 %%SMB|samba%% 地址。例如，使用格式 `smb://[server-ip]/[Time Machine share name]` 并点击 **连接**。
3. 当提示输入时，输入有共享访问权限的 Unraid 用户名和密码，然后点击 **连接**。
4. 如果您使用的是 macOS Ventura 13.0 或更高版本，请打开 **系统设置**；如果使用的是 macOS Monterey 12.x 或更早版本，请打开 **系统偏好设置**。然后导航到 **Time Machine**。
...
8. 时间机器将自动开始初始备份。请记住，第一次备份可能需要一些时间，但以后的备份会更快，因为它们是增量的。
...
1. 为每个人或将进行数据备份的 Mac **创建独特的 Unraid %%root 用户|root-user%% 账户**。例如，用户名称为 `larry`、`curly` 和 `moe`。这确保每个用户都有自己的身份和权限。

2. 为每个用户 **创建独立的时间机器 %%user shares|user-share%%**，并清晰地命名以与用户匹配，如 `tm-larry`、`tm-curly` 和 `tm-moe`。保持共享分开有助于管理每个用户的存储和访问。

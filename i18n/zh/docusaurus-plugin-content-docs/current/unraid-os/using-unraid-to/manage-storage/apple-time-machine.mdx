...
2. 配置以下设置:
   ...
   | 设置                          | 描述                                                                      |
   | --------------------------- | ----------------------------------------------------------------------- |
   | **%%用户共享\|user-share%% 名称** | 命名你的 %%user share\|user-share%% (例如，时间机器)。                              |
   | **最小可用空间**                  | 设置为 **1** (1 KB) 以获得最佳效果。这可以防止磁盘接近容量时时间机器备份失败。                          |
   | **包含的磁盘**                   | 选择 %%array\|array%% 中用于时间机器 %%user share\|user-share%% 的磁盘。             |
   | **启用写入时复制**                 | 保持 **自动** 以获得最佳性能。                                                      |
   | **%%SMB\|samba%% 导出**       | 设为 **是（时间机器）** 以启用时间机器支持。                                               |
   | **时间机器卷大小**                 | 设置为您所需的最大大小，或保留不设置。如果没有将最小可用空间设置为1，请首先应用共享，然后重新编辑，设置卷大小小于显示的计算后的最小可用空间。 |
   | **安全设置**                    | 按需配置 %%SMB\|samba%% 安全参数。                                               |
...
4. 如果使用 macOS Ventura 13.0 或更新版本打开 **系统设置**，或者在 macOS Monterey 12.x 或更早版本上打开 **系统偏好设置**。然后导航至 **时间机器**。
...
1. 为每个人或 Mac **创建独特的 Unraid %%root user|root-user%% 账户**，例如，用户命名为 `larry`、`curly` 和 `moe`。这确保了每个用户都有自己的身份和权限。

2. 为每个用户 **创建独立的时间机器 %%user shares|user-share%%**，并清晰地命名与用户匹配，如 `tm-larry`、`tm-curly` 和 `tm-moe`。分开管理共享有助于管理每个用户的存储和访问。

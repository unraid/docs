奇偶校验保护方法允许您在不丢失奇偶校验保护的情况下从您的 Unraid 阵列中删除数据磁盘。此过程需要精心准备和一定的技术知识，仅推荐给高级用户。
...
- 此方法仅允许您一次移除一个磁盘。
- 请注意，此过程没有来自 LimeTech 的官方支持，请谨慎操作。
...
<details>
  ...
  由于清零磁盘可能需要几个小时，请使用持久终端会话，例如通过 %%SSH|ssh%% 使用 `tmux`，或从 [Community Applications](/community-applications/) 安装终端插件。安装任何插件前，请查看其维护者、模板和支持详情。

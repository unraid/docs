...
| %%RAID\|raid%% Level | Data Protection | Space Efficiency | 使用场景                                                                                           |
| -------------------- | --------------- | ---------------- | ---------------------------------------------------------------------------------------------- |
...
| %%RAID 10\|raid10%%  | 可承受 1 个磁盘故障     | 50%              | Combines %%RAID 0\|raid0%% speed with %%RAID 1\|raid1%% redundancy for high-performance needs. |
| %%RAID 5\|raid5%%\*  | 可承受 1 个磁盘故障     | 67-94%           | **实验性。** 适用于大规模媒体存储的容量和冗余的平衡。                                                                  |
| %%RAID 6\|raid6%%\*  | 2 disk failures | 50-88%           | **实验性。** 为大容量硬盘提供额外的归档存储保护。                                                                    |
...
| 使用场景                         | 配置示例                                                                       | 好处                                    |
| ---------------------------- | -------------------------------------------------------------------------- | ------------------------------------- |
| High-performance %%VMs\|vm%% | NVMe SSD池，%%RAID 1\|raid1%%，%%BTRFS\|btrfs%% 或 %%ZFS\|zfs%%                | 快速 I/O 与冗余                            |
...
| Snapshots and backup targets | %%ZFS\|zfs%%池，%%RAIDZ1\|raidz1%%/%%RAIDZ2\|raidz2%%（多设备）                   | Supports native snapshots and backups |

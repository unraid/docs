...
- **%%Mover|mover%% 集成：** 写入 %%cache pool|cache-pool%% 的数据会根据您设置的计划自动传输到主 %%array|array%%。这使您的 [user shares](./shares.mdx) 保持有序且易于管理。
...
一旦您的池格式化为 %%BTRFS|btrfs%% 或 %%ZFS|zfs%%，您可以添加更多磁盘来提高冗余度并扩展存储。有关向现有池添加磁盘的详细说明，请参阅[添加磁盘到池](#adding-disks-to-a-pool)。
...
## 向池中添加磁盘 {/* #adding-disks-to-a-pool */}
...
1. 停止%%array|array%%。
2. 在**主**选项卡中，打开池。
3. 在**池设备**区域，将**插槽**设置为额外设备的确切数量。
4. 将新设备分配到可用槽位上。
5. Start the %%array|array%% to enable the changes.

:::note

启动 %%array|array%% 后，%%BTRFS|btrfs%% 会自动开始**平衡**操作，将数据重新分布到池中的所有设备上。根据池的大小和存储的数据量，这个过程可能需要几个小时甚至几天。在此期间，您的池可能会显得繁忙，但您可以正常使用它。您可以在**主**选项卡中点击第一个池设备并查看**平衡状态**以监控平衡进度。

:::

### ZFS 池

扩展 %%ZFS|zfs%% 池取决于您的池配置类型：
...
并非所有 %%ZFS|zfs%% 池都可以通过添加设备进行扩展。只有单vdev RAIDZ1/2/3 池允许一次一个磁盘地扩展。其他池配置需要您添加完整的 vdev，例如向镜像池添加一个新的镜像对或向一个 RAIDZ 池添加一个相同宽度的新 vdev。

:::

:::tip[Planning 为未来扩展]

如果您正在创建一个新的两设备%%ZFS|zfs%%池，并计划以后通过一次添加一个驱动器的方式进行扩展，请在初始设置时选择 **RAIDZ1**，而不是默认的镜像配置。 虽然两个设备池默认为镜像（提供冗余），选择 RAIDZ1 可以让您以后在不迁移数据的情况下逐步扩展池。如果您不打算扩展到两个以上的驱动器，建议使用默认的镜像配置以获得其简单性和性能。
...
1. 当 %%array|array%% 运行时，在 ***主→池设备***中，选择池名称以查看详细信息。
2. 在**池状态**区域，检查是否有**升级池**按钮。如果存在，在继续之前您需要点击此按钮。注意，升级池会限制您降级到 Unraid 的早期版本的能力。
3. 停止%%array|array%%。
4. 在 ***Main → Pool Devices*** 上，向池添加一个插槽。
5. 选择合适的驱动器（至少要与池中最小的驱动器一样大）。
6. 启动 %%array|array%%。

:::caution

如果看到"无效扩展"警告，则需要先使用第二步中提到的**升级池**按钮升级池。
...
![从 RAID 1 转换到单一配置文件](/img/Btrfs-raid1.jpg)
...
2. **保护数据：** 为每个存储池分配不同的 %%RAID|raid%% 级别或文件系统，以实现定制化的冗余和备份选项。
3. **隔离工作负载：** 将关键应用程序放在更快的冗余存储池中，将大容量数据存储在更具成本效益的设备上。
4. **灵活管理：** 你可以独立地扩展、减少或格式化存储池而不影响其他池。
5. 将新磁盘分配到之前的槽位。
6. 启动 %%array|array%%。
...
有时您可能需要在 %%cache pool|cache-pool%% 与主 %%array|array%% 之间移动文件，例如在准备维护、升级硬件或优化性能时。此过程在配置更改或更换驱动器之前备份您的 %%cache pool|cache-pool%% 也很有用。Unraid 提供了一个名为 %%Mover|mover%% 的内置工具，可自动执行此过程，用于 [user shares](./shares.mdx)。
...
当您在 Linux 层面在 [user shares](./shares.mdx) 之间移动文件时（例如使用 `mv` 或在 Docker 容器内），Linux 会尝试优化该操作。如果源和目标都显示在同一个挂载点（`/mnt/user`）上，Linux 可能会重命名文件，而不是移动它。这可能导致文件仍保留在原始磁盘或池中，即使该共享的“Use cache”设置为“No”。
...
当从多个池和 %%array|array%% 磁盘访问 [user share](./shares.mdx) 时，Unraid 会按以下顺序合并目录列表：

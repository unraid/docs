...
>
  ...
  | 使用       |          | **速度**       | 处理快速 | 过程十分缓慢 |   | \*\*阵列 |
  | 效果\*\*   | 在后台运行    | 需要将磁盘从阵列中移出  |      |        |   |        |
...
3. **取消分配目标磁盘：** 一旦阵列停止，找到您想替换的磁盘。从 ***主菜单 → 阵列设备*** 中删除它的槽位。
...
3. **取消分配已禁用的磁盘：** 转到 ***主菜单 → 阵列设备*** 并将已禁用的磁盘槽更改为“未分配”。
...
为了方便起见，您可以通过导航到 ***设置 → 调度程序*** 来安排这些检查在适合您的间隔自动运行。建议每月或每季度执行自动、纠正检查，以确保数据的持续健康并修复发现的任何同步错误。
...
- **磁盘重新排序：** 更改磁盘顺序不会影响 parity1，但 *可能* 会使 parity2 无效。
...
\**实际 NVMe 速度取决于 PCIe 代、冷却和网络带宽（例如，10GbE 上限约为 1,100 MB/s）。*
...
如果您的阵列无法启动，请按照这些步骤识别并修复常见问题。在 ***主菜单 → 阵列操作*** 下查找错误消息。

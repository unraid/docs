...
<DocCardList
  items={[
   ...
   },
   {
     ...
     label: "更换磁盘",
     description: "升级或交换故障驱动器",
   },
   {
     type: "link",
     href: "#removing-disks",
     label: "移除磁盘",
     description: "退役或缩小阵列大小",
   },
   {
     type: "link",
     href: "#checking-array-devices",
     label: "检查阵列设备",
     description: "监控 SMART 数据和诊断信息",
   },
   {
     type: "link",
     href: "#spinning-disks-down-or-up",
     label: "磁盘启动或停止",
     description: "管理磁盘的启动/停止",
   },
   {
     type: "link",
...
>
  |          | 清除       | 预清除          |      |        |   |        |
  | -------- | -------- | ------------ | ---- | ------ | - | ------ |
  | **目的**   | 确保基本兼容性  | 测试驱动器并为其做好准备 |      |        |   |        |
  | 使用       |          | **速度**       | 处理快速 | 过程十分缓慢 |   | \*\*数组 |
  | 影响\*\*   | 在后台运行    | 需要将磁盘从阵列中移出  |      |        |   |        |
...
2. \*\*停止阵列：\*\*接下来，您需要停止阵列。导航至 ***主菜单 → 阵列操作*** 并选择 **停止**。

3. \*\*取消分配目标磁盘：\*\*阵列停止后，找到要替换的磁盘。从 ***主菜单 → 阵列设备*** 中移除它。
...
5. 在开始之前，请确保要替换的数据驱动器已被禁用。若驱动器故障（显示红色指示器），则已禁用。如果驱动器健康但您想替换它，请取消分配该驱动器并在没有它的情况下启动阵列一次，以迫使 Unraid 将其标记为已禁用。

6. 如果您的更换数据磁盘不大于您的校验盘，请使用标准的[更换数据磁盘](#replacing-faileddisabled-disks)程序。
...
<details>
  ...
  <div class="text--center">
    ![红色"X"表示写入错误](/img/Diskfailureindicator.png)
...
<details>
  ...
  - 如果看到 %%CRC errors|CRC 错误%%，通常意味着电缆问题。注意这些错误，它们会随着时间的推移积累且不会重置。
    ...
    :::提示
    要随时了解驱动器问题，请在 Unraid 中[启用通知](../../getting-started/set-up-unraid/customize-unraid-settings.mdx#notification-settings)。这样，如果出现问题，您将立即收到提醒，从而降低数据风险。
    :::
</details>
...
7. **（可选）选择%%维护模式|maintenance-mode%%：**
   ...
   - 如果选择 %%维护模式|maintenance-mode%%，您仍需在下一步中点击 **开始** 以开始重建。
...
1. **验证磁盘健康状况：**
   - 对已禁用的驱动器运行 [%%SMART|smart%% 扩展测试](../../system-administration/advanced-tools/command-line-interface.mdx#smartctl) 以确认其实际健康。
   - 检查并确认仪表板中任何显示 %%SMART|smart%% 警告的警告图标。

2. **停止阵列：** 使用%%WebGUI|web-gui%%停止阵列。

3. \*\*取消分配禁用的磁盘：\*\*转到 ***主菜单 → 阵列设备*** 并将禁用的磁盘槽更改为"未分配"。

4. **开始阵列：** 启动阵列以便 Unraid 可以注册丢失的磁盘。阵列将显示磁盘为“未安装”。

5. **Verify emulated content (for data drives only)**: Check that the %%emulated disk|emulated-disk%% shows the content you expect.

6. **停止阵列：** 停止阵列以为下一步做好准备。

7. **重新分配已禁用的磁盘：** 将同一磁盘重新分配回其原始槽。

8. **（可选）选择 [%%维护模式|maintenance-mode%%](../../troubleshooting/common-issues/data-recovery.mdx#maintenance-mode)：**
   - 选择 %%维护模式|maintenance-mode%% 可以通过在重建期间阻止阵列访问来加速重建过程。
   - 在 %%维护模式|maintenance-mode%% 下，您无法在重建完成之前使用阵列。
   - %%维护模式|maintenance-mode%% 不会改变重建的启动方式 - 您仍将在下一步中点击 **开始**。
...
为了便捷，您可以通过导航至 ***设置 → 调度程序*** 来设置这些检查根据您的需求自动运行。建议每月或每季度执行自动校正检查，以确保数据的持续健康并修复发现的任何同步错误。
...
1. 如果磁盘被禁用且其数据正在重建，Unraid 将使用剩余的磁盘和校验信息重建数据。此过程中，读取速度可能会变慢到 30–60 MB/s 甚至更低，具体取决于系统中最慢的驱动器。
...
2. **恢复风险：** %%BTRFS|btrfs%% %%RAID 5|raid5%%/%%RAID 6|raid6%%不稳定；单驱动器池缺乏冗余。
3. **ZFS考虑因素：**%%ZFS|zfs%%需要更多的RAM，并且硬件要求比%%BTRFS|btrfs%%更严格。
...
<details>
  ...
  - 如果你怀疑任何驱动器发生故障或不可靠，最好避开turbo write，因为它依赖于每个驱动器正常运行。
    ...
    :::信息
    涡轮写入非常适合大量操作和需要高吞吐量的场景。然而，如果您专注于节能和最小化驱动器磨损，它可能不是日常任务的最佳选择。
    :::
</details>
...
<details>
  ...
  - 使用NVMe缓存：250–900 MB/s，取决于网络或驱动限制。
    ...
    :::提示
    考虑在 %%移动者|mover%% 运行之前使用缓存池（多个设备）以获得额外的冗余和数据保护。
    :::
</details>
...
如果您的阵列无法启动，请按照以下步骤识别和解决常见问题。在 ***主菜单 → 阵列操作*** 下查找错误消息。

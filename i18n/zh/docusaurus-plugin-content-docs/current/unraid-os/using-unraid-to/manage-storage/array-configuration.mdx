...
:::info[Disk 识别和端口灵活性]
Unraid 基于磁盘序列号和大小识别磁盘，而非它们连接的特定 SATA 端口。这意味着您可以在不同 SATA 端口之间切换驱动器，而不会影响其在 Unraid 中的分配。此功能在进行硬件故障排除时特别有用，比如寻找故障端口或更换不可靠的电源或 SATA 电缆。
:::
...
Unraid 为您的存储阵列提供多种维护和配置选项。关键操作包括：
...
:::tip[Hot-Swap 特征]
如果您使用的是现代 Unraid 兼容硬件，则可以无需关闭服务器来更换磁盘。这项称为热插拔的功能自 LimeTech 服务器起初就可用。
:::

#### 清除与预清除

Unraid 要求在将磁盘添加到奇偶校验保护的阵列之前，磁盘需处于清空状态——完全填充零并带有特殊签名。此过程确保了奇偶校验的完整性，并在磁盘准备期间保持阵列在线。
...
添加数据磁盘是增加 Unraid 阵列存储空间的好方法。通过增加更多磁盘，您可以保存更多文件、应用程序和备份，同时仍能保护数据。
...
<details>
  ...
  - 如果相关位是 1，1，1（加上 %%parity|奇偶校验%%），则 Unraid 计算：1 + x + 1 + 1 + 0 = 偶数。这时，x 必须为 1 以保持总数为偶数。
...
<details>
  ...
  - **奇偶校验 2：** 采用更复杂的算法（伽罗瓦域，而非传统 %%RAID 6|raid6%% 中的 RS），允许 Unraid 同时从任何两个缺失磁盘重建。在更大阵列中，此功能极为重要，因为多个故障的风险增加。双 %%parity|奇偶校验%% 显著提升了抵抗力，而不增加镜像冗余的开销。
</details>

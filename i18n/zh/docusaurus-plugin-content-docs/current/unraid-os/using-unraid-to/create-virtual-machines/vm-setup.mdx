...
1. 将鼠标悬停在用户模板上，然后单击箭头将其导出到服务器或下载它。
2. 在另一台 Unraid 系统上，使用 **从文件导入** 或 **上传** 来添加模板。
...
- **ZFS 的 VM 名称验证：** 防止在 [ZFS 存储池](../../advanced-configurations/optimize-storage/zfs-storage.mdx) 使用不受支持的字符。
...
- **存储位置**：快照与您的%%VM|vm%%文件一起存储，因此请确保您的%%cache pools|cache-pool%%或%%array|array%%有足够的空间。
...
Unraid 现在支持通过 `VirGL` 和 `QXL` 的高级 GPU 共享适用于 Linux VM（及支持 `Nouveau` 驱动的某些 Nvidia GPU）。

- 使用 `VirGL`：将 **显卡** 设置为 **虚拟**，并将 **VM 控制台视频驱动** 设置为 **VirtIO(3D)**。
- 使用 `QXL`：将 **显卡** 设置为 **虚拟**，并将 **VM 控制台视频驱动** 设置为 **`QXL` (最好)**。
- `VirGL` 不输出到物理显示器，并且不兼容 Windows VM 或标准 Nvidia 插件。
- `QXL` 支持多屏幕和可配置的视频内存。
...
- 参阅 [QEMU 9p 文档](http://wiki.qemu.org/Documentation/9psetup)。

...
<details>
  ...
  | 技术                                | 功能                                                      | 重要性                                                                                        |
  | --------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
  | %%KVM\|kvm%%                      | 充当嵌入在Linux内核中的%%hypervisor\|hypervisor%%。               | Allows Unraid to run %%VMs\|vm%% smoothly with hardware acceleration and minimal overhead. |
...
| **公共桥接 (br0)**      | Managed by Unraid. This option connects %%VMs\|vm%% directly to your LAN, with IPs assigned by your router. MAC addresses are preserved for consistent IP assignment. | 适用于应作为网络上常规设备运行的%%虚拟机\|vm%%，可被其他设备访问。 |

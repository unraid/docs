...
<Tabs>
  <TabItem value="Linux" label="Linux" default>
    ...
    打开终端 (使用 [SSH](../../system-administration/advanced-tools/command-line-interface.mdx) 或本地控制台) 并以 `root` 身份登录。
...
- 优点：更多的灵活性，允许磁盘用作 Unraid 共享或用于 [VM 快照](./vm-setup.mdx#vm-snapshots)。
- 缺点：需时，还需转换过程的可用空间。
...
#### 步骤 1：确定您的 VM 是否使用 Xen 的 GPLPV 驱动程序

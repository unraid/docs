...
休眠可让您将整个Windows %%VM|虚拟机%%的状态（包括打开的应用程序和文档）保存到磁盘中。 这使您能够关掉 %%VM|虚拟机%% 而不会丢失任何工作。 当您恢复时，Windows 会将所有内容恢复到您离开时的状态，跳过正常的引导过程。 当需要重启或关闭 Unraid 主机，或者想在节能的同时保持 %%VM|虚拟机%% 状态完整时，这个功能非常方便。
...
为了可靠地使用休眠，您必须安装 %%QEMU|qemu%% %%Guest Agent|访客代理%%，以便让 Unraid 可以与 %%VM|虚拟机%% 进行通讯，执行像休眠、关机和实时统计报告这样的高级操作。
...
Windows 11 需要 TPM 2.0 和安全启动。 Unraid 的**OVMF-TPM** BIOS 提供了满足这些要求所需的虚拟 TPM 支持。

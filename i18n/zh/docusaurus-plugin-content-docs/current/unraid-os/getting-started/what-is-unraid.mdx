...
<details>
  ...
  2. **复杂的生命周期管理** - 困难的安装体验可能会在移除后留下残留物。
  ...
  Docker 容器允许您在隔离、轻量级的环境中运行应用程序，而无需承担与 %%virtual machines|虚拟机%% 相关的开销。Unraid 通过 [Docker Hub](https://hub.docker.com/) 和 [Community Applications](/community-applications/) 提供的数千个预配置应用，简化了 Docker 的使用。这种容器化方式使 Unraid 用户能够同时运行多个应用程序而不会出现兼容性问题。它还通过使用独立的应用程序包帮助保持系统有序，并使通过 Docker 的生态系统增强服务器能力变得更加容易。

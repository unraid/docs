...
Docker 容器允许您在隔离、轻量的环境中运行应用程序，而不需承担 %%virtual machines|虚拟机%% 相关开销。Unraid 通过 [Docker Hub](https://hub.docker.com/) 和 [Community Applications](/community-applications/) 提供的预配置应用程序，简化了 Docker 的使用。这种容器化方法使 Unraid 用户能够同时运行多个应用程序而不会有兼容性问题。它还通过使用独立的应用程序包帮助保持系统有序，并便于通过 Docker 的生态系统增强服务器功能。

...
<details>
  ...
  ✅ 将启动顺序调整为：**Forced-FDD**，**USB-HDD**，**USB-ZIP**。\
  ✅ 禁用USB 2.0/3.0支持。\
  ✅ 切换任何快速启动功能。\
  ✅ 切换USB键盘支持。
  ...
  :::important
  许多主板限制启动设备选择为12个硬盘。如果您的USB闪存驱动器被识别为硬盘，在安装12个物理硬盘后可能无法从其启动。尽可能将BIOS配置为将闪存驱动器视作可移动设备。

  如果使用附加的HBA，您可以禁用INT 13h支持，以防止其连接的驱动器出现在可启动设备列表中，从而帮助保持在12个驱动器限制以内。
  :::
</details>

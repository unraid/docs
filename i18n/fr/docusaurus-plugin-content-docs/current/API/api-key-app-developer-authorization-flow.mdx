...
Les étendues suivent le modèle : `resource:action`. Les exemples incluent `docker:read` (accès en lecture à Docker), `vm:*` (accès complet aux machines virtuelles), `system:update` (accès à la mise à jour du système), `role:viewer` (accès rôle visionneur) et `role:admin` (accès rôle admin).

...
- **Stripe** convient uniquement aux données non critiques et temporaires - si un disque échoue, vous perdez tout.
  :::
...
<details>
  ...
  Vous avez peut-être entendu le conseil obsolète suivant : "%%ZFS|zfs%% nécessite 1 Go de RAM par To de stockage." Cela n'est plus applicable pour la plupart des utilisateurs. %%ZFS|zfs%% utilise la RAM pour son cache de remplacement adaptatif (ARC), qui accélère les lectures fréquemment consultées.

  Unraid limite automatiquement %%ZFS|zfs%% à l'utilisation d'une portion raisonnable de la RAM de votre système (généralement 1/8 du total de la RAM). Cela permet à %%ZFS|zfs%% de bien fonctionner sans affecter les conteneurs Docker, %%VMs|vm%% ou le système d'exploitation Unraid.
...
<details>
  ...
  Unraid détectera automatiquement et importera le pool %%ZFS|zfs%%. Les vdevs de support (comme journal, cache/L2ARC, spécial/dédiers) sont répertoriés sous **Sous-pools** dans le %%WebGUI|web-gui%%. Il n'est pas nécessaire d'ajouter séparément les sous-pools après l'importation. Unraid les importera automatiquement avec les disques de données principaux lorsque tous les lecteurs requis sont assignés.
  :::
...
| Vdev de cache (L2ARC)          | Fournit un cache de lecture basé sur SSD                      | Optionnel. Peut améliorer les vitesses de lecture pour les ensembles de travail importants.                                     |
...
| Vdev de cache (L2ARC)            | La piscine importe, mais le cache de lecture est perdu                    | Le pool fonctionne normalement, mais vous perdez l'amélioration des performances du cache L2ARC. Aucune donnée n'est perdue.                         |
...
| Cible de sauvegarde quotidienne ou de réplication | Utilisez un disque %%ZFS\|zfs%% (même dans l'%%array\|array%% Unraid) comme cible de réplication. Vous pouvez répliquer d'autres pools localement ou depuis un autre serveur Unraid.                                                             | Utilisez `zfs send/receive` ou des outils comme Syncoid pour des sauvegardes et récupérations rapides et fiables.                          |
| %%Snapshot\|snapshot%%-based recovery pool        | Keep point-in-time %%snapshot\|snapshot%%s of critical data or containers. %%snapshot\|snapshot%%s can be auto-scheduled and are space-efficient.                                                                                                | Cette fonctionnalité permet une récupération rapide des suppressions accidentelles ou des erreurs de configuration.                        |

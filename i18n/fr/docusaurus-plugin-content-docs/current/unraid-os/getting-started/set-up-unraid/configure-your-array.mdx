...
Une synchronisation de parité s'exécutera en arrière-plan pour protéger le %%array|array%% même avant que le formatage ne soit terminé. Le %%array|array%% fonctionnera dans un état non protégé jusqu'à ce que la synchronisation soit terminée, il est donc préférable d'attendre avant d'ajouter des données.

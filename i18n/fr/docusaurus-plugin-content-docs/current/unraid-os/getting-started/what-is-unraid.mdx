...
<details>
  ...
  Les conteneurs Docker vous permettent d’exécuter des applications dans des environnements isolés et légers, sans les surcharges associées aux %%machines virtuelles|vm%%. Unraid simplifie l’utilisation de Docker en donnant accès à des milliers d’applications préconfigurées via [Docker Hub](https://hub.docker.com/) et [Applications communautaires](/community-applications/). Cette approche conteneurisée permet aux utilisateurs d’Unraid d’exécuter plusieurs applications simultanément sans problème de compatibilité. Elle contribue également à maintenir le système organisé grâce à des paquets d’application autonomes et facilite l’extension des capacités du serveur via l’écosystème Docker.

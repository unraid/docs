...
## Séries mineures prises en charge et EOL

Unraid prend en charge deux séries mineures publiques à la fois :

- La **série mineure publique actuelle**, par exemple `7.3.x` une fois qu’une version bêta, RC ou stable de `7.3` est publique.
- La **série mineure publique précédente**, par exemple `7.2.x` pendant que `7.3.x` est la série publique actuelle.

Une bêta publique ou une RC est comptée comme une série mineure publique aux fins de la politique de support. Cela ne signifie pas que les versions bêta ou RC sont recommandées pour une utilisation en production ; cela signifie seulement que la fenêtre de support avance lorsque la prochaine série mineure devient publique.

Au sein d’une série mineure prise en charge, utilisez la dernière version de correctif ou préversion disponible pour cette série. Les anciennes versions de correctif de la même série sont supplantées et restent disponibles uniquement pour l’archive, le retour en arrière et la référence des notes de version.

Les anciennes séries mineures sont en **fin de vie (EOL)**. Les versions EOL ne sont pas prises en charge et n’incluent plus de corrections de bogues ni de mises à jour de sécurité.

Pour vérifier l’état actuel d’une version, consultez la [Version Archive](../download_list.mdx).
...
- Les versions bêta publiques et RC sont prises en compte pour identifier la série mineure publique actuelle.
...
- Les diagnostics depuis ***Tools -> Diagnostics***.

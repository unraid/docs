...
:::note
Si vous utilisez des lecteurs chiffrés et oubliez le mot de passe de chiffrement, la récupération des données est impossible - il n'y a pas de porte dérobée.
:::

<br />

<a id="réinitialisation-usine" />

### Comment puis-je complètement démarrer l'OS Unraid à partir de zéro ? (Procédure de réinitialisation usine)
...
Remplacez `X` par la lettre de lecteur correcte (par ex., sda, sdb, sdc)
:::
...
Pour changer le nom de votre serveur Unraid, accédez au %%WebGUI|web-gui%% à ***Paramètres → Paramètres du système → Identification***.
...
- Les cartes %%RAID|raid%% matérielles traditionnelles ne sont *pas* nécessaires; vous pouvez utiliser des HBA simples pour permettre à Unraid de gérer directement les disques.
...
2. Entrez en **Mode Maintenance** depuis l'onglet ***Principal***.
...
- À partir de l'OS Unraid 6.9, la partition 1 commence à 32KiB pour les appareils rotatifs et 1MiB pour les appareils non-rotatifs, quel que soit le style de partition.
  \-- Toujours [sauvegardez votre clé USB](../system-administration/secure-your-server/secure-your-flash-drive.mdx#backups) avant de commencer ce processus de conversion.
...
Dans le %%WebGUI|web-gui%%, allez à la page ***Paramètres du dispositif Flash → Dispositif Flash***.

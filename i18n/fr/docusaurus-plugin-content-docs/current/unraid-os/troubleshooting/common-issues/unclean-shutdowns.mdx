...
- **Activer la journalisation persistante :** Allez dans ***Paramètres → Serveur Syslog*** pour activer la journalisation qui persiste après un redémarrage. Voir [Journaux persistants (Serveur Syslog)](../diagnostics/capture-diagnostics-and-logs.mdx#persistent-logs-syslog-server) pour plus de détails.
...
- **Activer le support UPS** dans ***Paramètres → Paramètres d'UPS***.
...
<Tabs>
  ...
  <TabItem value="appareil" label="VMs d'appareil">
    ...
    :::note[Pourquoi les VMs d'appareil sont différentes]
...
Si vous rencontrez des arrêts non propres ou des conteneurs qui plantent lors de l'arrêt, envisagez d'augmenter le délai d'arrêt général à **180 secondes** (ou **300+ secondes** si vous avez plusieurs %%VMs|vm%%). Cela donne aux services plus de temps pour s'arrêter en douceur.
:::
...
2. **Arrêt des conteneurs Docker :** Tous les conteneurs s'arrêtent simultanément (temps total = délai d'arrêt de Docker).

3. **Autres services :** Cela inclut des tâches comme les conteneurs LXC et les plugins tiers, ce qui prend généralement quelques secondes.

4. **Arrêt de l'array :** Les disques doivent être démontés et les données synchronisées ; cela prend généralement 15-30 secondes.
...
Si toutes vos %%VMs|vm%% sont configurées pour hiberner plutôt que de s'éteindre, alors le délai de la VM est moins critique puisque l'hibernation est presque immédiate. Vous pourriez utiliser un délai de la VM plus faible (par exemple, 60-120 secondes) comme solution de secours pour toute %%VM|vm%% qui ne supporte pas l'hibernation.
...
<Tabs>
  <TabItem value="délais-vm" label="Délais VMs" default>
    Où régler : ***Paramètres → Gestionnaire de VM → Arrêt de VM*** (activez la vue Avancée)
    ...
    **Problèmes courants :**
    ...
    :::conseil[Recommandations de délais pour VMs]
    ...
    - **Testez votre configuration :** Arrêtez manuellement vos %%VMs|vm%% pour confirmer qu'elles s'arrêtent ou hibernent dans le délai d'attente.
      :::
  ...
  <TabItem value="délais-docker" label="Délais Docker">
    Où régler : ***Paramètres → Docker*** (activez la vue Avancée)
    ...
    **Comment ça fonctionne:**
    ...
    :::conseil[Recommandations de délais Docker]
    ...
    - Surveillez vos conteneurs pendant l'arrêt pour identifier ceux qui nécessitent systématiquement plus de temps.
      :::
  </TabItem>

  <TabItem value="délais-généraux" label="Délais Généraux">
    Où régler : ***Paramètres → Paramètres Disque → Délai d'arrêt***

    **Considérations UPS (facteur le plus critique) :**

    - Votre UPS doit fournir suffisamment d'autonomie pour compléter la séquence d'arrêt complète avant que la batterie ne soit épuisée.
    - Pour un **arrêt manuel**, vous pouvez régler des délais plus longs car vous contrôlez quand l'arrêt commence.
    - En cas **d'arrêt lié à une panne de courant**, votre délai est limité par la durée de la batterie de l'UPS.
    - **Testez votre UPS** en simulant une panne de courant pour s'assurer que votre serveur s'arrête proprement à temps.

    :::conseil[Recommandations de délais généraux]

    - **Si vous avez des arrêts non propres :** Augmentez à **180 secondes (3 minutes)** pour les systèmes sans %%VMs|vm%%.
    - **Pour les systèmes avec %%VMs|vm%% :** Utilisez **300+ secondes (5+ minutes)** si vous n'utilisez pas l'hibernation.
    - **Si vous utilisez l'hibernation :** **180-300 secondes** est généralement suffisant.
    - Assurez-vous que les délais ne sont pas plus longs que ce que votre UPS peut supporter lors d'une panne de courant.
      :::
  </TabItem>

  <TabItem value="tiers" label="Services Tiers">
    **Conteneurs LXC :**
    Le plugin LXC a ses propres réglages de délai pour arrêter les conteneurs. Comme les conteneurs Docker, les conteneurs LXC s'arrêtent généralement en quelques secondes, mais certains peuvent nécessiter plus de temps. Consultez les paramètres du plugin LXC pour le délai d'arrêt des conteneurs et incluez ce délai dans le calcul de votre délai d'arrêt général.

    **Autres services :**
    Certains plugins ou services personnalisés peuvent avoir leurs propres procédures d'arrêt. Référez-vous à la documentation des plugins pour des paramètres de délai spécifiques et incorporez-les dans vos calculs.
    ...
    **Plugin Dynamix Stop Shell :**
    Si vous utilisez fréquemment des sessions SSH ou terminal, des sessions ouvertes peuvent empêcher un arrêt propre car Unraid attend qu'elles se ferment avant de poursuivre.
    ...
    :::conseil[Quand utiliser le plugin]
    ...
    - Le plugin fermera les sessions de force, ce qui pourrait interrompre le travail.
      :::
  </TabItem>
</Tabs>

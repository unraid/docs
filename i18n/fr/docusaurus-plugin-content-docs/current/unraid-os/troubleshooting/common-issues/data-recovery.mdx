...
- **Contrôles de santé réguliers :** Gardez un œil sur les [données SMART](../../system-administration/monitor-performance/smart-reports-and-disk-health.mdx) de votre disque et effectuez des vérifications périodiques du système de fichiers.
  :::
...
- Le disque de parité n’a **pas** de système de fichiers. L'exécution de tout outil de réparation sur le disque de parité peut l'endommager et entraîner une perte de données irréversible.
  :::
...
| Partition de %%array\|array%% | /dev/md7p1     | Périphérique géré par Unraid | Yes                                 |
...
- L'utilisation de la partition brute (par exemple, `/dev/sdj1`) ne mettra pas à jour %%parity|parité%%, la laissant invalide.
  :::
...
| ------------------- | ----------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| %%XFS\|xfs%%        | Mode maintenance  | Yes                         | %%L'array\|array%% doit être démarrée en mode de Maintenance (disques non montés) pour la vérification/réparation. |
...
- C'est similaire à l'exécution de chkdsk ou scandisk sur Windows et au travail avec des fichiers renommés en `File0000.chk`, etc. Prenez le temps d'examiner le contenu de `lost+found`.
  :::
...
- \*\*Copie des blocs non essayés... \*\*: ddrescue fait sa première tentative pour lire tous les secteurs.
  :::
...
- **Zones défectueuses/erreurs de lecture** : Ces chiffres peuvent augmenter à mesure que ddrescue trouve plus de dommages, mais l'objectif est de minimiser les données irrécupérables.
  :::

...
| Système de fichiers | Data Protection                                                                       | Space Efficiency               | Fonctionnalités avancées                                         | Meilleur cas d'utilisation                               |
| ------------------- | ------------------------------------------------------------------------------------- | ------------------------------ | ---------------------------------------------------------------- | -------------------------------------------------------- |
| %%XFS\|xfs%%        | Aucune redondance intégrée                                                            | 100 %                          | Résistant à la corruption                                        | Par défaut pour les disques d'une baie ; haute fiabilité |
| %%ZFS\|zfs%%        | %%RAIDZ1\|raidz1%%/%%RAIDZ2\|raidz2%%/%%RAIDZ3\|raidz3%%, miroirs, etc.               | Varies by %%RAID\|raid%% level | %%Checksums\|checksum%%, %%Snapshots\|snapshot%%, %%RAID\|raid%% | Multi-device pools, %%VMs\|vm%%, Docker, advanced users  |
| %%BTRFS\|btrfs%%    | %%RAID 0\|raid0%%/%%RAID 1\|raid1%%/%%10\|raid10%%/%%5\|raid5%%/%%6\|raid6%%, mirrors | Varies by %%RAID\|raid%% level | %%Checksums\|checksum%%, %%Snapshots\|snapshot%%, %%RAID\|raid%% | Baies multi-appareils, mélange SSD/HDD, baies flexibles  |
...
- Pour obtenir de l'aide experte, postez votre problème avec les diagnostics sur les [forums Unraid](https://forums.unraid.net/).
  :::
...
- Vérifiez toujours attentivement avant de formater ou de réparer les disques pour éviter une perte de données accidentelle.
  :::
...
| **Aucun %%BTRFS\|btrfs%% valide trouvé**            | Disk may not belong to a valid pool; check pool assignments.                     |
| **Montage: mauvais type fs, mauvaise option, etc.** | Sélection incorrecte du système de fichiers ou disque non formaté.               |
| **Cannot mount /dev/mdX: Structure needs cleaning** | Le système de fichiers est corrompu; effectuez une réparation.                   |
...
- Si vous avez des doutes, postez vos diagnostics et messages d'erreur sur les [forums Unraid](https://forums.unraid.net/) pour une assistance experte.
  :::
...
- Si vous voyez un dossier `lost+found`, vérifiez son contenu pour tout fichier important.
  :::

...
- **Pour les pools mixtes ou extensibles :** %%BTRFS|btrfs%% est idéal si vous souhaitez utiliser différentes tailles de disque ou ajouter/retirer facilement des périphériques. Voir [pools BTRFS](/fr/unraid-os/using-unraid-to/manage-storage/cache-pools#btrfs-pools) pour plus de détails sur l'ajout de disques aux pools %%BTRFS|btrfs%%.

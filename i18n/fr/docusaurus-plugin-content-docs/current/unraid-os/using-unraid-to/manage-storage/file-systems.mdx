...
- Pour les disques de transfert ou les périphériques externes, il est recommandé d'utiliser le plugin **[Périphériques Non Assignés](#using-the-unassigned-devices-plugins)**, qui prend en charge des systèmes de fichiers supplémentaires.
  :::
...
Si vous n'êtes pas sûr, commencer avec les réglages par défaut est une bonne approche : utilisez %%XFS|xfs%% pour les disques %%array|array%% et %%BTRFS|btrfs%% pour les %%cache pools|cache-pool%%.
:::
...
1. **Définir le paramètre par défaut globalement** :
   ...
   - **Note :** Il n'y a actuellement pas de paramètre par défaut global pour %%cache pools|cache-pool%%. Ils par défautraient toujours à %%BTRFS|btrfs%% si laissés en "auto".
...
Si vous rencontrez des problèmes, n'hésitez pas à visiter les [forums Unraid](https://forums.unraid.net/). Joignez votre fichier zip de diagnostics système (trouvé sous ***Outils → Diagnostics***) pour obtenir de l'aide.

...
1. Vider le partage :
   ...
   - **Utiliser l'accès réseau** : Connectez-vous via **Windows Explorer** (%%SMB|samba%%) ou **macOS Finder** (%%AFP|AFP%%/%%SMB|samba%%) et supprimez manuellement les fichiers.

   :::caution

   La commande `rm -rf` supprimera définitivement les fichiers. Vérifiez soigneusement avant de l'utiliser. Assurez-vous qu'aucun service Docker ou %%VM|vm%% n'utilise le partage avant de le supprimer.

...
Vous pouvez configurer le [programme du Mover](../../getting-started/set-up-unraid/customize-unraid-settings.mdx#scheduler) en allant dans **Paramètres → Planificateur → Paramètres du Mover**. Le faire fonctionner automatiquement pendant les heures creuses aide à déplacer les fichiers entre le %%cache|cache%% et le %%array|array%% sans intervention manuelle.
...
- **Ouvrez le terminal web :** vous trouverez cette option dans le menu en haut à droite du %%WebGUI|web-gui%% Unraid.

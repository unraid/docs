...
%%User shares|user-share%% fournissent une vue d'ensemble des dossiers de niveau supérieur portant le même nom sur le **%%cache|cache%%** et les **lecteurs de l'array**. Le nom du partage correspond au nom du dossier, créant une vue accessible au réseau qui s'étend sur plusieurs lecteurs. Il est important de noter que bien que les répertoires semblent fusionnés, les fichiers individuels restent stockés sur un seul lecteur.
...
Au niveau Linux, les %%User shares|user-share%% sont accessibles sous `/mnt/user`, qui combine les fichiers de l'%%array|array%% et des %%pools|cache-pool%%. Cette vue logique se superpose au système de fichiers physique, ce qui signifie que les fichiers sont également visibles via **Disk Shares** au niveau du lecteur.
...
Le lecteur physique qui stocke un fichier est déterminé par les paramètres de partage tels que **%%Allocation Method|allocation-method%%**, **%%Included/Excluded Disks|included-or-excluded-disks%%** et **%%Split Level|split-level%%**. Les modifications apportées à ces paramètres n'affecteront que les nouveaux fichiers ; les fichiers existants resteront inchangés à moins d'être déplacés manuellement.
...
- Si les fichiers semblent "bloqués", envisagez de désactiver temporairement Docker et le gestionnaire VM, puis d'exécuter %%Mover|mover%% manuellement via ***Principal → Opération de l'Array → Déplacer***.
  :::
...
Pour plus de sécurité, il est préférable de garder vos partages en mode *Privé* et de n'accorder l'accès qu'aux utilisateurs de confiance.
:::
...
De plus, si vous copiez des fichiers en tant que `root` via le terminal, ils peuvent avoir des permissions restrictives. Si cela se produit, utilisez l'outil **Nouvelles Permissions** dans le menu **Outils** de la WebGUI ou les **Nouvelles Permissions Sécurisées Docker** si vous traitez avec des partages impliquant Docker pour réinitialiser les permissions, garantissant à tous les utilisateurs un accès réseau.
:::

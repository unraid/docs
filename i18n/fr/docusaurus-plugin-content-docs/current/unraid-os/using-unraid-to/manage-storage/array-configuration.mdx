...
1. Allez à ***Principal → Appareils de l'ensemble*** dans l'interface.
...
>
  ...
  | pour utilisation  |                                   | **Vitesse**                                 | Fonctionne rapidement | Processus beaucoup plus lent |   | \*\*Ensemble |
...
2. **Arrêtez l'ensemble :** Ensuite, vous voudrez arrêter l'ensemble. Naviguez vers ***Principal → Opération de l'ensemble*** et sélectionnez **Arrêter**.

3. **Désassigner le disque cible :** Une fois l'ensemble arrêté, trouvez le disque que vous souhaitez remplacer. Retirez-le de son emplacement dans ***Principal → Appareils de l'ensemble***.
...
3. **Désassignez le disque désactivé :** Allez à ***Principal → Appareils de l'ensemble*** et changez l'emplacement du disque désactivé pour "Non assigné."
...
Pour plus de commodité, vous pouvez planifier ces vérifications pour qu'elles s'exécutent automatiquement à des intervalles qui vous conviennent en naviguant vers ***Paramètres → Planificateur***. Il est conseillé de réaliser des vérifications automatiques et correctrices sur une base mensuelle ou trimestrielle pour assurer la santé continue de vos données et corriger toute erreur de synchronisation trouvée.
...
- **Réorganisation des disques :** Changer l'ordre des disques n'affectera pas la parité1, mais cela *peut* invalider la parité2.
...
\*_Les vitesses NVMe réelles dépendent de la génération PCIe, du refroidissement et de la bande passante réseau (par exemple, 10GbE plafonne à ~1,100 Mo/s).
...
Si votre ensemble ne démarre pas, suivez ces étapes pour identifier et résoudre les problèmes courants. Recherchez les messages d'erreur sous ***Principal → Opération de l'ensemble***.

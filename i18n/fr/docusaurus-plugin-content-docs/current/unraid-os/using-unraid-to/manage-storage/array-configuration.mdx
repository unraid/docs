  ...
  items={[
   {
     type: "link",
     href: "#adding-disks",
     label: "Ajouter des disques",
     description: "Augmenter la capacité de stockage",
   },
   {
     type: "link",
     href: "#replacing-disks",
     label: "Remplacer des disques",
     description: "Mettre à niveau ou échanger les disques défectueux",
   },
   {
     type: "link",
     href: "#removing-disks",
     label: "Retirer des disques",
     description: "Déclasser ou réduire la taille de l'ensemble",
   },
   {
     type: "link",
     href: "#checking-array-devices",
     label: "Vérification des appareils de l'ensemble",
     description: "Surveiller les données SMART et les diagnostics",
   },
   {
     type: "link",
     href: "#spinning-disks-down-or-up",
     label: "Arrêter ou démarrer les disques",
     description: "Gérer l'activation/désactivation des disques",
   },
   {
     type: "link",
     href: "#reset-the-array-configuration",
     label: "Réinitialiser la configuration de l'ensemble",
     description: "Rebâtir la structure de l'ensemble",
   },
  ]}
/>
...
>
  |                   | Effacer                           | Pré-Effacer                                 |                       |                              |   |             |
  | ----------------- | --------------------------------- | ------------------------------------------- | --------------------- | ---------------------------- | - | ----------- |
  | **Objectif**      | Assure la compatibilité de base   | Teste le disque et le prépare               |                       |                              |   |             |
  | pour utilisation  |                                   | **Vitesse**                                 | Fonctionne rapidement | Processus beaucoup plus lent |   | \*\*Tableau |
  | Impact\*\*        | Fonctionne en arrière-plan        | Exige que le disque soit hors de l'ensemble |                       |                              |   |             |
...
<details>
  ...
  - **Parité 2 :** Implémente la Q-parité en utilisant des calculs syndrome de champ de Galois (style Reed-Solomon), comparable à %%RAID 6|raid6%%. Cette implémentation de champ de Galois permet à Unraid de reconstruire à partir de deux pannes de disque simultanées. Cette fonctionnalité est significative pour les ensembles plus grands, où le risque de défaillances multiples augmente. La double %%parité|parity%% renforce considérablement la résilience sans les frais supplémentaires de la redondance en miroir.
</details>
...
2. **Arrêtez le tableau :** Ensuite, vous voudrez arrêter le tableau. Naviguez vers ***Principal → Opération du Tableau*** et sélectionnez **Arrêter**.

3. **Désattribuez le disque cible :** Une fois le tableau arrêté, trouvez le disque que vous souhaitez remplacer. Retirez-le de son emplacement dans ***Principal → Dispositifs du Tableau***.
...
<details>
  ...
  - Si vous voyez des erreurs de %%CRC|crc-errors%%, cela indique généralement des problèmes avec le câblage. Surveillez ces erreurs, car elles s'accumulent avec le temps et ne se réinitialisent pas.
    ...
    :::astuce
    Pour rester informé des problèmes de disque, [activez les notifications](../../getting-started/set-up-unraid/customize-unraid-settings.mdx#notification-settings) dans Unraid. De cette façon, vous êtes alerté immédiatement si quelque chose ne va pas, réduisant ainsi le risque pour vos données.
    :::
</details>
...
7. **(Optionnel) Choisissez le %%mode maintenance|maintenance-mode%% :**
   ...
   - Si vous choisissez %%Mode Maintenance|maintenance-mode%%, vous cliquerez toujours sur **Démarrer** à l'étape suivante pour commencer la reconstruction.
...
1. **Vérifiez la santé du disque :**
   ...
   - Recherchez et reconnaissez toutes les icônes d'avertissement sur le tableau de bord qui indiquent des avertissements %%SMART|smart%%.

2. **Arrêtez la matrice :** Utilisez le WebGUI pour arrêter la matrice.

3. **Désattribuez le disque désactivé :** Allez dans ***Principal → Dispositifs du Tableau*** et changez l'emplacement du disque désactivé en "Non attribué".

4. **Démarrez l'ensemble :** Démarrez l'ensemble pour qu'Unraid puisse enregistrer le disque manquant. L'ensemble affichera le disque comme "Non installé."

5. **Verify emulated content (for data drives only)**: Check that the %%emulated disk|emulated-disk%% shows the content you expect.

6. **Arrêtez l'ensemble :** Arrêtez l'ensemble pour vous préparer aux étapes suivantes.

7. **Réassigner le disque désactivé :** Réaffectez le même disque à son emplacement d'origine.

8. **(Optionnel) Choisissez [%%Mode Maintenance|maintenance-mode%%](../../troubleshooting/common-issues/data-recovery.mdx#maintenance-mode) :**
   - Sélectionner le %%Mode Maintenance|maintenance-mode%% peut accélérer le processus de reconstruction en empêchant l'accès à l'ensemble pendant la reconstruction.
   - En mode %%Maintenance|maintenance-mode%%, vous ne pourrez pas utiliser l'ensemble tant que la reconstruction n'est pas terminée.
   - Le %%Mode Maintenance|maintenance-mode%% ne change pas la façon dont la reconstruction est commencée — vous cliquerez toujours sur **Démarrer** à l'étape suivante.
...
Pour plus de commodité, vous pouvez planifier ces vérifications pour qu'elles s'exécutent automatiquement à des intervalles qui vous conviennent en accédant à ***Paramètres → Planificateur***. Il est conseillé d'effectuer des vérifications automatiques et correctrices sur une base mensuelle ou trimestrielle pour assurer la santé continue de vos données et corriger les erreurs de synchronisation trouvées.
...
<details>
  ...
  - Si vous pensez qu'un disque échoue ou est peu fiable, il est préférable d'éviter l'écriture turbo car elle repose sur le bon fonctionnement de chaque disque.
    ...
    :::info
    Turbo write est le mieux adapté pour les opérations en vrac et les scénarios nécessitant un débit élevé. Cependant, cela pourrait ne pas être le meilleur choix pour les tâches quotidiennes si vous vous concentrez sur les économies d'énergie et la minimisation de l'usure des disques.
    :::
</details>
...
<details>
  ...
  - Avec cache NVMe : 250–900 Mo/s selon les contraintes du réseau ou du disque.
    ...
    :::astuce
    Envisagez d'utiliser un pool de cache (plusieurs dispositifs) pour ajouter de la redondance et de la protection des données avant que le %%mouvement|mover%% ne s'exécute.
    :::
</details>
...
2. Localisez **Reglable (méthode_md_écriture)**.
...
Si votre tableau ne démarre pas, suivez ces étapes pour identifier et résoudre les problèmes courants. Recherchez des messages d'erreur sous ***Principal → Opération du Tableau***.

...
1. Aller à ***Principal → Périphériques de la matrice*** dans l'interface.
...
**Comment cela fonctionne-t-il :** Unraid utilise un bit spécial appelé le *bit de parité*, stocké pour chaque position de bit sur tous les disques de données et géré par un %%parity disk|parity-drives%% dédié. La %%parité|parity%% est calculée de telle sorte que le nombre total de bits dans chaque position à travers tous les disques (y compris le %%parity disk|parity-drives%%) est toujours un nombre pair (parité paire). Unraid utilise l'opération XOR (ou exclusif) pour ce calcul.

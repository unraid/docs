...
9. Vérifiez la santé du disque en exécutant un [test étendu %%SMART|smart%%](/unraid-os/system-administration/advanced-tools/command-line-interface#smartctl) et en prenant note de toute icône d'avertissement sur le tableau de bord.
...
- Vous avez vérifié l'état de santé du disque à l'aide des [rapports SMART](../../../system-administration/monitor-performance/smart-reports-and-disk-health.mdx).
...
Le parity swap est une procédure spéciale utilisée lorsque vous devez remplacer un disque de données par un disque plus grand que votre disque de parité actuel. Le processus installe un nouveau disque plus grand comme nouveau disque de parité, puis installe votre ancien disque de parité dans l’emplacement de données. Cela garantit que votre array reste protégé et permet d’utiliser à l’avenir des disques de données plus grands.

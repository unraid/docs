...
2. Entrez l'adresse %%SMB|samba%% de votre serveur Unraid. Par exemple, utilisez le format `smb://[serveur-ip]/[nom du partage Time Machine]` et cliquez sur **Se connecter**.
   ...
   | **Espace libre minimum**                       | Réglez sur **1** (1 Ko) pour de meilleurs résultats. Cela empêche Time Machine d'échouer lorsque les disques approchent de la capacité.                                                                                                                |
   ...
   | **Taille du volume Time Machine**              | Réglez sur votre taille maximale souhaitée, ou laissez non défini. Si vous ne fixez pas l'espace libre minimum à 1, appliquez d'abord le partage, puis rééditez et réglez la taille du volume à moins que l'espace libre minimum calculé qui apparaît. |
...
2. Entrez l'adresse %%SMB|samba%% de votre serveur Unraid. Par exemple, utilisez le format `smb://[adresse-serveur]/[nom du partage Time Machine]` et cliquez sur **Connecter**.
...
4. Ouvrez **Réglages Système** si vous utilisez macOS Ventura 13.0 ou plus tard, ou **Préférences Système** si vous êtes sur macOS Monterey 12.x ou antérieur. Ensuite, naviguez vers **Time Machine**.
...
8. Time Machine démarrera la sauvegarde initiale automatiquement. Gardez à l'esprit que la première sauvegarde peut prendre du temps, mais les sauvegardes futures seront plus rapides car elles sont incrémentielles.
...
1. **Créez des comptes Unraid %%root user|root-user%% uniques** pour chaque personne ou Mac qui sauvegardera les données. Par exemple, des utilisateurs nommés `larry`, `curly`, et `moe`. Cela garantit que chaque utilisateur dispose de sa propre identité et de ses propres permissions.

2. **Créez des %%user shares|user-share%% Time Machine individuels** pour chaque utilisateur, en les nommant clairement pour correspondre à l'utilisateur, comme `tm-larry`, `tm-curly`, et `tm-moe`. Cela aide à gérer le stockage et l'accès pour chaque utilisateur.

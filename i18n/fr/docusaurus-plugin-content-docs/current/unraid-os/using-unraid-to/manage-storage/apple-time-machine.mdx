...
2. Configurez les paramètres suivants :
   ...
   | **Espace libre minimum**                       | Réglez sur **1** (1 Ko) pour de meilleurs résultats. Cela empêche Time Machine d'échouer lorsque les disques approchent la capacité.                                                                                                                    |
   ...
   | **Taille du volume Time Machine**              | Réglez la taille maximale désirée, ou laissez non défini. Si vous ne réglez pas l'espace libre minimum à 1, appliquez d'abord le partage, puis modifiez à nouveau et réglez la taille du volume à moins de l'espace libre minimum calculé qui apparaît. |
...
4. Ouvrez **Paramètres système** si vous utilisez macOS Ventura 13.0 ou plus, ou **Préférences système** si vous êtes sur macOS Monterey 12.x ou une version antérieure. Ensuite, allez à **Time Machine**.
...
1. **Créez des comptes utilisateur Unraid %%root user|root-user%% uniques** pour chaque personne ou Mac qui sauvegardera des données. Par exemple, des utilisateurs nommés `larry`, `curly` et `moe`. Cela garantit que chaque utilisateur a sa propre identité et permissions.

2. **Créez des %%user shares|user-share%% Time Machine individuels** pour chaque utilisateur, en les nommant clairement pour correspondre à l'utilisateur, comme `tm-larry`, `tm-curly`, et `tm-moe`. Garder les partages séparés aide à gérer le stockage et l'accès pour chaque utilisateur.

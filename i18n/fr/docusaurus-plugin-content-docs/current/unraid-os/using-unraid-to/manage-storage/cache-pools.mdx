...
| %%RAID\|raid%% Level | Data Protection   | Space Efficiency | Cas d'utilisation                                                                                         |
| -------------------- | ----------------- | ---------------- | --------------------------------------------------------------------------------------------------------- |
...
| %%RAID 10\|raid10%%  | 1 panne de disque | 50%              | Combines %%RAID 0\|raid0%% speed with %%RAID 1\|raid1%% redundancy for high-performance needs.            |
...
| %%RAID 6\|raid6%%\*  | 2 disk failures   | 50-88%           | **Expérimental.** Offre une protection supplémentaire pour le stockage d'archives avec de grands disques. |
...
- **Capacité de remplacement à chaud :** Si votre matériel prend en charge le remplacement à chaud, vous n'aurez pas besoin d'éteindre votre système pour remplacer le disque.
  :::
...
<details>
  <summary><strong>Pourquoi les fichiers se retrouvent-ils parfois dans le mauvais pool ou cache ?</strong> - Cliquez pour étendre/réduire</summary>

  Lorsque vous déplacez des fichiers entre des [%%user shares|user-share%%](./shares.mdx) au niveau Linux (par exemple, en utilisant `mv` ou à l'intérieur d'un conteneur Docker), Linux tente d'optimiser l'opération. Si la source et la destination apparaissent sur le même point de montage (`/mnt/user`), Linux peut renommer le fichier au lieu de le déplacer. Cela peut entraîner le maintien des fichiers sur le disque ou pool d'origine, même si le réglage "Utiliser le cache" du partage est réglé sur "Non".
...
| --------------------------------- | -------------------------------------------------------------------------- | ---------------------------------------- |
| High-performance %%VMs\|vm%%      | Pool NVMe SSD, %%RAID 1\|raid1%%, %%BTRFS\|btrfs%% ou %%ZFS\|zfs%%         | Entrée/Sortie rapide avec redondance     |
| Stockage Docker/Appdata           | Pool SSD, %%RAID 1\|raid1%%, %%BTRFS\|btrfs%% ou %%ZFS\|zfs%%              | Accès rapide et protection des données   |
| Téléchargements de médias en vrac | Large HDD pool, %%RAID 0\|raid0%% or single, %%XFS\|xfs%%/%%BTRFS\|btrfs%% | Grande capacité avec moins de redondance |
| Isolation de projet/équipe        | Pools séparés pour chaque équipe/projet                                    | Réduit les conflits de ressources        |
| Snapshots and backup targets      | Pool %%ZFS\|zfs%%, %%RAIDZ1\|raidz1%%/%%RAIDZ2\|raidz2%% (multi-appareil)  | Supports native snapshots and backups    |
...
- %%XFS|xfs%% : Convient aux pools à un seul appareil.
  :::

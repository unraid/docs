...
Dans Unraid, un %%cache pool|cache-pool%% est un ensemble d'un ou de plusieurs disques, généralement des SSD ou des HDD à haute vitesse. Ces disques stockent temporairement les données avant qu'elles ne soient déplacées vers votre [array](./array/overview.mdx) principal. L'utilisation de %%cache pools|cache-pool%% peut améliorer considérablement les vitesses d'écriture, protéger vos données et fournir un stockage dédié à des tâches spécifiques, comme l'exécution de conteneurs Docker ou de [machines virtuelles](../create-virtual-machines/overview-and-system-prep.mdx).
...
- **Intégration de %%Mover|mover%% :** Les données écrites sur un %%cache pool|cache-pool%% sont automatiquement transférées vers votre %%array|array%% principal selon un calendrier que vous définissez. Cela garde vos [partages utilisateur](./shares.mdx) organisés et faciles à gérer.
...
### Ajout de disques à un pool {/* #btrfs-pools */}
...
![Conversion de RAID 1 à Single profile](/img/Btrfs-raid1.jpg)
...
:::info[Troubleshooting des opérations d'équilibrage si bloquées]
...
Il arrive parfois que vous deviez déplacer des fichiers entre votre %%cache pool|cache-pool%% et l'%%array|array%% principal, par exemple lors de la préparation d'une maintenance, d'une mise à niveau matérielle ou d'une optimisation des performances. Ce processus est également utile pour sauvegarder votre %%cache pool|cache-pool%% avant d'effectuer des modifications de configuration ou de remplacer des disques. Unraid fournit un outil intégré appelé %%Mover|mover%% pour automatiser ce processus pour les [partages utilisateur](./shares.mdx).
...
Lors de l'accès à un [partage utilisateur](./shares.mdx) depuis plusieurs pools et des disques de l'%%array|array%%, Unraid fusionne les listes de répertoires dans cet ordre :

...
<details>
  ...
  Comme le vidage d'un disque peut prendre de nombreuses heures, utilisez une session de terminal persistante, comme `tmux` via %%SSH|ssh%% ou un plugin de terminal installé depuis [Community Applications](/community-applications/). Vérifiez le mainteneur, le modèle et les détails de support de tout plugin avant de l'installer.

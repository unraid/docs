...
<Tabs>
  <TabItem value="type-réseau" label="Type de réseau" default>
    <DockerNetworkType />
  </TabItem>

  <TabItem value="mappages-volume" label="Mappages de volume">
    <DockerVolumeMappings />
  </TabItem>

  <TabItem value="mappages-port" label="Mappages de port">
    <DockerPortMappings />
  </TabItem>

  <TabItem value="variables-environnement" label="Variables d'environnement">
...
Certains conteneurs dépendent d'autres pour fonctionner correctement. Par exemple, une application peut avoir besoin d'un conteneur de base de données en premier, ou un service peut nécessiter un conteneur [%%VPN|vpn-tunnel%%](../../system-administration/secure-your-server/tailscale.mdx) actif avant de démarrer.
...
- Examinez la documentation de chaque conteneur pour comprendre les exigences spécifiques de démarrage.
  :::
...
Unraid vous offre un contrôle flexible sur les conteneurs qui démarrent automatiquement et dans quel ordre. Ceci est particulièrement utile si vous avez des conteneurs qui dépendent d'autres (comme un service de base de données ou un service %%VPN|vpn-tunnel%%).
...
:::tip
Définir l'ordre correct et les temps d'attente est particulièrement important pour les conteneurs qui dépendent de services fournis par d'autres (par exemple, démarrer une base de données avant une application qui l'utilise, ou un %%VPN|vpn-tunnel%% avant des applications dépendantes).
:::
...
Une fois que vous avez créé et démarré votre conteneur Docker, sa gestion est facile via le %%WebGUI|web-gui%%. Ce guide vous expliquera comment utiliser le menu contextuel, comprendre les indicateurs de santé du conteneur et accéder aux mappages de volumes dans votre conteneur.
...
| **Console**        | Ouvre une interface de ligne de commande pour interagir directement avec le conteneur.                                                               |
...
| **Journaux**       | Affiche la sortie du journal du conteneur pour le dépannage.                                                                                         |
...
| **Supprimer**      | Supprime le conteneur. Vous pouvez choisir de supprimer uniquement le conteneur ou aussi son image.                                                  |
| **Page du projet** | Ouvre la page de projet du conteneur pour la documentation et l'assistance.                                                                          |
| **Support**        | Fournit un accès aux ressources de support et d'aide pour le conteneur.                                                                              |
...
- **Stockage Appdata:**
  Chemin hôte : `/mnt/user/appdata/myapp`\

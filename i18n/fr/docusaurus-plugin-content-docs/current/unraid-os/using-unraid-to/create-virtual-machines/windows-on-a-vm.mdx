...
Si vous rencontrez des problèmes avec l'audio HDMI dans une %%VM|vm%% Windows qui utilise le %%GPU en passthrough|gpu-passthrough%% (ce qui se produit souvent avec les cartes graphiques NVIDIA), activer les Interruptions Signalées par Message (MSI) pourrait aider. MSI améliore la manière dont les interruptions sont gérées pour les périphériques en passthrough.

...
<Tabs>
  <TabItem value="créer" label="Création de snapshots" default>
    ...
    - Conservez les snapshots pour les jalons importants, mais supprimez les anciens pour économiser de l'espace de stockage.
  ...
  <TabItem value="gérer" label="Gestion des snapshots">
    ...
    **Supprimer snapshot** : Cela supprime simplement le fichier de superposition du snapshot sans affecter votre %%VM|vm%%. Votre %%VM|vm%% continue d'utiliser le fichier disque original, et tous les changements effectués après le snapshot sont perdus.
  ...
  <TabItem value="revenir" label="Revenir aux snapshots">
    ...
    :::caution[Data avertissement de perte]

    Revenir à un snapshot supprime définitivement tous les changements effectués après la création de ce snapshot. Assurez-vous de vouloir vraiment perdre ces changements avant de revenir en arrière.

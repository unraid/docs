...
| Technologie         | Ce qu'il fait                                                                        | Recommandé pour de meilleures performances                                         |
| ------------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
...
| **RAM**             | Gère les définitions des %%VM\|vm%%, ainsi que les interfaces de stockage et réseau. | 16 Go ou plus                                                                      |
...
<Tabs>
  <TabItem value="Linux" label="Linux" default>
    ...
    6. Prenez note de la lettre du périphérique (par exemple, `sdb`, `sdc`) et de la taille du disque. Vous aurez besoin d'au moins cet espace libre sur votre **%%array|array%%** ou **%%cache pool|cache-pool%%** pour le nouveau %%virtual disk|vdisk%%.
  ...
  <TabItem value="Windows" label="Windows">
    ...
    - Ensuite, faites défiler l'XML et trouvez la balise `<target>` pour le `<disk>` qui a un fichier `<source>` défini sur `vdisk1.img`. Cela devrait ressembler à ceci :

...
2. Défilez vers le bas dans l'XML et trouvez l'entrée `<disk>` existante pour votre %%disque virtuel|vdisk%% principal. Cela pointe généralement vers un fichier `.img` et ressemblera à ceci :
   ...
   Si votre disque est IDE, changez le bus de "SATA" à "IDE" et l'appareil cible de "hdd" à "hda".
...
<details>
  ...
  1. Dans votre VM %%Xen|xen-hvm%%, ouvrez le **Gestionnaire de périphériques Windows** (appuyez sur *touche Windows + X*, puis sélectionnez **Gestionnaire de périphériques**).
...
<details>
  ...
  :::caution
  La suppression de GPLPV nécessite une édition extensive du registre et la suppression manuelle des fichiers. Suivez les instructions du Wiki du Projet Xen **strictement** telles qu'écrites, car des erreurs peuvent entraîner une instabilité du système ou des échecs de démarrage.
  :::
</details>
...
<details>
  ...
  1. Assurez-vous que le **gestionnaire de VM** est activé :
     - Allez dans ***Paramètres → Gestionnaire VM***.
  ...
  :::tip[Why le disque temporaire ?
  Le disque temporaire de 1 Mo force Unraid à assigner le disque principal en tant que `hda` (IDE) au lieu de `vda` (%%VirtIO|virtio%%), permettant à Windows de démarrer sans pilotes %%VirtIO|virtio%% au départ. Ce disque temporaire sera retiré après l'installation du pilote.
  :::
</details>
...
<details>
  ...
  :::tip[Résolution des problèmes de pilote]
  Si vous remarquez des erreurs sur les périphériques après migration :
  ...
  3. Visitez la page des [Pilotes VirtIO de Red Hat](https://docs.fedoraproject.org/en-US/quick-docs/creating-windows-virtual-machines-using-virtio-drivers/) pour les dernières versions.
     :::
</details>

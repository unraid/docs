...
<Tabs>
  <TabItem value="Linux" label="Linux" default>
    ...
    6. Notez la lettre du périphérique (par exemple, `sdb`, `sdc`) et la taille du disque. Vous aurez besoin d'au moins cet espace libre sur votre **%%array|array%%** ou **%%cache pool|cache-pool%%** pour le nouveau %%virtual disk|vdisk%%.
    ...
    - Réglez le **BIOS** sur **OVMF**.
      Assurez-vous de préciser l'ISO de [pilotage VirtIO pour des performances optimales](./overview-and-system-prep.mdx#set-up-virtualization-preferences).
    ...
    - **\[vdisk\_share]** : Le partage où vous enregistrez vos images de %%VM|vm%%.
  ...
  <TabItem value="Windows" label="Windows">
    ...
    - Ensuite, faites défiler le XML et trouvez la balise `<target>` pour le `<disk>` qui a un fichier `<source>` défini sur `vdisk1.img`. Cela devrait ressembler à ceci :
    ...
    - Ouvrez l'**Explorateur de fichiers Windows** et accédez au %%CD-ROM virtuel VirtIO|virtio%% pour en explorer le contenu.
      ...
      - Cliquez avec le bouton droit sur le fichier **balloon.inf** et sélectionnez **Install** dans le menu contextuel. (Vous devez activer la visibilité de l'extension de fichier pour faire cela.)
    ...
    <h4>Étape 3 : Convertir le bus du disque en VirtIO</h4>

    - Si votre %%VM|vm%% est en cours d'exécution, allez-y et **arrêtez-le**.
    - Dans l'onglet **VMs**, cliquez sur l'icône %%VM|vm%% et sélectionnez **Edit XML** dans le menu contextuel.
...
[Comment passer des disques durs, convertir des disques et tester les performances dans les VM Unraid (YouTube)](https://www.youtube.com/watch?v=QaB9HhpbDAI).
...
<details>
  <summary><strong>Étape 3 : Désinstallez les pilotes GPLPV</strong> - Cliquer pour développer/réduire</summary>
  ...
  :::caution
  La suppression de GPLPV nécessite une modification approfondie du registre et une suppression manuelle des fichiers. Suivez exactement les instructions de la Xen Project Wiki, car les erreurs peuvent causer une instabilité du système ou des échecs de démarrage.
  :::
</details>
...
<details>
  <summary><strong>Étape 6 : Finalisez la configuration de la VM</strong> - Cliquer pour développer/réduire</summary>
  ...
  :::tip[Résolution des problèmes de pilotes]

...
3. Ouvrez ***Paramètres → Tailscale*** et cliquez sur Réauthentifier. Connectez-vous avec votre compte %%Tailscale|tailscale%%.
...
6. Dans ***Paramètres → Tailscale***, trouvez le nom et l'IP de votre serveur %%Tailnet|tailnet%%. Utilisez-les pour accéder aux partages SMB/NFS, conteneurs Docker, et plus depuis n'importe quel appareil sur votre %%Tailnet|tailnet%%.
...
- **Funnel** : Publie le site web du conteneur sur l'internet public via une URL HTTPS unique. Toute personne ayant le lien peut y accéder, même si elle n'est pas sur votre Tailnet. Utilisez ceci avec prudence, car cela expose votre service à l'internet plus large.
  :::
...
- Pour la plupart des utilisateurs, les paramètres par défaut fournissent un accès sécurisé et fiable. Des options avancées sont disponibles pour des besoins de réseau spécialisés.
  :::

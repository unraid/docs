...
Pour la plupart des utilisateurs, le paramètre par défaut **Utiliser le NAT** fonctionne immédiatement et permet d'accéder à Unraid et à la plupart des périphériques du réseau local. Cependant, si vous utilisez des conteneurs Docker avec des IP personnalisées ou des %%VM|vm%%s avec des exigences de réseau strictes :

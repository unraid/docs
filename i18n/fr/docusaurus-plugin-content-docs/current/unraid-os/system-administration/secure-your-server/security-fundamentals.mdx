...
- Envisagez d'utiliser le plugin **Dynamix Password Validator** de [Community Apps](/community-applications/) pour un retour d'information en temps réel sur la solidité.
...
### Évitez d'exposer le partage du périphérique de démarrage — ou rendez-le privé

Le périphérique de démarrage Unraid contient des fichiers système et de configuration critiques. Bien qu'il puisse être pratique d'exposer son partage %%SMB|samba%% (souvent encore nommé **`flash`** sur le disque, même lorsque le %%WebGUI|web-gui%% étiquette le volume **Boot device** sur la version 7.3.0+) pour une configuration avancée, cela introduit un risque important s'il est laissé public.

- N'exposez ce partage que si c'est absolument nécessaire et réglez-le sur **Privé**.

...
<details>
  ...
  Si vous avez utilisé un nom de pont personnalisé pour le réseau %%VM|vm%%, mettez à jour toutes les %%VMs|vm%% pour utiliser le pont `br0` par défaut en suivant ces étapes :
...
<details>
  ...
  Pour les anciens %%VMs|vm%%, vous devrez peut-être mettre à jour le pilote vidéo %%VNC|vnc-session%% :
...
<details>
  <summary><strong>Essayer de démarrer ma VM donne une erreur "Type de machine invalide"</strong> - Cliquez pour développer/réduire</summary>
...
<details>
  <summary><strong>Performance médiocre de la VM après la mise à niveau</strong> - Cliquez pour développer/réduire</summary>

  Si votre %%VM|vm%% est lente après une mise à jour, allez dans les paramètres du %%VM|vm%% (en **Vue Avancée**) et mettez à jour la version du type de **Machine** à la révision la plus récente (par exemple, passez de `i440fx-2.5` à `i440fx-2.7`). Assurez-vous de ne pas changer le préfixe (par exemple, ne passez pas de `i440fx` à `Q35`).
</details>

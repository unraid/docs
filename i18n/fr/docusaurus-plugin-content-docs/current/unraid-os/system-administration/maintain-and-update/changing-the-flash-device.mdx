...
4. Si des erreurs sont trouvées et réparées, vous pouvez continuer à utiliser l'appareil. Si les problèmes persistent ou se reproduisent, le remplacement est recommandé.
...
Il existe deux façons de remplacer votre dispositif USB Unraid : l'outil recommandé [créateur de clé USB](https://unraid.net/download) pour Windows ou macOS, ou la méthode manuelle pour les utilisateurs avancés. **Toujours sauvegarder votre dispositif flash avant de commencer.**
...
Vous avez maintenant remplacé votre périphérique USB et transféré votre clé d'enregistrement Unraid. Le système affichera la date d'enregistrement et la prochaine date de remplacement éligible.
...
- Si vous devez remplacer votre clé à nouveau dans les 12 mois, [contactez le support Unraid](https://unraid.net/contact). Pour un service plus rapide, incluez vos anciens et nouveaux %%GUID|guid%% USB, votre clé de licence et l'adresse e-mail utilisée pour l'achat.
...
N'assignez jamais un disque de données comme un %%parity-drives|parity%%. Des attributions incorrectes peuvent écrire la parité sur les données, entraînant une perte permanente de données.
...
2. Allez à l'onglet ***Principal***
   ...
   - Ne cliquez pas sur **Format** sur n'importe quel disque. Si plus de disques non montables apparaissent que prévu pour les disques %%parity|de parité%%, arrêtez et demandez de l'aide sur les forums.
...
6. Démarrez la %%array|array%% pour valider les attributions. Les disques de données devraient se monter, et leur contenu devrait rester intact.

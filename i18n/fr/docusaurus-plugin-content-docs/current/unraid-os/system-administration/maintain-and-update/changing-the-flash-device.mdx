...
4. Si des erreurs sont trouvées et réparées, vous pouvez continuer à utiliser l'appareil. Si les problèmes persistent ou réapparaissent, un remplacement est recommandé.
...
Il existe deux façons de remplacer votre dispositif flash USB Unraid : l'outil recommandé [USB flash creator tool](https://unraid.net/download) pour Windows ou macOS, ou la méthode manuelle pour les utilisateurs avancés. **Toujours sauvegarder votre dispositif flash avant de commencer.**
...
Si vous transférez une configuration d'essai vers un nouveau dispositif flash, vous ne pourrez pas démarrer l'%%array|array%% tant que vous n'aurez pas acheté une clé d'enregistrement valide. Les clés d'essai ne fonctionnent que sur l'appareil d'origine.
...
Vous avez maintenant remplacé votre dispositif flash USB et transféré votre clé d'enregistrement Unraid. Le système affichera la date d'enregistrement et la prochaine date de remplacement éligible.
...
- Si vous avez besoin de remplacer votre clé à nouveau dans les 12 mois, [contactez le support Unraid](https://unraid.net/contact). Pour un service plus rapide, incluez vos anciens et nouveaux %%GUIDs|guid%% USB, votre clé de licence et l'adresse email utilisée pour l'achat.
...
Ne jamais assigner un disque de données comme un %%parity drive|parity-drives%%. Des affectations incorrectes peuvent écraser les données avec des informations de parité, entraînant une perte de données permanente.
...
- **Disques de données** précédemment utilisés par Unraid conservent leurs données même après la réinitialisation de la configuration de l'%%array|array%%.
...
2. Allez à l'onglet ***Principal***
   ...
   - Ne cliquez pas sur **Formater** sur n'importe quel disque. Si plus de disques non montables apparaissent que le nombre attendu de disques de %%parity|parity%%, arrêtez et demandez de l'aide sur les forums.
...
5. Si vous avez un seul disque de %%parity|parity%%, l'ordre des disques de données n'est pas critique. Cochez **La parité est valide** uniquement si vous êtes sûr d'avoir assigné le même disque de parité physique et qu'aucune écriture n'a eu lieu depuis la configuration originale. Pour une double %%parity|parity%%, l'ordre des disques de données et l'assignation de parity1 et parity2 sont importants—parity1 et parity2 ne sont pas interchangeables, et la %%parity|parity%% doit être reconstruite si l'une d'elles est modifiée.

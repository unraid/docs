...
Les utilisateurs existants peuvent aller dans ***Tools → Onboarding Wizard*** pour vérifier les paramètres et migrer vers le démarrage interne.
...
- Nouveau : ajouter une commande utilisateur pour la taille maximale de l’ARC ZFS dans ***Paramètres → Paramètres du disque → Paramètre ajustable (zfs_arc_max)***. Cela était auparavant contrôlé via un paramètre personnalisé du pilote ZFS. Vous devrez peut-être migrer votre réglage vers le nouvel emplacement.
...
- Nouveau : Ajouter une page factice ***Settings → Tailscale*** pour faciliter la découverte du plugin.

...
Unraid OS 7.2.8 est une mise à jour stable axée sur la correction des vulnérabilités de sécurité, une mise à jour du noyau Linux vers 6.12.98-Unraid, des mises à jour de l’API Unraid, des ajouts de micrologiciels et des mises à jour générales des paquets de la distribution de base.

Nous recommandons de mettre à niveau depuis Unraid OS 7.2.7 afin de bénéficier des correctifs de sécurité inclus.
...
- Aucun problème connu spécifique à cette version n’est documenté. Consultez les notes de version d’Unraid OS 7.2.7 pour connaître les problèmes connus qui peuvent encore s’appliquer.

## Notes

- Cette version inclut des fichiers de microprogramme Intel iwlwifi ajoutés.

## Rétrogradation

- Aucun avertissement de restauration spécifique à cette version n’est documenté. Consultez les notes de restauration d’Unraid OS 7.2.7 avant de procéder à une restauration.

## Modifications par rapport à Unraid OS 7.2.7

### Sécurité

- Sécurité : inclut des mises à jour liées à la sécurité dans l’ensemble des paquets de la distribution de base. Les annotations CVE détaillées des paquets figurent sous Mises à jour de la distribution de base.
...
- Sécurité : corrige des failles de sécurité dans des composants qui affectaient Unraid OS 7.2.7, notamment bind, curl, dnsmasq, iperf3, libXfont2, libevent, libgpg-error, libidn, libseccomp, libssh, nginx, OpenSSH, OpenSSL, p11-kit, rsync, Samba, shadow, sqlite et xorg-server.

### Réseau / Matériel

- Nouveau : 304 fichiers de firmware Intel iwlwifi ajoutés.

## Docker

- Les réseaux automatiques Docker VLAN peuvent revenir à la passerelle Docker configurée afin que les conteneurs conservent la connectivité sortante.

### Unraid API

- Mettre à jour l’API Unraid vers [dynamix.unraid.net](\<https://dynamix.unraid.net>) 4.36.1 - [voir les changements](\<https://github.com/unraid/api/releases>).

### Noyau Linux

- Mise à niveau vers 6.12.98-Unraid
...
- libgpg-error : version 1.56 -> 1.61 (correctif de sécurité mentionné ; aucun identifiant CVE listé)
...
- openssh : version 10.2p1 -> 10.4p1-2 (correctif de sécurité mentionné ; aucun identifiant CVE listé)
...
- shadow : version 4.18.0 -> 4.19.4-6 (correctif de sécurité mentionné ; aucun identifiant CVE listé)

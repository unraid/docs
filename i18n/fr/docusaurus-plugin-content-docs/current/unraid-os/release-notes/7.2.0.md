...
Recherchez cette page pour `-rc.2` pour voir ce qui a changé depuis le dernier RC.
...
Pour des instructions détaillées, consultez [Mise à niveau d'Unraid](../../unraid-os/system-administration/maintain-and-update/upgrading-unraid.mdx). Des questions sur votre [licence](../../unraid-os/troubleshooting/licensing-faq.mdx#pre-release-policy) ?
...
- Avec le tableau en cours d'exécution, sur _**Principal → Périphériques de la piscine**_, sélectionnez le nom de la piscine pour voir les détails
...
- Amélioration : Un répertoire `lost+found` de niveau supérieur ne sera pas partagé \[-rc.2]
- Correctif : Dans smb.conf, définissez `smb3 directory leases = no` pour éviter des problèmes avec la version actuelle de Samba \[-rc.2]
- Correctif : Restaurer les commentaires dans les fichiers par défaut `/etc/modprobe.d/*.conf` \[-rc.2]
- Correctif : Windows ne parvient pas à créer un nouveau dossier pour un partage avec pool primaire=ZFS et disque secondaire=array ext4 \[-rc.2]
- Correctif : Les nouveaux périphériques ajoutés à une matrice existante avec parité valide doivent être repartitionnés \[-rc.2]
- Correctif : Ne pas éteindre les périphériques pour lesquels le test auto SMART est en cours \[-rc.2]
- Correctif : Nouvel appareil de la matrice non disponible pour les partages jusqu'à ce que la matrice soit redémarrée \[-rc.2]
...
- Correctif : La page _**Paramètres → Paramètres Réseau → Règles Interfaces**_ affichait parfois le mauvais pilote réseau (ce n'était qu'un problème d'affichage)
...
- Correctif : Changer les paramètres par défaut de la VM Windows 11 \[-rc.2]
- Correctif : Aberration d'affichage lors de la création d'un instantané de VM \[-rc.2]
...
- Fonctionnalité : Ajouter un bouton _Force Install_ pour contourner les vérifications de version lors de l'installation manuelle de plugins
- Fonctionnalité : Ajout de la page _**Outils → Ouvrir le terminal**_ ; vous pouvez l'accéder en recherchant "terminal". Il est possible de supprimer le bouton Terminal de la barre d'outils via _**Paramètres → Paramètres d'affichage → Afficher le bouton Terminal dans l'en-tête**_
- Fonctionnalité : _**Utilisateurs → Root → Clés SSH autorisées**_ prend désormais en charge plus de formats (merci [wandercone](https://github.com/wandercone))
...
- Amélioration : Afficher la RAM et la vitesse réseau en unités lisibles \[-rc.2]
- Correctif : Sur _**Paramètres → Paramètres d'affichage → Taille de la police**_, supprimer les options extrêmes qui cassent le WebGUI \[-rc.2]
- Correctif : Aberration d'affichage sur _**Paramètres → Planificateur**_ lorsqu'il est défini sur Horaire \[-rc.2]
- Correctif : Les éléments désactivés du menu déroulant n'étaient pas affichés en couleur désactivée dans Firefox \[-rc.2]
...
- Correctif : Prévenir les erreurs liées à _searchLink_ lors de l'installation de plugins
- Correctif : Avertissements PHP lors de l'importation de tunnels WireGuard
- Amélioration : Le fuseau horaire _Europe/Kiev_ renommé en _Europe/Kyiv_ pour s'aligner avec la base de données des fuseaux horaires de l'IANA
...
- Correctif : Anonymisation supplémentaire de diagnostics.zip \[-rc.2]
- Amélioration : Protéger le WebGUI contre les erreurs PHP fatales \[-rc.2]
- Amélioration : Ajuster la journalisation lors de l'installation de plugins \[-rc.2]
- Correctif : Fuite mémoire emhttpd \[-rc.2]

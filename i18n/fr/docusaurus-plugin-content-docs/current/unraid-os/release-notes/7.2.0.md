...
#### Extension ZFS RAIDZ {/* #raidz-expansion */}

Vous pouvez maintenant étendre vos pools RAIDZ1/2/3 à un seul VDEV, un disque à la fois. Pour des instructions détaillées, voir [extension RAIDZ](/unraid-os/using-unraid-to/manage-storage/cache-pools#raidz-expansion).
...
- Correctif : Dans ***Paramètres → Paramètres d'affichage → Taille de police***, supprimer les options extrêmes qui cassent le WebGUI
...
- Correctif : Échec du NAT Docker en raison du manque de br_netfilter
- Correctif : Les exécutions programmées du déménageur ne sont pas enregistrées

### Noyau Linux

- version 6.12.54-Unraid
  - intégré : CONFIG_EFIVAR_FS : système de fichiers des variables EFI
  - CONFIG_INTEL_RAPL : support Intel RAPL via l'interface MSR
  - CONFIG_NLS_DEFAULT : changement de "iso8859-1" à "utf8"
  - Ajout de la prise en charge de l'eMMC :
    - CONFIG_MMC : Support carte MMC/SD/SDIO
    - CONFIG_MMC_BLOCK : Pilote de périphérique bloc MMC
    - CONFIG_MMC_SDHCI : Support Interface de Contrôleur Hôte Digital Sécurisé
    - CONFIG_MMC_SDHCI_PCI : Support SDHCI sur bus PCI
    - CONFIG_MMC_SDHCI_ACPI : Support SDHCI pour contrôleurs SDHCI énumérés par ACPI
    - CONFIG_MMC_SDHCI_PLTFM : SDHCI plate-forme et assistant pilote OF

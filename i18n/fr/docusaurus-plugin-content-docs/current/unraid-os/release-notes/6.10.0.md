...
- Ajout de ce qu'on appelle "add-relaxable-rmrr-5_8_and_up.patch" modifié pour
  notre noyau :
...
- sysctl: gérer le dépassement du maximum net.netfilter.nf_conntrack_count (augmenter le réglage à 131072) - chapeau à
  Membre de la Communauté [@DieFalse](https://forums.unraid.net/profile/77686-diefalse/)
- Correction d'un bogue trouvé par [@thohell](https://forums.unraid.net/profile/12113-thohell/) où md_sync_limit n'était pas respecté pour
  limiter l'utilisation du cache stripe_head lorsque d'autres I/O sont actifs.
  ...
  ajustée en modifiant la valeur 'Paramètres/Disque/Paramètres Ajustables (md_sync_limit)'.
...
- correctif : "add-relaxable-rmrr-5_8_and_up.patch" modifié pour ce
  noeud
- fonctionnalités ajoutées :
  ...
  - CONFIG_DRM_I915_GVT_KVMGT : Activer le support KVM/VFIO pour Intel
    GVT-g
  ...
  - CONFIG_ISCSI_TCP : Initiateur iSCSI via TCP/IP (par Membre de la Communauté [@ich777](https://forums.unraid.net/profile/72388-ich777/))
...
- Options de noyau BPF (sur demande de l'utilisateur) :
  ...
  - CONFIG_BPF_JIT_ALWAYS_ON : Activer en permanence BPF JIT et supprimer
    l'interpréteur BPF
  - CONFIG_NET_CLS_BPF : Classificateur basé sur BPF
  - CONFIG_NET_CLS_ACT : Actions
  - CONFIG_NET_ACT_BPF : Action basée sur BPF
  - CONFIG_IKHEADERS : Activer les en-têtes du noyau via
...
- nouveaux paramètres par défaut d'emhttp
  ...
  - changer la valeur par défaut réglable poll_attributes de 30 min à 30
    sec.
...
- rc.nginx : supprimer le bloc de serveur par défaut renvoyant 404 pour https si
  USE_SSL==no et aucun certificat signé par CA
...
- shfs : corriger le bogue où les permissions étaient ignorées
  ('default_permissions' manquant dans la commande de montage)
- sysctl : gérer le dépassement du maximum net.netfilter.nf_conntrack_count

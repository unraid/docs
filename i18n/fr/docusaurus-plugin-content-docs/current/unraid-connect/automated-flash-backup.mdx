...
<details>
  ...
  La sauvegarde inclut uniquement les fichiers de configuration, en veillant à ce que les informations sensibles, telles que les mots de passe et les clés %%WireGuard|wireguard%%, soient exclues. Les fichiers XML des modèles Docker peuvent faire partie de la sauvegarde et pourraient contenir des identifiants spécifiques à l'application.

  <h4>Conservation des données</h4>

  Seule la dernière sauvegarde est conservée. Si une sauvegarde dépasse certaines limites—spécifiquement, si des fichiers individuels dépassent 10 Mo ou si la taille totale du référentiel dépasse 100 Mo—elle sera supprimée et recréée. Les sauvegardes sont également supprimées lorsque vous désactivez la fonctionnalité de sauvegarde flash ou après une période d'inactivité spécifique.

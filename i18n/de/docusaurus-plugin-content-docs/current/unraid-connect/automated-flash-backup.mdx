...
Backups enthalten keine Dateien `config/shadow` oder `config/smbpasswd`. Benutzerkonten bleiben erhalten, Passwörter jedoch nicht. Nach der Wiederherstellung müssen Sie alle Benutzerpasswörter einschließlich des root-Passworts zurücksetzen. %%WireGuard|wireguard%%-Schlüssel sind ebenfalls ausgeschlossen.

...
Für die anfängliche WiFi-Einrichtung benötigen Sie entweder:
...
WiFi hat einige Einschränkungen im Vergleich zu kabelgebundenen Netzwerken:
...
##### Docker und virtuelle Maschinen Überlegungen

Beim Gebrauch von WiFi gibt es spezifische Überlegungen für Docker-Container und %%Virtuelle Maschinen|vm%%:
...
Docker kann nicht an zwei Netzwerken teilnehmen, die dasselbe Subnetz teilen. Wenn Sie zwischen kabelgebundenen und drahtlosen Verbindungen wechseln, müssen Sie Docker neu starten und alle vorhandenen Container neu konfigurieren, um die neue Schnittstelle zu verwenden. Diese Netzwerk-Konfigurationsänderung erfordert Container-Neukonfiguration. Es wird stark empfohlen, entweder kabelgebunden oder drahtlos zu wählen und nicht zwischen ihnen zu wechseln.
:::
...
**Virtuelle Maschinen** können Ihren Server in einen Desktop oder Media Player verwandeln und Anwendungen ausführen, die nicht für Linux entwickelt wurden. Für Details zur Verwaltung von %%VMs|vm%% auf Unraid, siehe [VM Einrichtung](../../using-unraid-to/create-virtual-machines/vm-setup.mdx).

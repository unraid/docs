...
<details>
  ...
  Docker-Container ermöglichen das Ausführen von Anwendungen in isolierten, ressourcenschonenden Umgebungen ohne den mit %%virtual machines|vm%% verbundenen Overhead. Unraid vereinfacht die Nutzung von Docker, indem es Zugriff auf Tausende vorkonfigurierte Apps über [Docker Hub](https://hub.docker.com/) und [Community Applications](/community-applications/) bietet. Dieser containerisierte Ansatz ermöglicht es Unraid-Benutzern, mehrere Anwendungen gleichzeitig auszuführen, ohne Kompatibilitätsprobleme. Außerdem hilft er dabei, das System durch eigenständige Anwendungspakete übersichtlich zu halten, und erleichtert es, die Serverfunktionen durch das Docker-Ökosystem zu erweitern.

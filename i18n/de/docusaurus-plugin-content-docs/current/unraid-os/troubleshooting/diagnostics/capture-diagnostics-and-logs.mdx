...
| %%WebGUI\|Web-GUI%% nicht verfügbar | Access via **%%SSH\|ssh%%**, telnet, or direct console to run the `diagnostics` command. The zip file saves to `/boot/logs`.                                                | Erfassen Sie die Diagnosedaten immer vor dem Neustart, um die Protokolle intakt zu halten.                                                      |
...
| **Local %%syslog\|syslog%%**  | Keeps logs on the %%array\|array%% or %%cache\|cache%%, reducing wear on flash | Weniger zugänglich bei einem Systemabsturz             | Kontinuierliche Protokollierung ohne externe Geräte |

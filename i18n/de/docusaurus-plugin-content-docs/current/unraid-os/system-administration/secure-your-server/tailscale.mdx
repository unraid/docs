...
2. Suchen Sie in Unraid im Tab [**Community Apps**](/community-applications/) nach dem offiziellen %%Tailscale|tailscale%%-Plugin und installieren Sie es.
3. Öffnen Sie ***Einstellungen → Tailscale → Einstellungen***, aktivieren Sie die ***Erweiterte Ansicht*** und klicken Sie auf Reauthentifizieren. Melden Sie sich mit Ihrem %%Tailscale|tailscale%%-Konto an.
...
1. Installieren Sie zuerst %%Tailscale|tailscale%% auf jedem Computer, der auf Ihre Docker-Container zugreift.
2. Während das Unraid-Tailscale-Plugin für die Docker-Integration nicht zwingend erforderlich ist, wird dringend empfohlen, es zu installieren und sich auf Ihrem Unraid-Server anzumelden, um die beste Erfahrung zu erzielen.
...
- **Anwenden** Sie Ihre Änderungen und überprüfen Sie den **Erweiterten Log** oder **Log-Viewer** auf Tailscale-Nachrichten. Klicken Sie auf den **Zur Authentifizierung, besuchen...** Link, um den Container in Ihrem Tailnet zu genehmigen.

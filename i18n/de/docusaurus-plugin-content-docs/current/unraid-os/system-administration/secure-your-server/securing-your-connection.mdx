...
| **SSL/TLS verwenden** | Set in ***Settings → Management Access***. Controls whether %%SSL\|ssl%% is enabled.     |
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **Selbstsigniert**    | Für nur lokale Zugriffe, wenn Sie kein vertrauenswürdiges Zertifikat benötigen.                                                 | Easy to set up, but browsers show warnings. Traffic is encrypted once the warning is accepted. |
...
| **Benutzerdefiniert** | For advanced users who want a wildcard or custom domain certificate (requires %%DNS\|dns-name-resolution%% configuration).      | Vertrauenswürdig, flexibel, erfordert jedoch zusätzliche Einrichtung.                          |

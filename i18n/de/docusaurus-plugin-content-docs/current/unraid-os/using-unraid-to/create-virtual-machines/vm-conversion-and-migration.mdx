...
| **Speicher**         | SSD- oder NVMe-**%%cache pool\|cache-pool%%** mit ausreichend Platz               | Dedicated NVMe %%cache pool\|cache-pool%% for %%VM\|vm%% storage |
...
<Tabs>
  <TabItem value="Linux" label="Linux" default>
    ...
    2. Gehen Sie zum **%%VMs|vm%%**-Tab im %%WebGUI|web-gui%%. Falls der Tab fehlt, überprüfen Sie, ob die Virtualisierung aktiviert wurde und die Hardwareanforderungen erfüllt sind.
    3. Klicken Sie auf die Registerkarte **Haupt**.
    4. Starten Sie das **%%array|array%%**, falls es noch nicht läuft.
    5. Finden Sie Ihre Festplatte unter **Nicht zugewiesene Geräte**.
    6. Notieren Sie sich den Gerätebuchstaben (z. B. `sdb`, `sdc`) und die Festplattengröße. Sie benötigen mindestens diesen freien Platz auf Ihrem **%%array|array%%** oder **%%cache pool|cache-pool%%** für das neue %%virtual disk|vdisk%%.
    ...
    - **[vdisk_share]**: Die Freigabe, in der Sie Ihre %%VM|vm%%-Images speichern
  ...
  <TabItem value="Windows" label="Windows">
    ...
    - Scrollen Sie als nächstes im XML nach unten und finden Sie das `<target>`-Tag für das `<disk>`, das eine `<source>`-Datei auf `vdisk1.img` gesetzt hat. Es sollte so aussehen:
    ...
    <h4>Schritt 3: Konvertieren Sie den Festplatten-Bus zurück zu VirtIO</h4>

    - Wenn Ihre %%VM|vm%% läuft, schalten Sie sie herunter.
    - Klicken Sie im **VMs-Tab** auf das %%VM|vm%%-Symbol und wählen Sie **XML bearbeiten** aus dem Kontextmenü.
    - Suchen Sie den `<disk>`-Abschnitt für `vdisk1.img` und ändern Sie `bus='ide'` zurück zu `bus='virtio'` im `<target>`-Tag.
...
[Wie man Festplatten durchreicht, Disketten konvertiert und die Leistung in Unraid VMs testet (YouTube)](https://www.youtube.com/watch?v=QaB9HhpbDAI).
...
2. Scrollen Sie im XML-Dokument nach unten und finden Sie den bestehenden `<disk>` Eintrag für Ihre primäre %%virtuelle Festplatte|vdisk%%. Dieser verweist normalerweise auf eine `.img` Datei und sieht ungefähr so aus:
   ...
   Falls Ihre Festplatte IDE ist, ändern Sie den Bus von "SATA" zu "IDE" und das Zielgerät von "hdd" zu "hda".
...
<details>
  ...
  1. Öffnen Sie in Ihrer %%Xen|xen-hvm%% VM den **Windows-Geräte-Manager** (drücken Sie *Windows-Taste + X* und wählen Sie dann **Geräte-Manager**).
  ...
  :::tip
  Wenn Sie keine GPLPV-Treiber verwenden, können Sie die nächsten Schritte überspringen und ab dem Abschnitt zum Neustart im %%KVM|kvm%%-Modus fortfahren.
  :::
</details>
...
<details>
  ...
  :::caution
  Die Entfernung von GPLPV erfordert eine umfangreiche Registrierung Bearbeitung und manuelles Löschen von Dateien. Befolgen Sie die Xen Project Wiki-Anweisungen **genau** so, wie sie geschrieben sind, da Fehler zu Systeminstabilität oder Startfehler führen können.
  :::
</details>
...
<details>
  ...
  :::tip[Why die temporäre Festplatte?]
  Die temporäre 1-MB-Festplatte zwingt Unraid dazu, die primäre Festplatte als `hda` (IDE) statt `vda` (%%VirtIO|virtio%%) zuzuweisen, sodass Windows zunächst ohne %%VirtIO|virtio%%-Treiber gestartet werden kann. Diese temporäre Festplatte wird nach der Treiberinstallation entfernt.
  :::
</details>
...
<details>
  ...
  Wenn Ihnen nach der Migration Fehler auf Geräten auffallen:

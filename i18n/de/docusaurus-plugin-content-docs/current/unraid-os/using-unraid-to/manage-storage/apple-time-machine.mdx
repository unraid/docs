...
2. Enter the %%SMB|samba%% address of your Unraid server. For example, use the format `smb://[server-ip]/[Time Machine share name]` and click **Connect**.
   ...
   | **Mindestfreier Speicherplatz**                 | Auf **1** (1 KB) setzen für beste Ergebnisse. Dies verhindert, dass die **Time Machine** fehlschlägt, wenn sich die Festplattenkapazität dem Maximum nähert.                                                                                                                     |
   ...
   | **Time Machine-Volume-Größe**                   | Auf Ihre gewünschte Höchstgröße setzen oder nicht festlegen. Wenn der Mindestfreiraum nicht auf 1 gesetzt wird, wenden Sie das **share** zuerst an und bearbeiten Sie es erneut, um die Volume-Größe auf weniger als den berechneten Mindestfreiraum festzulegen, der erscheint. |
...
4. Öffnen Sie **Systemeinstellungen**, wenn Sie macOS Ventura 13.0 oder später verwenden, oder **Systemeinstellungen**, wenn Sie auf macOS Monterey 12.x oder früher sind. Navigieren Sie dann zu **Time Machine**.
...
8. Time Machine wird das initiale Backup automatisch starten. Beachten Sie, dass das erste Backup eine Weile dauern kann, spätere Backups werden jedoch schneller, da sie inkrementell sind.
...
1. **Erstellen Sie einzigartige Unraid %%root-Benutzer|root-user%%-Konten** für jede Person oder jeden Mac, der Daten sichern wird. Beispielsweise Benutzer mit den Namen `larry`, `curly` und `moe`. Dies stellt sicher, dass jeder Benutzer seine eigene Identität und Berechtigungen hat.

2. **Erstellen Sie individuelle Time Machine %%user shares|user-share%%** für jeden Benutzer, und benennen Sie sie klar, um mit dem Benutzer übereinzustimmen, z. B. `tm-larry`, `tm-curly` und `tm-moe`.

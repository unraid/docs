...
2. Konfigurieren Sie die folgenden Einstellungen:
   ...
   | **Mindestfreier Speicherplatz**                 | Setzen Sie auf **1** (1 KB) für beste Ergebnisse. Dies verhindert, dass Time Machine fehlschlägt, wenn sich die Festplatten der Kapazität nähern.                                                                                                                                            |
   ...
   | **Time Machine-Volume-Größe**                   | Setzen Sie auf Ihre gewünschte Maximalgröße oder lassen Sie es unset. Wenn Sie den Mindestfreiraum nicht auf 1 setzen, wenden Sie die Freigabe zuerst an, bearbeiten Sie sie dann erneut und setzen Sie die Volumegröße auf kleiner als den berechneten Mindestfreiraum, der angezeigt wird. |
...
4. Öffnen Sie **Systemeinstellungen**, wenn Sie macOS Ventura 13.0 oder später verwenden, oder **Systemeinstellungen**, wenn Sie macOS Monterey 12.x oder früher verwenden. Navigieren Sie dann zu **Time Machine**.
...
8. Time Machine startet das erste Backup automatisch. Beachten Sie, dass das erste Backup eine Weile dauern kann, spätere Backups werden jedoch schneller, da sie inkrementell sind.
...
1. **Erstellen Sie einzigartige Unraid %%root user|root-user%%-Konten** für jede Person oder jeden Mac, der Daten sichern will. Zum Beispiel Benutzer mit den Namen `larry`, `curly` und `moe`. Dies stellt sicher, dass jeder Benutzer seine eigene Identität und Berechtigungen hat.

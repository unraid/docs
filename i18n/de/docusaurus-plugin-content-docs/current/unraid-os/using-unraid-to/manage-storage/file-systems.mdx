...
Weitere Einzelheiten zum Hinzufügen vorhandener Laufwerke finden Sie in den [Release Notes](../../release-notes/7.2.0.mdx#ext234-ntfs-und-exfat-unterstützung).
...
1. Move all data off the drive to another location (another %%array|array%% drive, %%cache pool|cache-pool%%, or external backup). Use one of these methods:
   ...
   **Oder über die Befehlszeile (nur Unraid 7.0 und 7.1):**
   ...
   - Bewegen Sie Dateien [mithilfe des 'mover', um eine Array-Festplatte zu leeren](../../release-notes/7.0.0.mdx#using-mover-to-empty-an-array-disk) auf andere Array-Laufwerke

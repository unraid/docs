...
| Dateisystem      | Data Protection                                                                       | Space Efficiency               | Erweiterte Funktionen                                            | Bester Anwendungsfall                                   |
| ---------------- | ------------------------------------------------------------------------------------- | ------------------------------ | ---------------------------------------------------------------- | ------------------------------------------------------- |
| %%XFS\|xfs%%     | Keine eingebaute Redundanz                                                            | 100%                           | Robust gegen Korruption                                          | Standard für Array-Laufwerke; hohe Zuverlässigkeit      |
| %%ZFS\|zfs%%     | %%RAIDZ1\|raidz1%%/%%RAIDZ2\|raidz2%%/%%RAIDZ3\|raidz3%%, Spiegel, usw.               | Varies by %%RAID\|raid%% level | %%Checksums\|checksum%%, %%Snapshots\|snapshot%%, %%RAID\|raid%% | Multi-device pools, %%VMs\|vm%%, Docker, advanced users |
| %%BTRFS\|btrfs%% | %%RAID 0\|raid0%%/%%RAID 1\|raid1%%/%%10\|raid10%%/%%5\|raid5%%/%%6\|raid6%%, mirrors | Varies by %%RAID\|raid%% level | %%Checksums\|checksum%%, %%Snapshots\|snapshot%%, %%RAID\|raid%% | Multigeräte-Pools, gemischte SSD/HDD, flexible Pools    |
...
Wenn Sie unsicher sind, ist es ein guter Ansatz, mit den Standardeinstellungen zu beginnen: Verwenden Sie %%XFS|xfs%% für %%array|array%% Laufwerke und %%BTRFS|btrfs%% für %%cache pools|cache-pool%%.
:::
...
3. **Copy your data back**: (Optional) Depending on where you moved the files, you may want to move them back to the newly formatted drive.
...
- Für fachkundige Hilfe, posten Sie Ihr Problem zusammen mit den Diagnoseinformationen in den [Unraid-Foren](https://forums.unraid.net/).
  :::
...
- Überprüfen Sie immer sorgfältig, bevor Sie Laufwerke formatieren oder reparieren, um versehentlichen Datenverlust zu vermeiden.
  :::
...
| **Kein gültiges %%BTRFS\|btrfs%% gefunden**         | Disk may not belong to a valid pool; check pool assignments.               |
| **Mount: falscher FS-Typ, schlechte Option, etc.**  | Falsche Auswahl des Dateisystems oder die Festplatte ist nicht formatiert. |
| **Cannot mount /dev/mdX: Structure needs cleaning** | Dateisystem ist beschädigt; führen Sie eine Reparatur durch.               |
...
- Wenn Sie Zweifel haben, posten Sie Ihre Diagnose- und Fehlermeldungen in den Unraid [Foren](https://forums.unraid.net/) für professionelle Unterstützung.
  :::

...
| %%RAID\|raid%% Level | Data Protection     | Space Efficiency | Anwendungsfall                                                                                 |
| -------------------- | ------------------- | ---------------- | ---------------------------------------------------------------------------------------------- |
...
| %%RAID 10\|raid10%%  | 1 Festplattenfehler | 50%              | Combines %%RAID 0\|raid0%% speed with %%RAID 1\|raid1%% redundancy for high-performance needs. |
| %%RAID 5\|raid5%%\*  | 1 Festplattenfehler | 67-94%           | **Experimentell.** Balanciert Kapazität und Redundanz für große Medienlager.                   |
| %%RAID 6\|raid6%%\*  | 2 disk failures     | 50-88%           | **Experimentell.** Bietet zusätzlichen Schutz für Archivspeicher mit großen Festplatten.       |
...
| ---------------------------- | -------------------------------------------------------------------------- | ------------------------------------- |
| High-performance %%VMs\|vm%% | NVMe SSD-Pool, %%RAID 1\|raid1%%, %%BTRFS\|btrfs%% oder %%ZFS\|zfs%%       | Schnelle I/O mit Redundanz            |
| Docker/Appdata-Speicher      | SSD-Pool, %%RAID 1\|raid1%%, %%BTRFS\|btrfs%% oder %%ZFS\|zfs%%            | Schneller Zugriff und Datenschutz     |
| Massenmedien-Downloads       | Large HDD pool, %%RAID 0\|raid0%% or single, %%XFS\|xfs%%/%%BTRFS\|btrfs%% | Hohe Kapazität mit weniger Redundanz  |
| Projekt-/Team-Isolation      | Separate Pools für jedes Team/Projekt                                      | Reduziert Ressourcenkonflikte         |
| Snapshots and backup targets | %%ZFS\|zfs%%-Pool, %%RAIDZ1\|raidz1%%/%%RAIDZ2\|raidz2%% (mehrere Geräte)  | Supports native snapshots and backups |

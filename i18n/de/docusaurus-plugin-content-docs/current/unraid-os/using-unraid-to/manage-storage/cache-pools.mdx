...
In Unraid ist ein %%cache pool|cache-pool%% eine Ansammlung aus einem oder mehreren Laufwerken, typischerweise SSDs oder schnellen HDDs. Diese Laufwerke speichern Daten vorübergehend, bevor sie auf Ihr [Haupt-%%array|array%%](/de/unraid-os/using-unraid-to/manage-storage/array/overview) verschoben werden. Der Einsatz von %%cache pools|cache-pool%% kann die Schreibgeschwindigkeit deutlich erhöhen, Ihre Daten schützen und dedizierten Speicher für spezielle Aufgaben wie den Betrieb von Docker-Containern oder [%%virtual machines|vm%%](../create-virtual-machines/overview-and-system-prep.mdx) bereitstellen.

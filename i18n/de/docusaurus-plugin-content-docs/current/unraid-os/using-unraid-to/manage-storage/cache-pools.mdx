...
In Unraid ist ein %%cache pool|cache-pool%% eine Sammlung aus einem oder mehreren Laufwerken, typischerweise SSDs oder schnellen HDDs. Diese Laufwerke speichern Daten vorübergehend, bevor sie auf Ihr Haupt-[Array](./array/overview.mdx) verschoben werden. Die Verwendung von %%cache pools|cache-pool%% kann die Schreibgeschwindigkeit erheblich verbessern, Ihre Daten schützen und dedizierten Speicher für bestimmte Aufgaben wie das Ausführen von Docker-Containern oder [virtuellen Maschinen](../create-virtual-machines/overview-and-system-prep.mdx) bereitstellen.
...
:::info[Troubleshooting bei Balancevorgängen bei Blockaden beheben]
...
Es gibt Zeiten, in denen Sie Dateien zwischen Ihrem %%cache pool|cache-pool%% und dem Haupt-%%array|array%% verschieben müssen, etwa bei der Vorbereitung von Wartungsarbeiten, beim Aktualisieren von Hardware oder beim Optimieren der Leistung. Dieser Vorgang ist auch nützlich, um Ihren %%cache pool|cache-pool%% vor Konfigurationsänderungen oder dem Ersetzen von Laufwerken zu sichern. Unraid bietet ein integriertes Werkzeug namens %%Mover|mover%%, um diesen Vorgang für [Benutzerfreigaben](./shares.mdx) zu automatisieren.

...
Unraid %%cache pools|cache-pool%% können in zwei Hauptmodi betrieben werden: **Einzelgerätemodus** und **Mehrgerätemodus**. Wenn Sie den Unterschied zwischen diesen Modi kennen, können Sie das richtige Gleichgewicht zwischen Leistung, Flexibilität und Datenschutz für Ihre Bedürfnisse finden.
...
Zu den häufigen Aufgaben für den %%cache pool|cache-pool%% gehören:

- Sichern Sie Ihren Pool auf dem %%array|array%%
...
<h3>Warum sollten Sie Ihren %%cache pool|cache-pool%% sichern?</h3>
...
Um Ihren %%cache pool|cache-pool%% zu sichern:
...
4. **Freigabespeicher und Mover-Aktion festlegen (Cache → Array):**
   ...
   - Für jede %%user share|user-share%%, die Sie verschieben möchten, setzen Sie **Hauptspeicher** auf den Quell-%%cache pool|cache-pool%% und **Sekundärspeicher** auf das %%array|array%%.
   - Setzen Sie die **Mover-Aktion** auf **Cache → Array**.
5. **Überprüfen Sie den Speicherplatz auf dem %%array|array%%**: Stellen Sie sicher, dass ausreichend freier Speicherplatz für Ihre Dateien vorhanden ist.
6. **Dateien auf das %%array|array%% verschieben**: Klicken Sie auf der **Hauptseite** auf **Jetzt verschieben**. Dadurch werden Dateien vom %%cache pool|cache-pool%% auf das %%array|array%% basierend auf der Mover-Aktion übertragen.
7. **Überprüfen Sie, ob Ihr Pool leer ist**: Sobald der %%Mover|mover%% abgeschlossen ist, vergewissern Sie sich, dass keine Dateien mehr im %%cache pool|cache-pool%% vorhanden sind.
...
Nachdem Sie Ihre Wartung abgeschlossen oder Ihr Gerät ersetzt haben, können Sie Dateien vom %%array|array%% zurück auf den %%cache pool|cache-pool%% wiederherstellen, indem Sie diese Schritte befolgen:

1. **Freigabespeicher und Mover-Aktion festlegen (Array → Cache):**
   - Gehen Sie zur **Shares**-Registerkarte.
   - Für jede %%user share|user-share%%, die Sie wiederherstellen möchten, setzen Sie **Hauptspeicher** auf den Ziel-%%cache pool|cache-pool%% und **Sekundärspeicher** auf das %%array|array%%.
   - Setzen Sie **Mover-Aktion** auf **Array → Cache**.
2. **Überprüfen Sie den Speicherplatz im Pool**: Stellen Sie sicher, dass im %%cache pool|cache-pool%% ausreichend freier Speicherplatz vorhanden ist.
3. **Dateien zurück in den Pool verschieben**: Gehen Sie auf die **Hauptseite** und klicken Sie auf **Jetzt verschieben**, um Dateien zurück in den %%cache pool|cache-pool%% zu übertragen.
4. **Inhalt im Pool überprüfen**: Nachdem der Transfer abgeschlossen ist, überprüfen Sie, ob Ihr %%cache pool|cache-pool%% die erwarteten Dateien enthält und dass die Beteiligungen auf dem %%array|array%% leer sind.
...
Wenn Ihr %%cache pool|cache-pool%% noch nicht als %%BTRFS|btrfs%% oder %%ZFS|zfs%% formatiert ist, folgen Sie diesen einfachen Schritten:
...
2. **Stoppen Sie das %%array|array%%**: Stellen Sie sicher, dass Sie das %%array|array%% stoppen, um den Konvertierungsprozess zu beginnen.
...
4. **Starten Sie das %%array|array%%**: Starten Sie nach der Formatänderung das %%array|array%%.
...
1. **Stoppen Sie das %%array|array%%**: Beginnen Sie erneut mit dem Stoppen des %%array|array%%.
2. **Weisen Sie zusätzliche Laufwerke zu:** Im **Main**-Tab können Sie ein oder mehrere neue Geräte Ihrem Pool zuweisen.
3. **Starten Sie das %%array|array%%**: Sobald die Laufwerke zugewiesen sind, starten Sie das %%array|array%%.
...
1. **Stoppen Sie das %%array|array%%**: Beginnen Sie, indem Sie das aktuelle %%array|array%% stoppen, um sicherzustellen, dass der Prozess reibungslos verläuft.
...
6. **Starten Sie das %%array|array%%**: Nachdem Sie alle Geräte zugewiesen haben, starten Sie das %%array|array%% erneut, um diese Änderungen zu aktivieren.
...
1. **Starten Sie das %%array|array%%** im Normalmodus, wenn es nicht bereits läuft.
...
1. **Stoppen Sie das %%array|array%%**: Gehen Sie zum **Haupt**-Tab, finden Sie den **Array-Betrieb**-Abschnitt und klicken Sie auf die **Stopp**-Schaltfläche. Dies wird das %%array|array%% sicher anhalten, um die Festplatte zu ersetzen.
...
6. **Starten Sie das %%array|array%%**: Klicken Sie auf die **Start**-Schaltfläche, um den Prozess der Integration der neuen Festplatte in das %%array|array%% zu beginnen.
...
Das Festlegen eines minimalen freien Speicherplatzes für Ihren %%cache pool|cache-pool%% kann eine bessere Kontrolle über die Dateiplatzierung bieten, insbesondere beim Umgang mit großen Dateien wie hochauflösenden Videos. Diese Einstellung hilft Unraid zu wissen, wann es aufhören soll, auf den Pool zu schreiben, und direkt auf das größere Speicher %%%%array|array%%% zu schreiben, um Unterbrechungen oder Datenkorruption zu vermeiden.
...
- Wenn Ihr Anteil einen %%cache pool|cache-pool%% als **Hauptspeicher** verwendet, gehen die Dateien in den Pool, bis er den minimalen freien Speicherplatz erreicht, dann werden sie direkt an das %%array|array%% gesendet.
- If set to use a %%cache pool|cache-pool%% exclusively (no **Secondary storage**), this setting is not applied.
- Wenn nur das %%array|array%% als **Hauptspeicher** verwendet wird, gehen die Dateien direkt in das %%array|array%%.
...
Es gibt Zeiten, in denen Sie Dateien zwischen Ihrem %%cache pool|cache-pool%% und dem Haupt-%%array|array%% verschieben müssen, z. B. bei der Vorbereitung auf Wartungsarbeiten, der Hardwareaufrüstung oder der Leistungsoptimierung. Unraid bietet ein eingebautes Tool namens %%Mover|mover%%, um diesen Vorgang für [%%user shares|user-share%%](./shares.mdx) zu automatisieren.
...
<details>
  <summary><strong>Warum landen Dateien manchmal im falschen Pool oder Cache?</strong> - Zum Erweitern/Zusammenklappen klicken</summary>

  Wenn Sie Dateien auf Linux-Ebene zwischen [%%user shares|user-share%%](./shares.mdx) verschieben (zum Beispiel mit `mv` oder innerhalb eines Docker-Containers), versucht Linux, den Vorgang zu optimieren. Wenn sowohl die Quelle als auch das Ziel auf dem gleichen Einhängepunkt (`/mnt/user`) erscheinen, könnte Linux die Datei umbenennen, anstatt sie zu verschieben. Dies kann dazu führen, dass Dateien auf der ursprünglichen Festplatte oder im ursprünglichen Pool verbleiben, selbst wenn die Einstellung "Cache verwenden" des Freigabeverzeichnisses auf "Nein" gesetzt ist.
...
- %%XFS|xfs%%: Geeignet für Pools mit einem einzelnen Gerät.
  :::

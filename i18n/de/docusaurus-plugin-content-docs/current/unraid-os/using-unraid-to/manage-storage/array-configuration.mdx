...
1. Gehe zu ***Main → Array-Geräte*** in der Oberfläche.
...
3. **Weisen Sie das Zieldisk nicht zu:** Sobald das Array gestoppt ist, finden Sie das Laufwerk, das Sie ersetzen möchten. Entfernen Sie es aus seinem Slot in ***Main → Array-Geräte***.
...
<details>
  ...
  :::caution
  Wenn Sie versuchen, ein unmountbares %%emuliertes Laufwerk|emulated-disk%% wiederherzustellen, wird das neue Laufwerk ebenfalls unmountbar. Reparieren Sie immer erst das Dateisystem, bevor Sie versuchen, einen Wiederherstellungsprozess durchzuführen.
  :::
</details>
...
3. **Deaktiviertes Laufwerk nicht zuweisen:** Gehen Sie zu ***Main → Array-Geräte*** und ändern Sie den Slot des deaktivierten Laufwerks auf "Nicht zugewiesen".
...
Zur Bequemlichkeit können Sie diese Überprüfungen so planen, dass sie automatisch in Intervallen ausgeführt werden, die Ihnen passen, indem Sie ***Einstellungen → Zeitplan*** navigieren. Es wird empfohlen, monatliche oder vierteljährliche automatische, korrigierende Prüfungen durchzuführen, um die fortwährende Datenintegrität sicherzustellen und vorhandene Synchronisierungsfehler zu beheben.
...
- **Disk-Neuanordnung:** Die Änderung der Laufwerksreihenfolge hat keine Auswirkungen auf Parität1, kann aber Parität2 ungültig machen.
...
\*_Tatsächliche NVMe-Geschwindigkeiten hängen von der PCIe-Generation, Kühlung und der Netzwerkbandbreite ab (z. B. 10GbE ist auf ca. 1.100 MB/s begrenzt).
...
Wenn Ihr Array nicht startet, befolgen Sie diese Schritte, um häufige Probleme zu identifizieren und zu beheben. Suchen Sie nach Fehlermeldungen unter ***Main → Array Operation***.

...
Wenn Sie Dienste wie Docker oder %%VM|vm%% Manager starten, erstellt Unraid automatisch ein [Set von Standard-Benutzerfreigaben](#default-shares), die darauf ausgelegt sind, gängige Plugins, Docker-Container und %%virtuelle Maschinen (VMs)|vm%% zu unterstützen. Sie können bei Bedarf bequem zusätzliche Freigaben für Ihre persönlichen Daten erstellen.
...
#### Eine Freigabe mit Daten löschen
...
1. Die Freigabe leeren:
   ...
   - **Verwenden Sie den Netzwerkzugriff**: Verbinden Sie sich über **Windows Explorer** (%%SMB|samba%%) oder **macOS Finder** (%%AFP|AFP%%/%%SMB|samba%%) und löschen Sie die Dateien manuell.

   :::vorsicht

   Der `rm -rf` Befehl wird Dateien dauerhaft löschen. Überprüfen Sie genau, bevor Sie ihn verwenden. Stellen Sie sicher, dass keine Docker- oder %%VM|vm%% Dienste die Freigabe verwenden, bevor Sie sie löschen.
...
:::note
High-water ist die Standardmethode und funktioniert gut für die meisten Benutzer. Sie reduziert das ständige Hin- und Her-Navigieren auf den Laufwerken im Vergleich zu "Most-free".
:::

...
- Mit laufendem Array wählen Sie auf **_Main → Poolgeräte_** den Poolnamen aus, um die Details anzuzeigen.
...
- Array stoppen
- Auf **_Main → Poolgeräte_** fügen Sie dem Pool einen Slot hinzu.
...
Anwendungsfall: Angenommen, Sie sind ein Inhaltsersteller mit einer Kiste voller Festplatten, die alle Ihre historischen Videos enthalten. Beim Erstellen eines Arrays (oder nach dem Ausführen von **_Tools → Neue Konfiguration_**) fügen Sie alle Ihre vorhandenen Datenlaufwerke (leer oder mit Daten in einem unterstützten Laufwerksformat) dem Array hinzu. Eventuelle Paritätslaufwerke werden überschrieben, aber die Datenlaufwerke behalten ihre Daten. Sie können den Paritätsschutz genießen, sie im Netzwerk freigeben und alle Vorteile von Unraid nutzen.
...
Die **_Main_**-Seite wird jetzt warnen, wenn Array- oder Poollaufwerke mit ReiserFS formatiert sind; diese Laufwerke müssen so schnell wie möglich auf ein anderes Dateisystem migriert werden, da sie in einer zukünftigen Version von Unraid (wahrscheinlich Unraid 7.3) nicht mehr verwendbar sein werden. Ebenso wird gewarnt, wenn Laufwerke in einer veralteten Version von XFS formatiert sind; diese müssen vor 2030 migriert werden. Siehe [Umstellung auf einen neuen Dateisystemtyp](../using-unraid-to/manage-storage/file-systems.mdx#converting-to-a-new-file-system-type) in den Dokumenten für Details.
...
- Verbesserung: Ein oberstes `lost+found`-Verzeichnis wird nicht freigegeben.
- Korrektur: In smb.conf `smb3 directory leases = no` setzen, um Probleme mit der aktuellen Samba-Version zu vermeiden.
- Korrektur: Kommentare in den Standarddateien `/etc/modprobe.d/*.conf` wiederherstellen.
- Korrektur: Windows kann keinen neuen Ordner für einen Share mit primärem=ZFS-Pool und sekundärem=EXT4-Array-Disk erstellen.
...
- Fix: Die Seite **_Einstellungen → Netzwerkeinstellungen → Schnittstellenregeln_** zeigte manchmal den falschen Netzwerktreiber an (es war nur ein Anzeigeproblem).
...
- dynamix.unraid.net 4.25.3 - [Änderungen ansehen](https://github.com/unraid/api/releases).
...
- Feature: Hinzufügen einer _Force Install_-Schaltfläche, um Versionsprüfungen beim manuellen Installieren von Plugins zu umgehen.
- Feature: Hinzufügen der Seite **_Tools → Terminal öffnen_**; kann über die Suche nach "Terminal" darauf zugegriffen werden. Optionale Entfernung der Terminal-Schaltfläche aus der Symbolleiste über **_Einstellungen → Anzeigeeinstellungen → Terminal-Schaltfläche in Kopfzeile anzeigen_**.
- Feature: **_Benutzer → Root → SSH autorisierte Schlüssel_** unterstützt nun mehr Formate (danke [wandercone](https://github.com/wandercone)).
...
- Fix: Verhindern, dass Firefox beim Starten des Arrays das Popup zum Erneut senden/Abbrechen anzeigt (danke an [dkaser](https://github.com/dkaser))
...
- Fix: Auf _**Einstellungen → Anzeigeeinstellungen → Schriftgröße**_ extreme Optionen entfernen, die das WebGUI beeinträchtigen.
...
- Fix: Verhindern Sie Fehler im Zusammenhang mit _searchLink_, wenn Plugins installiert werden.
- Fix: PHP-Warnungen beim Importieren von WireGuard-Tunneln.
- Verbesserung: Zeitzone _Europe/Kiev_ in _Europe/Kyiv_ umbenannt, um sich an die IANA-Zeitzonendatenbank anzupassen.
- Verbesserung: Discord-Benachrichtigungsagent verbessern; Agent aktivieren/deaktivieren, um Updates zu erhalten (Dank [mgutt](https://github.com/mgutt)).
...
- Fix: Docker NAT-Fehler aufgrund fehlendem br\_netfilter.
...
- Version 6.12.54-Unraid
  ...
  - eMMC-Unterstützung hinzugefügt:
    - CONFIG_MMC: Unterstützung für MMC/SD/SDIO-Karte
    - CONFIG_MMC_BLOCK: MMC-Blockgerätetreiber
    - CONFIG_MMC_SDHCI: Unterstützung für SD-Hostcontroller-Schnittstelle
    - CONFIG_MMC_SDHCI_PCI: SDHCI-Unterstützung auf dem PCI-Bus
    - CONFIG_MMC_SDHCI_ACPI: SDHCI-Unterstützung für ACPI-enumerierte SDHCI-Controller
    - CONFIG_MMC_SDHCI_PLTFM: SDHCI-Plattform und OF-Treiberhilfe
...
- perl: Version 5.42.0
- php: Version 8.3.26-x86_64-1_LT mit gettext-Extension

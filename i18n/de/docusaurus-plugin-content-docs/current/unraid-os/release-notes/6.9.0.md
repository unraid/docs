...
Mehrere Gerätepools werden standardmäßig immer noch mit dem btrfs `raid1`-Profil erstellt. Wenn Sie 3 oder mehr Geräte in einem Pool haben, können Sie nun auf das `raid1c3`-Profil (x3 Kopien der Daten auf separaten Geräten) umverteilen. Wenn Sie 4 oder mehr Geräte in einem Pool haben, können Sie nun auf `raid1c4` (x4 Kopien der Daten auf separaten Geräten) umverteilen. Wir haben auch die `raid6`-Balance-Operation geändert, um Meta-Daten auf `raid1c3` zu setzen (zuvor war es `raid1`).
...
Dank an [@Squid](https://forums.unraid.net/profile/10290-squid/): Die Sprachpakete werden über das Community Applications-Plugin installiert. Suchen Sie nach der Kategorie **Sprache**.

- Hinweis: Community-Apps müssen auf dem neuesten Stand sein, um Sprachen zu installieren. Weitere Details finden Sie [hier](https://forums.unraid.net/topic/38582-plug-in-community-applications/page/124/?tab=comments#comment-866710).
...
*Mehr über den Mehrsprachensupport erfahren Sie in [diesem Leitfaden](https://unraid.net/blog/unraid-6-9-multi-language-support).*
...
- iSCSI-Kernel-Unterstützung hinzugefügt:
   - CONFIG_ISCSI_TARGET: iSCSI-Target-Modus-Stack von Linux-iSCSI.org
  - CONFIG_ISCSI_TARGET_CXGB4: Chelsio iSCSI-Target-Offload-Treiber
  - CONFIG_LOOPBACK_TARGET: TCM Virtuelles SAS-Target und Linux/SCSI LDD
  ...
  - CONFIG_TARGET_CORE: Generischer Target-Core-Mod und ConfigFS
    Infrastruktur

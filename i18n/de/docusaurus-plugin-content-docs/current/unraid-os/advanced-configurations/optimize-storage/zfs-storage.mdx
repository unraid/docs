...
- **Stripe** ist nur für nicht-kritische, temporäre Daten geeignet - fällt eine Festplatte aus, verlieren Sie alles.
  :::
...
%%ZFS|zfs%% bietet erweiterte Funktionen, die die Speicher-Effizienz und Leistung von Unraid erheblich verbessern können. Zwei häufige Themen von Interesse sind Komprimierung und Speicheranforderungen.
...
<details>
  ...
  Vielleicht sind Sie auf den veralteten Rat gestoßen: "%%ZFS|zfs%% benötigt 1 GB RAM pro 1 TB Speicher." Dies gilt für die meisten Benutzer nicht mehr. %%ZFS|zfs%% verwendet RAM für seinen Adaptive Replacement Cache (ARC), der häufig aufgerufene Lesevorgänge beschleunigt.

  Unraid begrenzt %%ZFS|zfs%% automatisch darauf, einen angemessenen Anteil des RAM Ihres Systems zu verwenden (normalerweise ein Achtel des gesamten RAMs). Dies erlaubt %%ZFS|zfs%%, gut zu funktionieren, ohne Docker-Container, %%VMs|vm%% oder das Unraid Betriebssystem zu beeinträchtigen.
...
<details>
  ...
  Unraid erkennt und importiert den %%ZFS|zfs%% Pool automatisch. Unterstützungs-vdevs (wie Log, Cache/L2ARC, Sonder/dedup) sind unter **Subpools** im %%WebGUI|web-gui%% aufgeführt. Es ist nicht nötig, Subpools separat nach dem Import hinzuzufügen. Unraid importiert sie automatisch zusammen mit den Hauptdaten-Festplatten, wenn alle erforderlichen Laufwerke zugewiesen sind.
  :::
...
| Log vdev (SLOG)                  | Verbessert die Synchronisierungs-Schreibleistung | Optional. Nur für bestimmte Arbeitslasten von Nutzen.                                                                      |
| Cache vdev (L2ARC)               | Bietet SSD-basierten Lese-Cache                  | Optional. Kann die Lesegeschwindigkeit für große Arbeitssätze verbessern.                                                  |
...
| Cache (L2ARC) vdev           | Pool wird importiert, aber der Lese-Cache geht verloren                    | Der Pool funktioniert normalerweise, aber Sie verlieren den Leistungsvorteil des L2ARC Caches. Es gehen keine Daten verloren.                                |

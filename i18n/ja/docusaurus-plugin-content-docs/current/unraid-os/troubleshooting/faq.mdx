...
### ビルドまたはシステム構成に関してヘルプが必要です。どこへ行けばよいですか？ {/* #build-help */}
...
### Unraid に推奨されるコントローラーはどれですか？ {/* #hardware-raid-support */}
...
### 内蔵コントローラーがいっぱいの場合、ストレージを追加する最適な方法は何ですか？ {/* #add-more-storage */}
...
### Unraid には、書き込みを防ぐためにドライブ上の不良セクタを記憶する割り当て機能はありますか？ {/* #bad-sector-allocation */}
...
### ブートデバイスが故障し、バックアップがありません。構成を復元するにはどうすればよいですか？ {/* #usb-failed-restore */}
...
### root パスワードを忘れてしまった場合はどうすればよいですか？ {/* #forgot-root-password */}
...
### Unraid OS を最初から完全に再セットアップするにはどうすればよいですか？（工場出荷時リセット手順） {/* #factory-reset */}

1. 保持したいデータは必ずバックアップしてください。
2. %%array|array%% を停止し、サーバーをシャットダウンしてください。
3. ブートデバイスを取り外します。
4. Unraid USB Flash Creator ツールを使用して、ドライブを再フォーマットし、Unraid を再インストールします。
5. ブートデバイスを再挿入し、サーバーを起動します。
6. %%WebGUI|web-gui%% でターミナルを開き、`lsblk` を実行してすべてのドライブを一覧表示します（ブートデバイスは除く）。
...
### サーバーのホスト名はどうやって変更しますか？ {/* #change-hostname */}
...
### AMD GPU を割り当てた Windows VM をシャットダウンするたびに、再起動に失敗します。どうすればよいですか？ {/* #amd-gpu-vm-restart */}

多くの AMD GPU では機能レベルのリセットに問題があり、これがこの問題の原因となっています。回避策としては、シャットダウン前に Windows 内から GPU を取り外すか、通常この問題を回避できる NVIDIA GPU を使用します。

<br />

### CPU に統合グラフィックスがない場合、プライマリ GPU を VM にパススルーするにはどうすればよいですか？ {/* #primary-gpu-passthrough */}

これは実現可能ですが、追加の手順が必要です。設定ガイダンスについては、[仮想マシンの GPU パススルー](../using-unraid-to/create-virtual-machines/vm-setup.mdx) を参照してください。
...
### Unraid は RAID1/5/6/10 などのさまざまな RAID タイプをサポートしていますか？ {/* #raid-types-support */}
...
- 従来型のハードウェア %%RAID|raid%% カードは不要です。Unraid にドライブを直接管理させるために、シンプルな HBA を使用できます。

<br />

### 現在、MBR 形式のパーティションテーブルでフォーマットされたデバイスのアレイがありますが、GPT に変換したいです。どうすればよいですか？ {/* #mbr-to-gpt-conversion */}
...
1. 有効な %%parity|parity%% と、ブートデバイスの最新バックアップがあることを確認してください。
...
- Unraid OS 6.9 以降では、パーティション形式に関係なく、パーティション 1 は回転型デバイスで 32KiB、非回転型デバイスで 1MiB から始まります。
  \-- この変換プロセスを開始する前に、必ず [ブートデバイスをバックアップ](/unraid-os/system-administration/secure-your-server/secure-your-boot-drive/#backups) してください。
...
### `http://tower` または `http://tower.local` で WebGUI に接続できないようです。どうすればよいですか？ {/* #webgui-connection */}
...
- USB Flash Creator でブートメディアを作成する際に静的 IP を割り当てた場合は、そのアドレスを使用してください。
- サーバーにモニターとキーボードを接続すると、起動後にローカルコンソールに IP アドレスが表示されます。

**この問題の一般的な原因:**

- 使用しているコンピューターとサーバーが異なるサブネットまたは VLAN にある可能性があります。
- ルーターがローカルホスト名の名前解決に対応していない可能性があります。
...
### USB Flash Creator で Unraid を起動メディアにインストールできません。どうすればよいですか？ {/* #usb-creator-issue */}

USB Flash Creator がシステムまたは USB ドライブで動作しない場合は、[手動インストール方法](../getting-started/set-up-unraid/create-your-bootable-media.mdx#manual-install-method) を使って Unraid のブートデバイスを準備できます。この方法は Windows、macOS、Debian/Ubuntu Linux に対応しています。

<br />

### UEFI を使って起動するようにシステムを設定する必要があります。どうすればよいですか？ {/* #uefi-boot-config */}

%%UEFI|uefi%% ブートモードは、いくつかの方法で設定できます:

#### インストール

USB Flash Creator を使用する場合は、Unraid を USB ドライブに書き込む前に、%%UEFI|uefi%% ブートモードを有効にするオプションを選択してください。

#### オプション 2: レガシーモードで起動した後

%%WebGUI|web-gui%% で、***設定 → ブートデバイス*** を開きます（Unraid **7.3.0** 以降）。それ以前のリリースでは、同じページは ***設定 → Flash device*** にあります。

%%UEFI|uefi%% ブートモードを有効にして、サーバーを再起動してください。

#### オプション 3: フォルダーを手動で名前変更

ブートボリューム（USB ドライブ）上で、`EFI~` フォルダーの名前を `EFI` に変更します（末尾のチルダ `~` を削除します）。

%%UEFI|uefi%% ブートモードは、いくつかの方法で設定できます:

Unraid のブートデバイス（該当する場合は USB）を最優先の起動対象として設定し、%%UEFI|uefi%% ブートモードを有効にします（利用可能であれば、CSM/Legacy/互換モードは必ず無効にしてください）。

<br />

### WebGUI を使用する際に Web ブラウザーで問題が発生しています。どうすればよいですか？ {/* #webgui-browser-issues */}
...
### Unraid のトライアルを延長するにはどうすればよいですか？ {/* #extend-trial */}
...
:::important
トライアルを継続するには、同じブートデバイスを使用する必要があります。ブートメディアを変更すると、最初から新しいトライアルを開始する必要があります。
:::

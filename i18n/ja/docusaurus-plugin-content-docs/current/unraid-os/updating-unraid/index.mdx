...
古い Unraid バージョンで作成した %%OVMF|ovmf%% ベースの %%VMs|vm%% では、EFI シェルが表示されることがあります。次のコマンドを入力すると %%VM|vm%% を起動できます:

1. `fs0:` と入力します。
2. 次に `cd efi/boot` と入力します。
3. 最後に `bootx64.efi` と入力します。

`fs0:` で動作しない場合は、代わりに `fs1:` を試してください。問題が続く場合は、[Unraid フォーラム](https://forums.unraid.net/) でサポートを求めてください。

...
Dockerコンテナを使うと、%%virtual machines|vm%%に伴うオーバーヘッドなしに、隔離された軽量な環境でアプリケーションを実行できます。Unraidは、[Docker Hub](https://hub.docker.com/)と[Community Applications](/community-applications/)を通じて、事前設定済みの数千ものアプリにアクセスできるようにすることで、Dockerの利用を簡単にします。このコンテナ化アプローチにより、Unraidユーザーは互換性の問題なしに複数のアプリケーションを同時に実行できます。また、自己完結型のアプリケーションパッケージを使用することでシステムを整理しやすくし、Dockerのエコシステムを通じてサーバー機能を簡単に拡張できます。

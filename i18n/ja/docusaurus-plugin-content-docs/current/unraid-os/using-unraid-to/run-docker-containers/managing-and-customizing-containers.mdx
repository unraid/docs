...
Unraid で Docker コンテナをカスタマイズする前に、基本的な設定オプションを理解しておくと便利です。各 [コンテナテンプレート](/community-applications/) には、ネットワーク、ストレージ、環境変数を設定するための使いやすいインターフェースが用意されており、複雑なコマンドラインツールを使わずにコンテナの動作をニーズに合わせて調整できます。

...
1. **VMs** タブで Unraid の %%VM|vm%% アイコンをクリックし、**Start with console (%%VNC|vnc-session%%)** を選択します。
2. %%VM|vm%% の **%%WebGUI|web-gui%%** に `http://[VM-IP]` でアクセスします。
3. ***Settings → Identification*** に移動します:
   - 一意の **Server name** を設定します（例: `Unraid-VM`）。
   - "Development instance." のような説明を追加します。
4. （任意）***Settings → Display settings*** に移動し、ホストと区別できるよう別のカラーテーマを選択します。
5. ***Settings → %%SMB|samba%% settings → Workgroup settings*** に移動し、競合を避けるため **Local master** を *No* に設定します。
6. UPS パススルーを行う場合（ホストに UPS がある場合）:
...
7. 設定したデバイスで %%array|array%% を起動します。
8. プラグイン/Docker のテスト用に [**Community Applications**](https://unraid.net/community/apps) をインストールします。
9. 物理サーバーと同じように、***Tools → Update OS*** から %%VM|vm%% を更新します。

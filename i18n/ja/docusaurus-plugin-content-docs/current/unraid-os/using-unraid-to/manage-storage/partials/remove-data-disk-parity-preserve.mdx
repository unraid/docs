...
<details>
  ...
  ディスクのゼロ化には数時間かかることがあるため、%%SSH|ssh%% 経由の `tmux` や [Community Applications](/community-applications/) からインストールしたターミナルプラグインなど、永続的なターミナルセッションを使用してください。プラグインをインストールする前に、メンテナー、テンプレート、サポート情報を確認してください。

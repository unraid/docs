...
このページでは、**Unraid Connect リモートアクセス**について説明します。これは、%%UPnP|upnp%% またはルーター上の手動ポート転送ルールを使用して、%%WebGUI|web-gui%% をインターネットに公開します。%%WebGUI|web-gui%% を WAN トラフィックに公開せずに完全なリモート管理を行いたい場合は、公式の [Tailscale 統合](../unraid-os/system-administration/secure-your-server/tailscale.mdx) を推奨します。Docker コンテナへの接続やネットワークドライブへのアクセスなど、より高度な要件がある場合は、%%VPN Tunnel|vpn-tunnel%% が引き続き推奨ソリューションです。

:::important[Security 注意]

リモートアクセスを有効にする前に、root パスワードが強力で一意であることを確認してください。必要に応じて、**ユーザー** ページで更新してください。さらに、セキュリティ上の脆弱性から保護するため、Unraid OS を最新バージョンに更新しておいてください。[ここで Unraid の更新について詳しく見る](/unraid-os/updating-unraid/)
...
ほとんどのユーザーにとって、公式の [Tailscale 統合](../unraid-os/system-administration/secure-your-server/tailscale.mdx) のほうが、%%WebGUI|web-gui%% を公開インターネットにさらさずにリモート管理する方法として適しています。Unraid Connect リモートアクセスは、公開される %%WebGUI|web-gui%% URL にブラウザーでアクセスする必要がある場合にのみ使用してください。
...
動的なオンデマンドのリモートアクセスは、現在のところ Unraid API の設定ページには表示されません。ポート転送を介して %%WebGUI|web-gui%% を公開し続けずにリモート管理が必要な場合は、[Tailscale](../unraid-os/system-administration/secure-your-server/tailscale.mdx) または別の %%VPN Tunnel|vpn-tunnel%% を使用してください。

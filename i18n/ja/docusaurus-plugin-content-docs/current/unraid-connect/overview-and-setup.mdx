...
Unraid Connect は、Unraid OS 6.10 以降を必要とするプラグインとして提供されています。開始する前に、サーバーがインターネットに接続されていること、および [Community Applications](/community-applications/) プラグインがインストールされていることを確認してください。
...
- Unraid %%WebGUI|web-gui%% の **Apps** タブに移動します。
- **Unraid Connect** を検索してプラグインのインストールに進みます。ダイアログを閉じる前に、インストールが完全に完了するまで待ってください。
- Unraid %%WebGUI|web-gui%% の右上隅で Unraid ロゴをクリックし、**Sign In** を選択します。
...
フルリモート管理には、公式の[Tailscale統合](../unraid-os/system-administration/secure-your-server/tailscale.mdx)を推奨します。これにより、公開インターネットに公開することなく、%%WebGUI|web-gui%%とサービスへプライベートにリモートアクセスできます。

...
| %%WebGUI\|web-gui%% no disponible        | Access via **%%SSH\|ssh%%**, telnet, or direct console to run the `diagnostics` command. The zip file saves to `/boot/logs`.                     | Siempre captura diagnósticos antes de reiniciar para mantener los registros intactos.                                                        |
...
| **Remote %%syslog\|syslog%%** | Los registros se almacenan en otro dispositivo                                                   | Necesita un servidor separado siempre encendido | Monitoreo a largo plazo (semanas a meses)   |
| **Local %%syslog\|syslog%%**  | Mantiene registros en el %%array\|array%% o %%cache\|cache%%, reduciendo el desgaste en el flash | Menos accesible si hay un fallo del sistema     | Registro continuo sin dispositivos externos |

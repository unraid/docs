...
| **%%syslog\|syslog%% local**  | Keeps logs on the %%array\|array%% or %%cache\|cache%%, reducing wear on flash | Less accessible if there's a system crash       | Continuous logging without external devices |

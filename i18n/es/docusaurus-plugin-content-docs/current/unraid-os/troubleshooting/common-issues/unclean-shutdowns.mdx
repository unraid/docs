...
- **Configura los tiempos de espera de apagado:** Establezca el UPS para que active un apagado controlado antes de que la batería se agote. Ajuste los umbrales de "Tiempo de ejecución restante de la batería" o "Nivel de carga de la batería" para proporcionar suficiente tiempo para que Unraid [detenga el array](../../using-unraid-to/manage-storage/array/overview.mdx#startstop-the-array) y se apague de forma segura.
...
<Tabs>
  ...
  <TabItem value="aparato" label="VMs de electrodomésticos">
    ...
    Las %%VMs|vm%% de appliance están diseñadas para ejecutar software específico y, a menudo, no permiten la instalación de paquetes adicionales, como %%QEMU|qemu%% %%Guest Agent|guest-agent%%. Esto significa que la hibernación no está disponible, por lo que deberá confiar en una configuración adecuada del tiempo de espera.

...
- **Habilitar registro persistente:** Vaya a ***Configuración → Servidor Syslog*** para activar el registro que persiste después de un reinicio. Vea [Registros persistentes (Servidor Syslog)](../diagnostics/capture-diagnostics-and-logs.mdx#persistent-logs-syslog-server) para más detalles.
...
- **Habilitar soporte para UPS** en ***Configuración → Configuraciones de UPS***.
...
<Tabs>
  ...
  <TabItem value="aparato" label="VMs de electrodomésticos">
    ...
    :::nota[Por qué las VMs de electrodomésticos son diferentes]

    Las %%VMs|vm%% de electrodomésticos están diseñadas para ejecutar software específico y a menudo no permiten la instalación de paquetes adicionales, como el %%QEMU|qemu%% %%Guest Agent|guest-agent%%. Esto significa que la hibernación no está disponible, por lo que deberá confiar en una configuración de tiempo de espera adecuada.
...
Si está experimentando apagados no limpios o contenedores que se bloquean durante el apagado, considere aumentar el tiempo de espera general de apagado a **180 segundos** (o **300+ segundos** si tiene múltiples %%VMs|vm%%). Esto le da a los servicios más tiempo para apagarse correctamente.
:::

...
- **Revisiones de salud regulares:** Mantén un ojo en los [datos SMART](../../system-administration/monitor-performance/smart-reports-and-disk-health.mdx) de tu unidad y realiza revisiones periódicas del sistema de archivos.
  :::
...
- La unidad de paridad **no** tiene un sistema de archivos. Ejecutar cualquier herramienta de reparación en la unidad de paridad puede corromperla y podría resultar en pérdida de datos irreversible.
  :::
...
| Partición de %%array\|matriz%% | /dev/md7p1  | Dispositivo gestionado por Unraid | Yes                                   |
...
- Utilizar la partición cruda (por ejemplo, `/dev/sdj1`) no actualizará %%parity|parity%%, dejándola inválida.
  :::
...
| ------------------- | --------------------- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| %%XFS\|xfs%%        | Modo de Mantenimiento | Yes                                 | La %%array\|matriz%% debe iniciarse en Modo de Mantenimiento (unidades no montadas) para verificación/reparación. |
...
- **Copiando bloques no intentados...**: ddrescue está haciendo su primer intento de leer todos los sectores.
  :::
...
- **áreas dañadas/errores de lectura**: Estos números pueden aumentar a medida que ddrescue encuentra más daños, pero el objetivo es minimizar la pérdida irrecuperable de datos.
  :::

...
Un archivo de imagen de Docker corrupto (`docker.img`) es un problema común, a menudo causado por el agotamiento del %%cache pool|cache-pool%% o un apagado inadecuado. Afortunadamente, un contenedor bien configurado almacena todos los datos variables fuera del archivo de imagen (usualmente en la compartición `appdata`), por lo que puede recrear la imagen de forma segura sin perder la configuración de su aplicación o datos.

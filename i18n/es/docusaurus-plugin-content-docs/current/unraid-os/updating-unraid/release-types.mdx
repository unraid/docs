...
## Series menores compatibles y fin de vida (EOL)

Unraid admite dos series menores públicas a la vez:

- La **serie menor pública actual**, como `7.3.x` una vez que cualquier versión beta, RC o estable de `7.3` sea pública.
- La **serie menor pública anterior**, como `7.2.x` mientras `7.3.x` es la serie pública actual.

Una beta pública o RC cuenta como una serie menor pública a efectos de la política de soporte. Esto no significa que las versiones beta o RC se recomienden para uso en producción; solo significa que la ventana de soporte avanza cuando la siguiente serie menor se hace pública.

Dentro de una serie menor compatible, usa la compilación de parche o de prepublicación más reciente disponible para esa serie. Las compilaciones de parche anteriores de la misma serie quedan reemplazadas y siguen disponibles solo para archivo, reversión y referencia en las notas de la versión.

Las series menores anteriores están **al final de su vida útil (EOL)**. Las versiones EOL no tienen soporte y ya no incluyen correcciones de errores ni actualizaciones de seguridad.

Para comprobar el estado actual de una versión, consulta el [Archivo de versiones](../download_list.mdx).
...
- Las versiones beta públicas y RC cuentan al identificar la serie menor pública actual.

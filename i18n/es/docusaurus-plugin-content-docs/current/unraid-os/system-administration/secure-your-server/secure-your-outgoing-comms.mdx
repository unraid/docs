...
### Configuración de un servidor proxy {/* #setting-up-a-proxy-server */}
...
- **Si necesitas configurar el tuyo propio:**
  - Busca un contenedor de proxy HTTP en [Community Applications](/community-applications/) y revisa la plantilla, el mantenedor y los detalles de soporte antes de instalarlo.
...
## Conexiones VPN salientes {/* #outgoing-vpn-connections */}
...
1. Es posible que necesites desactivar el túnel temporalmente para actualizaciones de Unraid o instalaciones de complementos.
2. Solo un túnel del sistema puede estar activo a la vez.
3. Unraid ignora las configuraciones de DNS del archivo importado. Establece el DNS de Unraid en un servidor público confiable (p. ej., `8.8.8.8`, `8.8.4.4`).
   :::
...
- **[WireGuard VPN Tunneled Access](wireguard.mdx)** - Esta discusión comunitaria se centra en enrutar contenedores Docker o tráfico del sistema a través de proveedores comerciales de VPN WireGuard. Incluye ejemplos de configuración en el mundo real, consejos para probar filtraciones de DNS y recomendaciones para solucionar problemas de túneles salientes.
- **[Hilo de Plug-in de WireGuard de Dynamix](wireguard.mdx)** - Este es el hilo oficial para el plugin de WireGuard de Dynamix. Cubre actualizaciones del plugin, solicitudes de funciones, informes de errores y preguntas y respuestas generales relacionadas con el soporte integrado de WireGuard en Unraid.
- **[Inicio Rápido de WireGuard](wireguard.mdx)** - Una guía paso a paso para configurar conexiones VPN WireGuard entrantes en Unraid. Este recurso incluye explicaciones de configuración, instrucciones para configurar pares y consejos para el acceso remoto.

...
3. Abra ***Configuración → Tailscale*** y haga clic en Reautenticar. Inicie sesión con su cuenta de %%Tailscale|tailscale%%.
...
6. En ***Configuración → Tailscale***, encuentre el nombre y la IP de %%Tailnet|tailnet%% de su servidor. Utilícelos para acceder a compartir SMB/NFS, contenedores Docker, y más desde cualquier dispositivo en su %%Tailnet|tailnet%%.
...
| -------------------- | --------------------------- | ------------------ | --------------------------------------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `host`               | Desactivado                 | No                 | No                                                  | Yes                                   | La integración %%Tailscale\|tailscale%% no está disponible en modo host                                  |
| `puente`             | Desactivado                 | Yes                | Yes                                                 | *Habilitado*: Sí  *Deshabilitado*: No | Habilita ambas URLs si está habilitado; solo interfaz web %%Tailscale\|tailscale%% si está deshabilitado |
| `eth0`/`br0`/`bond0` | Desactivado                 | Yes                | Yes                                                 | Yes                                   | Ambas URLs son accesibles independientemente de la configuración                                         |
| `container`/`wg0`    | Deshabilitado (no probado)  | Yes                | Desconocido                                         | Desconocido                           | Usar con precaución; no probado completamente                                                            |

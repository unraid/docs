...
6. En ***Ajustes → Tailscale***, encuentra el nombre y la IP de Tu %%Tailnet|tailnet%% de tu servidor. Utiliza estos para acceder a los recursos SMB/NFS, los contenedores de Docker, y más desde cualquier dispositivo en tu %%Tailnet|tailnet%%.
   \+7. (Opcional) Habilita **Incluir compañeros Tailscale en /etc/hosts** (ver [Resolución de nombres de host mejorada](#enhanced-hostname-resolution)) para añadir automáticamente dispositivos Tailscale a la resolución de nombres de host de tu servidor, facilitando el acceso a otros dispositivos en tu %%Tailnet|tailnet%%.
...
El complemento de Tailscale incluye una opción para añadir automáticamente compañeros Tailscale al archivo `/etc/hosts` de tu servidor. Esta función simplifica el acceso a los dispositivos al habilitar conexiones basadas en nombres de host a otros dispositivos en tu %%Tailnet|tailnet%%.

Los beneficios incluyen:

- Acceder a otros dispositivos Tailscale utilizando nombres de host amigables en lugar de direcciones IP.
- Actualizaciones automáticas cuando los dispositivos se unen o abandonan tu %%Tailnet|tailnet%%.

...
- **Características de %%ZFS|zfs%%:** Puede utilizar %%snapshots|instantáneas%% y replicación de %%ZFS|zfs%% en este disco, lo que lo hace ideal para objetivos de respaldo, conjuntos de datos específicos o escenarios donde desea características de %%ZFS|zfs%% junto con el almacenamiento tradicional de Unraid.
...
- **Stripe** solo es adecuado para datos no críticos y temporales; si falla algún disco, lo pierdes todo.
...
<details>
  ...
  Podrías haber encontrado el consejo obsoleto: “%%ZFS|zfs%% requiere 1 GB de RAM por 1 TB de almacenamiento.” Esto ya no es aplicable para la mayoría de los usuarios. %%ZFS|zfs%% utiliza RAM para su Caché de Reemplazo Adaptativo (ARC), lo que acelera las lecturas frecuentemente accedidas.

  Unraid automáticamente limita %%ZFS|zfs%% a usar una porción razonable de la RAM de su sistema (generalmente 1/8 del total de RAM). Esto permite que %%ZFS|zfs%% funcione bien sin afectar los contenedores de Docker, %%VMs|vm%%, o el sistema operativo de Unraid.
...
| Vdev de cache (L2ARC)            | El pool se importa, pero se pierde el cache de lectura                   | El pool funciona normalmente, pero pierde el impulso de rendimiento de la caché L2ARC. No se pierde ningún dato.                      |
...
| Objetivo de respaldo o replicación diaria  | Utilice un disco %%ZFS\|zfs%% (incluso dentro del %%array\|matriz%% de Unraid) como un objetivo de replicación. Puede replicar otros pools localmente o desde otro servidor Unraid.                                   | Utiliza `zfs send/receive` o herramientas como Syncoid para respaldos y restauraciones rápidas y confiables.                   |
| %%Snapshot\|snapshot%%-based recovery pool | Keep point-in-time %%snapshot\|snapshot%%s of critical data or containers. %%snapshot\|snapshot%%s can be auto-scheduled and are space-efficient.                                                                     | Esta función permite una rápida recuperación de eliminaciones accidentales o errores de configuración.                         |

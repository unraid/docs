...
- Con la matriz en ejecución, en **_Principal → Dispositivos de grupo_**, seleccione el nombre del grupo para ver los detalles
...
- Detener el array
- En **_Principal → Dispositivos de grupo_**, agregue una ranura al grupo
...
Caso de uso: suponga que eres creador de contenido con una caja llena de discos duros que contienen todos tus videos históricos. Al crear una matriz por primera vez (o después de ejecutar **_Herramientas → Nueva Configuración_**), agrega todas tus unidades de datos existentes (vacías o con datos en un formato de unidad compatible) a la matriz. Cualquier unidad de paridad será sobrescrita pero las unidades de datos conservarán sus datos. Puedes disfrutar de protección de paridad, compartirlas en la red y aprovechar al máximo todo lo que Unraid tiene para ofrecer.
...
La página **_Principal_** ahora avisará si alguna unidad de la matriz o del grupo está formateada con ReiserFS; estas unidades necesitan ser migradas a otro sistema de archivos lo antes posible ya que no serán utilizables en una futura versión de Unraid (probablemente Unraid 7.3). De manera similar, avisará si hay unidades formateadas en una versión obsoleta de XFS; estas deben ser migradas antes de 2030. Consulte [Conversión a un nuevo tipo de sistema de archivos](../using-unraid-to/manage-storage/file-systems.mdx#converting-to-a-new-file-system-type) en la documentación para más detalles.
...
- Corrección: La página **_Configuración → Configuración de Red → Reglas de Interfaz_** a veces mostraba el controlador de red incorrecto (era solo un problema de visualización)
...
- Función: Agregar botón _Forzar Instalación_ para evitar verificaciones de versión al instalar manualmente complementos
- Función: Agregar la página **_Herramientas → Abrir Terminal_**; se puede acceder buscando "terminal". Opcionalmente, se puede quitar el botón Terminal de la barra de herramientas a través de **_Configuración → Configuración de Pantalla → Mostrar Botón Terminal en el encabezado_**
- Función: **_Usuarios → Root → Claves autorizadas SSH_** ahora admite más formatos (gracias [wandercone](https://github.com/wandercone))
...
- Corrección: Prevenir errores relacionados con _searchLink_ al instalar complementos
- Solución: Advertencias de PHP al importar túneles WireGuard
- Mejora: zona horaria _Europa/Kiev_ renombrada a _Europa/Kyiv_ para alinearse con la Base de Datos de Zonas Horarias de IANA
...
- versión 6.12.54-Unraid
  ...
  - Añadido soporte para eMMC:
    - CONFIG_MMC: soporte para tarjetas MMC/SD/SDIO
    - CONFIG_MMC_BLOCK: controlador de dispositivos de bloque MMC
    - CONFIG_MMC_SDHCI: soporte para la Interfaz de Controlador de Anfitrión Digital Seguro
    - CONFIG_MMC_SDHCI_PCI: soporte SDHCI en bus PCI
    - CONFIG_MMC_SDHCI_ACPI: soporte SDHCI para controladores SDHCI enumerados por ACPI
    - CONFIG_MMC_SDHCI_PLTFM: controlador helper de plataforma SDHCI y OF

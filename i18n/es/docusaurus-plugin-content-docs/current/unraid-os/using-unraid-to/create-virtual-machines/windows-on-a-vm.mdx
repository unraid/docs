...
El indexado de búsqueda de Windows escanea continuamente el almacenamiento de su %%VM|vm%% para catalogar archivos y obtener resultados de búsqueda más rápidos. Sin embargo, en una %%VM|vm%%, esto puede causar un I/O de disco innecesario, ralentizar el rendimiento y aumentar el desgaste en su almacenamiento físico, especialmente en los SSD en su [%%cache pool|cache-pool%%](../manage-storage/cache-pools.mdx).
...
Si tiene problemas con el audio HDMI en una %%VM|vm%% de Windows que utiliza %%GPU passthrough|gpu-passthrough%% (lo que a menudo ocurre con las tarjetas gráficas NVIDIA), habilitar las interrupciones de mensajes señalados (MSI) puede ayudar. MSI mejora la gestión de interrupciones para los dispositivos pasados.

...
En Unraid, un %%cache pool|cache-pool%% es una colección de una o más unidades, normalmente SSD o HDD de alta velocidad. Estas unidades almacenan datos temporalmente antes de que se muevan a tu [array](./array/overview.mdx) principal. Usar %%cache pools|cache-pool%% puede mejorar significativamente las velocidades de escritura, proteger tus datos y proporcionar almacenamiento dedicado para tareas específicas como ejecutar contenedores Docker o [máquinas virtuales](../create-virtual-machines/overview-and-system-prep.mdx).
...
- **Integración de %%Mover|mover%%:** Los datos escritos en una %%cache pool|cache-pool%% se transfieren automáticamente a tu %%array|array%% principal según un horario que configures. Esto mantiene tus [shares de usuario](./shares.mdx) organizados y fáciles de gestionar.
...
Una vez que su grupo esté formateado como %%BTRFS|btrfs%% o %%ZFS|zfs%%, puede agregar más discos para redundancia y para expandir el almacenamiento. Para obtener instrucciones detalladas sobre cómo agregar discos a grupos existentes, consulte [Añadiendo discos a un grupo](#adding-disks-to-a-pool).
...
3. En la sección **Dispositivos del Pool**, ajuste **Ranuras** al número exacto de dispositivos adicionales.
4. Asigne los nuevos dispositivos a las ranuras disponibles.
5. Inicie el %%arreglo|array%% para habilitar los cambios.

:::note

Después de comenzar el %%array|array%%, %%BTRFS|btrfs%% comienza automáticamente una operación de **Balance** para redistribuir los datos entre todos los dispositivos en el grupo. Este proceso puede tardar varias horas o incluso días, dependiendo del tamaño de su grupo y la cantidad de datos almacenados. Durante este tiempo, su grupo puede parecer ocupado, pero puede seguir usándolo normalmente. Puede monitorear el progreso del Balance en la pestaña **Principal** haciendo clic en el primer dispositivo del grupo y verificando **Estado del Balance**.

:::

### Pools ZFS

La expansión de pools de %%ZFS|zfs%% depende de su tipo de configuración de pool:

- **Pools RAIDZ1/2/3 de un solo vdev:** Un pool con un grupo de discos en una configuración RAIDZ. Se puede expandir un disco a la vez. Vea [expansión RAIDZ](#raidz-expansion) a continuación.
- **Pools en espejo:** Un pool donde los discos están emparejados en espejos. Se pueden añadir pares de espejos adicionales para aumentar la capacidad.
- **Pools multi-vdev:** Un pool con múltiples grupos de discos (e.g., múltiples grupos RAIDZ o múltiples pares en espejo). No se puede expandir añadiendo discos individuales a grupos existentes.

:::important

No todos los pools %%ZFS|zfs%% pueden expandirse añadiendo dispositivos. Solo los pools single-vdev RAIDZ1/2/3 permiten expansión disco a disco. Otras configuraciones de pool requieren agregar vdevs completos, como añadir un nuevo par en espejo a un pool espejado o añadir un nuevo vdev del mismo ancho a un pool RAIDZ.

:::

:::tip[Planning para futura expansión]

Si está creando un nuevo pool %%ZFS|zfs%% de dos dispositivos y planea expandirlo más tarde añadiendo discos uno a la vez, elija **RAIDZ1** durante la configuración inicial en lugar de la configuración predefinida en espejo. Mientras que los pools de dos dispositivos tienen un espejo por defecto (que proporciona redundancia), elegir RAIDZ1 le permite expandir el pool de forma incremental en el futuro sin tener que migrar sus datos. Si no planea expandirse más allá de dos discos, se recomienda la configuración en espejo por su simplicidad y rendimiento.
...
Para expandir un pool RAIDZ single-vdev:

1. Con el %%array|array%% en ejecución, en ***Principal → Dispositivos del Pool***, seleccione el nombre del pool para ver los detalles.
2. En el área de **Estado del Grupo**, verifique si hay un botón de **Actualizar Grupo**. Si existe, deberá hacer clic en él antes de continuar. Tenga en cuenta que actualizar el grupo limitará su capacidad para volver a versiones anteriores de Unraid.
3. Detenga el %%array|array%%.
4. En ***Principal → Dispositivos del Pool***, agregue una ranura al pool.
5. Seleccione la unidad adecuada (debe ser al menos tan grande como la unidad más pequeña en el pool).
6. Inicie el %%array|matriz%%.

:::caution

Si ve una advertencia de "expansión inválida", necesita actualizar primero el pool usando el botón **Actualizar Grupo** mencionado en el paso 2.
...
![Conversión de RAID 1 a perfil único](/img/Btrfs-raid1.jpg)
...
:::info[Troubleshooting problemas de operaciones de equilibrio si se atascan]
...
2. **Proteger datos:** Asignar diferentes niveles de %%RAID|raid%% o sistemas de archivos a cada pool para opciones de redundancia y respaldo personalizadas.
3. **Aislar cargas de trabajo:** Mantenga aplicaciones críticas en pools más rápidos y redundantes y almacene datos masivos en dispositivos más grandes y rentables.
4. **Gestionar con flexibilidad:** Puedes expandir, reducir o formatear pooles de forma independiente sin afectar a otros.
...
Hay ocasiones en las que puede que necesites mover archivos entre tu %%cache pool|cache-pool%% y el %%array|array%% principal, por ejemplo, al prepararte para mantenimiento, actualizar hardware u optimizar el rendimiento. Este proceso también es útil para hacer una copia de seguridad de tu %%cache pool|cache-pool%% antes de realizar cambios de configuración o sustituir unidades. Unraid proporciona una herramienta integrada llamada %%Mover|mover%% para automatizar este proceso para las [shares de usuario](./shares.mdx).
...
Al acceder a una [share de usuario](./shares.mdx) desde varios pools y discos del %%array|array%%, Unraid fusiona los listados de directorios en este orden:

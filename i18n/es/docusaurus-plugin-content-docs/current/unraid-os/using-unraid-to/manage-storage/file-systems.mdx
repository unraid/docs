...
| Sistema de Archivos | Data Protection                                                                       | Space Efficiency               | Características Avanzadas                                        | Mejor Caso de Uso                                                |
| ------------------- | ------------------------------------------------------------------------------------- | ------------------------------ | ---------------------------------------------------------------- | ---------------------------------------------------------------- |
...
| %%ZFS\|zfs%%        | %%RAIDZ1\|raidz1%%/%%RAIDZ2\|raidz2%%/%%RAIDZ3\|raidz3%%, espejos, etc.               | Varies by %%RAID\|raid%% level | %%Checksums\|checksum%%, %%Snapshots\|snapshot%%, %%RAID\|raid%% | Multi-device pools, %%VMs\|vm%%, Docker, advanced users          |
| %%BTRFS\|btrfs%%    | %%RAID 0\|raid0%%/%%RAID 1\|raid1%%/%%10\|raid10%%/%%5\|raid5%%/%%6\|raid6%%, mirrors | Varies by %%RAID\|raid%% level | %%Checksums\|checksum%%, %%Snapshots\|snapshot%%, %%RAID\|raid%% | Pools de dispositivos múltiples, SSD/HDD mixtos, pools flexibles |
...
- Para ayuda de expertos, publique su problema junto con los diagnósticos en los [foros de Unraid](https://forums.unraid.net/).
  :::
...
- Siempre verifique dos veces antes de formatear o reparar unidades para evitar la pérdida accidental de datos.
  :::
...
| **No se encontró una %%BTRFS\|btrfs%% válida**                           | Disk may not belong to a valid pool; check pool assignments.                  |
...
| **Cannot mount /dev/mdX: Structure needs cleaning**                      | El sistema de archivos está corrompido; realiza una reparación.               |
...
- Si tiene dudas, publique sus diagnósticos y mensajes de error en los [foros de Unraid](https://forums.unraid.net/) para obtener asistencia experta.
  :::
...
- Si ve una carpeta `lost+found`, verifique su contenido por si hay archivos importantes.
  :::

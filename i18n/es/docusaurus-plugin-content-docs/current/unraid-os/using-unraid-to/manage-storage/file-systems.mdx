...
| %%ZFS\|zfs%%        | Sin redundancia incorporada                                                           | 100%                                   | %%Checksums\|suma de verificaciones%%, %%Snapshots\|instantáneas%%, %%RAID\|raid%% | Pools de dispositivos múltiples, %%VMs\|vm%%, Docker, usuarios avanzados |
| %%ZFS\|zfs%%        | %%RAIDZ1\|raidz1%%/%%RAIDZ2\|raidz2%%/%%RAIDZ3\|raidz3%%, espejos, etc.               | Varía según el nivel de %%RAID\|raid%% | %%Checksums\|suma de verificaciones%%, %%Snapshots\|instantáneas%%, %%RAID\|raid%% | Pools de dispositivos múltiples, SSD/HDD mixtos, pools flexibles         |
...
- **Mejor para:** %%Array|arreglo%% de discos o grupos de un solo disco cuando busca una alternativa a %%XFS|xfs%%.
...
- **Inicie el array**: Una vez que haya seleccionado los nuevos sistemas de archivos, inicie su %%array|array%% de nuevo.
...
- **Para pools mixtos o en expansión:** %%BTRFS|btrfs%% es ideal si deseas utilizar diferentes tamaños de unidades o agregar o quitar dispositivos fácilmente. Consulta [Pools BTRFS](/es/unraid-os/using-unraid-to/manage-storage/cache-pools#btrfs-pools) para obtener detalles sobre cómo agregar discos a los pools %%BTRFS|btrfs%%.
...
1. Detenga el %%array|array%%.
2. Para obtener ayuda experta, publica tu problema junto con los diagnósticos en los [foros de Unraid](https://forums.unraid.net/).
   :::
...
3. Si el disco está deshabilitado y se está emulando, ejecute la comprobación/reparación en el %%emulated disk|disco emulado%% antes de intentar una reconstrucción. Si el %%emulated disk|disco emulado%% no se puede montar, el disco reconstruido tampoco se podrá montar.
...
2. Inicie el array en el modo correcto:
   ...
   - **Para %%ZFS|zfs%%:** Use **Modo normal** y ejecute un %%scrub|scrub%% para verificar errores. Consulte la [página de almacenamiento ZFS](../../advanced-configurations/optimize-storage/zfs-storage.mdx) para obtener orientación sobre el mantenimiento y procedimientos de reparación de grupos %%ZFS|zfs%%.

3. Si el disco está deshabilitado y se está emulando, ejecute la reparación en el %%emulated disk|disco emulado%% antes de intentar hacer una reconstrucción.

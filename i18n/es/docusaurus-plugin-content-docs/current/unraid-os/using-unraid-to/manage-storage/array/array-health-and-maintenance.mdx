...
Para mayor comodidad, puedes programar estas verificaciones para que se ejecuten automáticamente en intervalos que te convengan navegando a ***Configuración → Programador***. Es recomendable realizar verificaciones automáticas de corrección mensual o trimestralmente para asegurar la salud continua de tus datos y corregir cualquier error de sincronización encontrado.
...
Si tu arreglo no inicia, sigue estos pasos para identificar y solucionar problemas comunes. Busca mensajes de error en ***Principal → Operación de Arreglo***.

...
2. Configura los siguientes ajustes:
   ...
   | **Espacio libre mínimo**                                        | Configura a **1** (1 KB) para obtener los mejores resultados. Esto evita que Time Machine falle cuando los discos se aproximen a su capacidad.                                                                                                 |
   ...
   | **Tamaño de volumen de Time Machine**                           | Configura al tamaño máximo deseado, o déjalo sin fijar. Si no configuras espacio libre mínimo a 1, aplica el recurso primero, luego edita nuevamente y establece el tamaño del volumen a menos del espacio libre mínimo calculado que aparece. |
...
2. Introduce la dirección %%SMB|samba%% de tu servidor Unraid. Por ejemplo, usa el formato `smb://[dirección-IP-del-servidor]/[nombre del recurso compartido de Time Machine]` y haz clic en **Conectar**.
...
4. Abre **Ajustes del Sistema** si estás usando macOS Ventura 13.0 o posterior, o **Preferencias del Sistema** si estás en macOS Monterey 12.x o versiones anteriores. Luego navega a **Time Machine**.
...
8. Time Machine iniciará automáticamente la copia de seguridad inicial. Ten en cuenta que la primera copia de seguridad puede tardar un tiempo, pero las siguientes serán más rápidas ya que son incrementales.
...
1. **Crea cuentas únicas de %%root user|root-user%% en Unraid** para cada persona o Mac que respaldará datos. Por ejemplo, usuarios llamados `larry`, `curly` y `moe`. Esto asegura que cada usuario tenga su propia identidad y permisos.

2. **Crea recursos compartidos de Time Machine %%user shares|user-share%% individuales** para cada usuario, nombrándolos claramente para coincidir con el usuario, como `tm-larry`, `tm-curly` y `tm-moe`. Mantener los recursos compartidos separados ayuda a gestionar el almacenamiento y el acceso para cada usuario.

3. Asigna los permisos de usuario apropiados para cada %%user share|user-share%%:
...
4. Configura Time Machine en cada Mac para conectarse a su %%user share|user-share%% designado:
...
1. Prueba el contenedor de Docker de Time Machine: [Disponible a través de Aplicaciones de la Comunidad](https://unraid.net/community/apps?q=time+machine#r), esta opción puede proporcionar copias de seguridad más estables que las %%user shares|user-share%% %%SMB|samba%% directas.
2. Verifica el firewall de macOS: Desactiva temporalmente el firewall para ver si está bloqueando las conexiones.
3. Crea un destino de respaldo nuevo: Borra y recrea el %%user share|user-share%% de Time Machine si los problemas persisten.

...
2. Introduce la dirección %%SMB|samba%% de tu servidor Unraid. Por ejemplo, usa el formato `smb://[server-ip]/[nombre del recurso Time Machine]` y haz clic en **Conectar**.
   ...
   | **Espacio libre mínimo**                                        | Establezca en **1** (1 KB) para obtener los mejores resultados. Esto evita que Time Machine falle cuando los discos se acercan a su capacidad.                                                                                                            |
   ...
   | **Tamaño de volumen de Time Machine**                           | Establezca el tamaño máximo deseado o déjelo sin configurar. Si no establece el espacio libre mínimo en 1, aplique la partición primero, luego vuelva a editar y establezca el tamaño del volumen a menos del espacio libre mínimo calculado que aparece. |
...
2. Ingrese la dirección %%SMB|samba%% de su servidor Unraid. Por ejemplo, use el formato `smb://[server-ip]/[Time Machine share name]` y haga clic en **Conectar**.
...
4. Abra **Configuración del Sistema** si está usando macOS Ventura 13.0 o posterior, o **Preferencias del Sistema** si está en macOS Monterey 12.x o anterior. Luego navegue hasta **Time Machine**.
...
1. **Crea cuentas únicas de %%root user|root-user%% de Unraid** para cada persona o Mac que realizará copias de seguridad de los datos. Por ejemplo, usuarios llamados `larry`, `curly` y `moe`. Esto garantiza que cada usuario tenga su propia identidad y permisos.

2. **Crea recursos compartidos de %%user shares|user-share%% de Time Machine individuales** para cada usuario, nombrándolos claramente para que coincidan con el usuario, como `tm-larry`, `tm-curly` y `tm-moe`. Mantener los recursos compartidos separados ayuda a gestionar el almacenamiento y el acceso para cada usuario.

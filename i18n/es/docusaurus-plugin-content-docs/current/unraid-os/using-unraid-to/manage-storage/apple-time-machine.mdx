...
2. Configura los siguientes ajustes:
   ...
   | **Tamaño de volumen de Time Machine**                           | Configura el tamaño máximo deseado, o déjalo sin configurar. Si no configuras espacio libre mínimo a 1, aplica el recurso primero, luego edita nuevamente y establece el tamaño del volumen a menos del espacio libre mínimo calculado que aparece. |
...
4. Abre **Ajustes del Sistema** si estás usando macOS Ventura 13.0 o posterior, o **Preferencias del Sistema** si estás en macOS Monterey 12.x o versiones anteriores. Después navega a **Time Machine**.
...
1. **Crea cuentas de %%root user|root-user%% de Unraid únicas** para cada persona o Mac que vaya a hacer copias de seguridad de los datos. Por ejemplo, usuarios llamados `larry`, `curly` y `moe`. Esto asegura que cada usuario tenga su propia identidad y permisos.

2. **Crea recursos compartidos de %%user shares|user-share%% de Time Machine individuales** para cada usuario, nombrándolos claramente para que coincidan con el usuario, como `tm-larry`, `tm-curly` y `tm-moe`. Mantener los recursos compartidos separados ayuda a gestionar el almacenamiento y acceso para cada usuario.

  ...
  items={[
   {
     tipo: "enlace",
     href: "#adding-disks",
     etiqueta: "Agregar Discos",
     descripción: "Expandir capacidad de almacenamiento",
   },
   {
     tipo: "enlace",
     href: "#replacing-disks",
     etiqueta: "Reemplazar Discos",
     descripción: "Actualizar o cambiar discos defectuosos",
   },
   {
     tipo: "enlace",
     href: "#removing-disks",
     etiqueta: "Eliminar Discos",
     descripción: "Descomisionar o reducir el tamaño del arreglo",
   },
   {
     tipo: "enlace",
     href: "#checking-array-devices",
     etiqueta: "Verificación de dispositivos del arreglo",
     descripción: "Monitorear datos SMART y diagnósticos",
   },
   {
     tipo: "enlace",
     href: "#spinning-disks-down-or-up",
     etiqueta: "Hacer girar discos hacia abajo o arriba",
     descripción: "Gestionar giro de discos",
   },
   {
     tipo: "enlace",
     href: "#reset-the-array-configuration",
     etiqueta: "Restablecer la configuración del arreglo",
     descripción: "Reconstruir la estructura del arreglo",
   },
  ]}
/>
...
1. Ir a ***Principal → Dispositivos del Arreglo*** en la interfaz.
...
>
  |                | Borrar                             | Pre-despejar                                 |                      |                         |   |             |
  | -------------- | ---------------------------------- | -------------------------------------------- | -------------------- | ----------------------- | - | ----------- |
  | **Propósito**  | Garantiza la compatibilidad básica | Prueba la unidad y la prepara                |                      |                         |   |             |
  | para su uso    |                                    | **Velocidad**                                | Funciona rápidamente | Proceso mucho más lento |   | \*\*Arreglo |
  | Impacto\*\*    | Funciona en segundo plano          | Requiere que el disco esté fuera del arreglo |                      |                         |   |             |
...
<details>
  ...
  Unraid trabaja con un bit especial llamado *bit de paridad*, que se almacena en cada posición de bit a través de todos los discos de datos y es gestionado por un disco de %%paridad|parity-drives%% dedicado. La %%paridad|parity%% se calcula de tal manera que el número total de bits en cada posición a través de todos los discos (incluyendo el disco de %%paridad|parity-drives%%) sea siempre un número par, un proceso conocido como paridad par.
...
<details>
  ...
  - **Paridad 2:** Implementa paridad Q usando cálculos de síndrome de campo de Galois (estilo Reed-Solomon), comparable a %%RAID 6|raid6%%. Esta implementación de campo de Galois permite que Unraid se reconstruya a partir de fallos de disco simultáneos. Esta característica es significativa para arreglos más grandes, donde aumenta el riesgo de múltiples fallos. La doble %%paridad|parity%% aumenta significativamente la resiliencia sin la sobrecarga adicional de redundancia reflejada.
</details>
...
2. **Detener el arreglo:** A continuación, querrá detener el arreglo. Navegue a ***Principal → Operación de Arreglo*** y seleccione **Detener**.

3. **Desasignar el disco objetivo:** Una vez que el arreglo esté detenido, encuentre el disco que desea reemplazar. Retírelo de su ranura en ***Principal → Dispositivos del Arreglo***.
...
7. **(Opcional) Elija el %%Modo de Mantenimiento|maintenance-mode%%:**
   ...
   - Si escoge el %%Modo de Mantenimiento|maintenance-mode%%, aún debe hacer clic en **Iniciar** en el siguiente paso para comenzar la reconstrucción.
...
1. **Verifique la salud del disco:**
   ...
   - Revise y reconozca cualquier ícono de advertencia en el tablero que indique advertencias %%SMART|smart%%.

2. **Detener el arreglo:** Usa el %%WebGUI|web-gui%% para detener el arreglo.

3. **Desasignar el disco deshabilitado:** Vaya a ***Principal → Dispositivos del Arreglo*** y cambie la ranura del disco deshabilitado a "No Asignado."

4. **Comenzar el arreglo:** Comience el arreglo para que Unraid pueda registrar el disco faltante. El arreglo mostrará el disco como "No instalado."

5. **Verify emulated content (for data drives only)**: Check that the %%emulated disk|emulated-disk%% shows the content you expect.

6. **Detener el arreglo:** Detener el arreglo para prepararse para los siguientes pasos.

7. **Volver a asignar el disco deshabilitado:** Asigne nuevamente el mismo disco a su ranura original.

8. **(Opcional) Elegir [%%Modo de Mantenimiento|maintenance-mode%%](../../troubleshooting/common-issues/data-recovery.mdx#maintenance-mode):**
   - Seleccionar %%Modo de Mantenimiento|maintenance-mode%% puede acelerar el proceso de reconstrucción al prevenir el acceso al arreglo durante la reconstrucción.
   - Mientras está en el %%Modo de Mantenimiento|maintenance-mode%%, no podrá usar el arreglo hasta que la reconstrucción haya terminado.
   - El %%Modo de Mantenimiento|maintenance-mode%% no cambia la forma en que se inicia la reconstrucción—todavía tendrá que hacer clic en **Iniciar** en el siguiente paso.
...
Para comodidad, puede programar estas verificaciones para ejecutarse automáticamente en intervalos que le convengan, navegando a ***Configuración → Programador***. Es recomendable realizar verificaciones automáticas y correctivas mensuales o trimestrales para asegurar la salud continua de sus datos y correg...
...
- **Reordenar discos:** Cambiar el orden de los discos no afectará la paridad1, pero *puede* invalidar la paridad2.
...
1. Navegue a ***Configuración → Ajustes de Disco***.
...
\**Las velocidades NVMe reales dependen de la generación PCIe, enfriamiento y ancho de banda de la red (p. ej., 10GbE se limita a ~1,100 MB/s).*
...
Si su arreglo no comienza, siga estos pasos para identificar y solucionar problemas comunes. Busque mensajes de error en ***Principal → Operación de Arreglo***.

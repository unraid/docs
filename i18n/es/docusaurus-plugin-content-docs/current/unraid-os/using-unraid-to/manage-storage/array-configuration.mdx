...
2. **Detener el arreglo:** A continuación, querrás detener el arreglo. Navega a ***Principal → Operación del Arreglo*** y selecciona **Detener**.
...
<details>
  ...
  - Si ves errores %%CRC|crc-errors%%, generalmente apunta a problemas con el cableado. Ten cuidado con estos errores, ya que se acumulan con el tiempo y no se reinician.
    ...
    :::tip
    Para estar informado sobre problemas de discos, [habilita notificaciones](../../getting-started/set-up-unraid/customize-unraid-settings.mdx#notification-settings) en Unraid. De esta manera, serás alertado de inmediato si algo sale mal, reduciendo el riesgo para tus datos.
    :::
</details>
...
3. **Desasignar el disco deshabilitado:** Ve a ***Principal → Dispositivos del Arreglo*** y cambia la ranura del disco deshabilitado a "No asignado".
...
Para mayor comodidad, puedes programar estas verificaciones para que se ejecuten automáticamente en intervalos que te convengan navegando a ***Configuraciones → Programador***. Es recomendable realizar verificaciones automáticas y correctivas de manera mensual o trimestral para asegurar la salud continua de tus datos y corregir cualquier error de sincronización encontrado.
...
<details>
  ...
  - Si sospechas que alguna unidad está fallando o es poco confiable, es mejor evitar la turbo escritura, ya que depende de que todas las unidades funcionen.
    ...
    :::info
    La escritura turbo es más adecuada para operaciones masivas y escenarios que requieren un alto rendimiento. Sin embargo, puede que no sea la mejor opción para tareas cotidianas si te enfocas en ahorrar energía y minimizar el desgaste de los discos.
    :::
</details>
...
<details>
  ...
  - Con caché NVMe: 250–900 MB/s dependiendo de las limitaciones de la red o la unidad.
    ...
    :::tip
    Considera usar un grupo de caché (múltiples dispositivos) para una mayor redundancia y protección de datos antes de que el %%Mover|mover%% se ejecute.
    :::
</details>
...
Si tu arreglo no arranca, sigue estos pasos para identificar y solucionar problemas comunes. Busca mensajes de error en ***Principal → Operación del Arreglo***.

...
Los contenedores de Docker te permiten ejecutar aplicaciones en entornos aislados y ligeros sin la sobrecarga asociada con las %%virtual machines|máquinas virtuales%%. Unraid simplifica el uso de Docker proporcionando acceso a miles de aplicaciones preconfiguradas a través de [Docker Hub](https://hub.docker.com/) y [Community Applications](/community-applications/). Este enfoque contenedorizado permite a los usuarios de Unraid ejecutar múltiples aplicaciones simultáneamente sin problemas de compatibilidad. También ayuda a mantener el sistema organizado mediante el uso de paquetes de aplicaciones auto-contenidos y facilita la mejora de las capacidades del servidor a través del ecosistema de Docker.

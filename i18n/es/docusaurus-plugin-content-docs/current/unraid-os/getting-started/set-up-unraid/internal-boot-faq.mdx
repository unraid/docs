...
<div style={{ marginBottom: "1.5rem" }}>
  <p>
    <strong>Configuración de un nuevo servidor</strong>
  </p>
...
Cuando usas el arranque interno con un dispositivo USB flash separado para la licencia, el USB de licencia debe poder detectarse por su etiqueta de volumen FAT. Usa la etiqueta predeterminada `UNRAID` a menos que la configuración de arranque establezca un parámetro de kernel `unraidlabel=` coincidente.

Por ejemplo, si el USB de licencia está etiquetado como `MYLABEL`, la configuración de arranque debe incluir:

```text
unraidlabel=MYLABEL
```

Las etiquetas personalizadas deben usar 11 caracteres o menos. Usa letras mayúsculas `A-Z`, números `0-9`, guiones (`-`) o guiones bajos (`_`).

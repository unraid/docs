...
Los ámbitos siguen el patrón: `resource:action`. Ejemplos incluyen `docker:read` (acceso de lectura a Docker), `vm:*` (acceso completo a VMs), `system:update` (acceso de actualización al sistema), `role:viewer` (acceso de rol de visualizador) y `role:admin` (acceso de rol de administrador).
